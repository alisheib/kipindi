...
| **Agent commission 10% · fee VAT-EXCLUSIVE (TZS 118,000) · SLA 5 WORKING days · 5% withholding** | 🔴 **READ ON PRODUCTION 2026-09-09 — three of the four landed, the fourth did NOT.** `SystemConfig["agent.config"]` (updated 2026-09-08T18:00:49Z) reads `defaultCommissionPct 10` ✅ · `maxCommissionPct 10` ✅ · `feeVatTreatment EXCLUSIVE` ✅ · `reviewSlaDays 5` ✅ · `agentWithholdingTaxPct 5` ✅ — **but `feeVatRatePct` is `0`, not 18.** `feeBreakdownFor(100_000, "EXCLUSIVE", 0)` returns `totalTzs 100,000 · vatTzs 0`, so every applicant-facing surface now quotes **TZS 100,000** and registration books **nothing** to `HOUSE:TAX`, against a law that says 118,000 and 18%. The audit chain shows it moved 18 → 0 at 2026-09-08T17:42:24Z inside a save whose evident purpose was the Lipa destination change (`Digital Selcom Bank / 0769777877` → `Selcom LIPA NAMBA - OCEAN ENTERTAINMENT LIMITED / 7006 3747`), and **no screen could show that**, because the audit's `changes` field is the whole posted form rather than a diff. ⛔ **This is a RATE and only Ali can settle it** — RULES §5 step 1 — either production returns to 18% or the law is amended to 0%. §2.10 · `MONEY-GATE-REMEDIATION.md` §7.1 |
