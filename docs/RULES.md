...
| **Agent commission 10% · registration fee TZS 100,000, NO VAT · SLA 5 WORKING days · 5% withholding** | ✅ **READ ON PRODUCTION 2026-09-09 and now SETTLED.** `SystemConfig["agent.config"]` reads `defaultCommissionPct 10` · `maxCommissionPct 10` · `feeVatTreatment EXCLUSIVE` · `reviewSlaDays 5` · `agentWithholdingTaxPct 5` · **`feeVatRatePct 0`**. The last of those contradicted three documents saying 118,000 at 18%, and ⭐ **Ali ruled on 2026-09-09 that the 100,000 is right and the DOCUMENTS were wrong** — the fee bears no VAT. `feeBreakdownFor(100_000, "EXCLUSIVE", 0)` → `total 100,000 · vat 0`. ⛔ NOT retroactive: the one agent registered before the change paid 118,000 and `HOUSE:TAX` holds that 18,000 as a real liability to TRA, never to be backfilled or reclassified. §2.10 · `docs/COMPLIANCE-DECISIONS.md` § 2026-09-09 · `npm run test:agent-fee-copy` |
| **An internal credit never invents a balance** | ✅ **live in code 2026-09-09.** `creditInternal` read `updated?.balance ?? wallet.balance + amount`, so when the wallet write returned null — **no money moved** — it fabricated the figure, wrote a CONFIRMED transaction, stamped `balanceAfter` with it, posted a ledger group, and returned success. §2.10's agent commission was recorded **PAID** on it and 5% withheld to `HOUSE:TAX` on income never received. It now refuses and audits. §2.10 · `npm run test:lock-tx-threading` §2c |
| **A config gate closes only on a read that ANSWERED** | ✅ **live in code 2026-09-09.** `loadConfig` returned `null` for "no row", "no database" **and "the query FAILED"** alike, so all four hydration gates latched on a read that never landed and pinned that container on **code defaults for its whole life**. ⛔ The 2026-09-08 fix (§1.2) fixed only the *concurrent-caller* half and its docblock claimed otherwise — `isPaymentPaused()` still answered false for every rail after one blip. `loadConfigResult` now separates *did the store answer* from *was anything in it*. §2.3 · `npm run test:config-hydration-gate` |
| **The terms state ALL THREE cancellation conditions** | ✅ **live in code 2026-09-09.** `/legal/terms` §4 promised the 5-minute window in EN/SW/ZH and omitted the RUNWAY and NOT-BONUS-FUNDED conditions — a right §2.6 records as unreachable by construction on Up & Down 3- and 5-minute rounds (0 of 688 rounds ever cashed out). All three languages now state the runway condition and name the rounds where cash-out never applies. §2.6 · ✅ **it has a guard now** — `npm run test:terms-cancellation` RENDERS §4 in all three languages and asserts all three conditions, proven RED on the genuine pre-fix page (4 failures, all three languages) |
| **An approved payout carries no phantom gateway id** | ✅ **live in code 2026-09-09.** A withdrawal held in `AML_REVIEW` kept the `wdr_…` correlation id in `providerRef` — **our** id, which no gateway has ever seen. The moment approval flipped it to `PROCESSING` the row became sweep-eligible (the 30-minute grace is measured on `createdAt`, already spent), and the sweep could query that phantom id during the dispatch round-trip, read the default-to-FAILED verdict, and refund a payout **in flight**. Every AML payout is at or above the review threshold by construction. The claim now clears `providerRef`/`payoutRail`, routing the window into the sweep's own `!ref` safe branch. ⚠️ 2026-09-13: withdrawals are no longer held for review (COMPLIANCE-DECISIONS 2026-09-13 third) — this now concerns only a row held before that change and released on `/admin/aml`. §2.7 · `npm run test:aml-dispatch-window` |
...
from VAT-inclusive to **VAT-exclusive** (then **TZS 118,000**; ⭐ superseded 2026-09-09 — the fee bears **no VAT** and an applicant owes **TZS 100,000**), the review
...
| **Registration fee** | ⭐ **TZS 100,000 — NO VAT** (Ali, 2026-09-09). `feeVatRatePct` is **0**, so `feeBreakdownFor(100_000, "EXCLUSIVE", 0)` → `total 100,000 · vat 0` and a registration books **nothing** to `HOUSE:TAX`. `feeVatTreatment` stays `EXCLUSIVE`, which at a zero rate adds nothing — so ⛔ **no surface may state a VAT treatment**: the binding terms and `/agent` go SILENT on VAT when the computed component is 0, because they otherwise rendered "(TZS 100,000 plus TZS 0 VAT)". `/admin/agents` still shows `0%` deliberately, being the officer screen where a wrong rate is caught. ⚠️ `registrationFeeTzs` is what an applicant owes only while the rate is 0; what they owe is always `feeBreakdown().totalTzs`, and every surface that quotes, validates or refunds reads THAT — a guard exists because `agent-application-security` once attested the net against the total and four downstream legs failed as a chain. ⭐ **PAID FROM THE APPLICANT'S WALLET** (Ali, 2026-09-10): they deposit on the ordinary rails, then pay from that balance — no receipt, no typed reference, no officer attestation, because a wallet debit carries the payer's identity by construction and a Lipa/QR payment carries no reference on any network. Still waivable with a typed reason; **refunded in full within 7 days** of a rejection, back to the wallet it came from. ⚠️ **It DOES enter the player ledger** — a `Transaction` plus a balanced group posting to `HOUSE:AGENT_FEE`, whose money-in leg is the **PLAYER** account and ⛔ **never `EXTERNAL:SELCOM`** (those shillings already entered as a DEPOSIT; booking them twice breaks the trial balance). The programme is therefore **inside the money invariants**, not beside them: the debit is all-or-nothing, row-locked and idempotent. `COMPLIANCE-DECISIONS.md` § 2026-09-10. ⛔ **A refund reverses the VAT actually BOOKED, never today's rate** — the application collected at 18% still nets `HOUSE:TAX` to zero. ⛔ **NOT retroactive**: that agent paid 118,000 and the 18,000 in `HOUSE:TAX` is a real liability to TRA. `npm run test:agent-fee-copy` |
...
| **Turning it on** | One word — `inviteRewards: "ACTIVE"`, or `FEATURE_INVITEREWARDS=ACTIVE`. The suites that keep the paid path executable while it sleeps are named in `docs/PLAYER-INVITE-UNPAID.md` §7, and not repeated here, so the two lists cannot drift apart. ⛔ Rewarding referrals is a regulated inducement — clear it with the Gaming Board before flipping it |

Enforced by `npm run test:player-invite-unpaid` with
`npm run red:player-invite-unpaid` (6/6 mutations proven to turn it red). Only the test runs in
`predeploy`. The red control is NOT in `predeploy`, because it mutates the working tree; it is run
by hand, alone or through `npm run red:all`, and CI's `test:red-anchors` checks only that its six
...
|---|---|---|
| 2026-09-25 | ⭐ **The player invite re-opens UNPAID.** `invite` ACTIVE (link, QR, attribution, who-joined count); `inviteRewards` WITHDRAWN: the platform pays player referrers nothing, refused in code above the affiliate config. Cash to an inviter is paid off-platform and recorded nowhere. | §2.10a · `docs/COMPLIANCE-DECISIONS.md` § 2026-09-25 · `docs/PLAYER-INVITE-UNPAID.md` |
