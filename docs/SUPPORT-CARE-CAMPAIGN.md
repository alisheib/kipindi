...
| 10 | 🟡 Routing and chrome — dialability, badges, duplicated constants | ✅ **9/9 — LIVE.** ⭐ the ledger had **7** rows; the §4 prose hid an 8th (10.8) and Ali found a 9th by looking at the live footer (10.9). ⚠️ 10.8 fixes the CLAIM only; its classifier is open and named |
...
| 🟡 | **10.8** ⭐ **A ROW THAT DID NOT EXIST — the §4 prose carried an 8th finding with no ledger line.** CONFIRMED: the local was named `lostBets` and its filter is `BET_PLACED && CONFIRMED` — **every bet, won or lost** — while the officer-facing detail said *"within 30 min of a losing bet"*. ⛔ **AND IT CANNOT KNOW, AT THIS ANCHOR, EVEN IN PRINCIPLE:** the window opens when a bet is PLACED, and the outcome is not decided until its market SETTLES, hours or days later. ⭐ **The false NEGATIVE is the dangerous half** — a player who really chases, depositing right after a bet settles against them, is never caught, because settlement is nowhere near placement; what IS caught is anyone topping up after betting, including a winner. **🟡 HALF DONE, AND THE HALVES ARE NAMED:** ✅ the false CLAIM is gone from the detail string, the local's name and the module docblock — an operator-facing compliance surface no longer states something the code cannot know. ☐ the CLASSIFIER is unchanged and still OPEN: a real loss anchor needs the settled position, which is outside this transaction stream, and the detector walks EVERY user in batches, so a per-user query turns an existing walk into a timeout. ⛔ The marker id stays `CHASING_LOSSES` — it is persisted on existing flags and on the Board-facing RG report, and renaming an enum to fix a sentence would rewrite history officers have already acted on | `responsible-gambling.ts` |
| ✅ | **10.9** ⭐ **A SECOND ROW THAT DID NOT EXIST — ALI FOUND IT BY LOOKING AT THE LIVE FOOTER.** *"Why isn’t our support number there?"* It was not: the Play-safe column published the statutory helpline and our EMAIL and **no phone at all**, so a player who wanted to CALL us had to find `/help` first. Now every page carries `Contact us · 0769777877` (tel: `+255769777877`) **above** `Helpline · 0800 11 0011`. ⛔ **Labelled as OURS and placed ABOVE, never INSTEAD OF** — `HELPLINE()` is the independent problem-gambling line, pinned and unchanged; ours carries no tariff and no "free" claim, because we publish no tariff for the desk (A-5). Threaded as PROPS from `app-shell` (E-226): the footer is `"use client"`, so a getter call there reads the browser bundle’s default and could never show what an officer saved — which is **why the phone was MISSING rather than merely wrong**. ⭐ **No guard could have caught it: §14 asks whether a rendered contact is ACTIONABLE, never whether one is MISSING.** Absence is outside its population by construction — the identical blindness row 10.4 records for `test:i18n` | `public-footer.tsx` · `app-shell.tsx` · `i18n-dict.ts` |
...
- **`email.ts:913`** says *"Amounts **over** TZS 1,000,000 are reviewed"*; `payments.ts:197` holds at
  ...
  ⚠️ 2026-09-13: no longer true — withdrawals are no longer held for review (COMPLIANCE-DECISIONS 2026-09-13 third).
...
- **Dropped as speculative:** a sheet-overflow claim (unmeasured — it is a MEASURE task, not a fix),
  ...
  product adjudication for Ali, not a defect). ⚠️ 2026-09-13: no longer true — withdrawals are no
  longer held for review (COMPLIANCE-DECISIONS 2026-09-13 third).
...
| 9 | **Put our desk number in the footer ABOVE the helpline and label it "Contact us" — and change NOTHING about the helpline** | Replacing `0800 11 0011` with our own number, which is what "use our support number" could have meant | **Ali's instruction, 2026-09-11, read against Unit 2's standing ruling.** ⛔ A swap would have been the campaign's most severe defect re-created at scale: every *"(free)"* becomes a tariff claim we cannot source, `chat.ts` would call our own desk *"independent of 50pick"*, and **RULE 2 would instruct the model to hand our number to a self-identifying problem gambler.** ⭐ The gap he spotted was real; the remedy is an ADDITION, not a substitution | `public-footer.tsx` · `app-shell.tsx` · `i18n-dict.ts` | ⚠️ Touches a responsible-gambling surface — but ADDS a contact, changes no RG claim |
| 10 | **Do NOT extend the same line to `reality-check.tsx`, `global-error.tsx` or the 61 email footers** | Sweeping the new "Contact us" line onto every surface that renders the helpline | **3 — seal the class, and 1 — follow the precedent**, but each for a different reason, which is why this is one row and not three. ⛔ `reality-check` is an RG INTERRUPTION shown beside a Self-exclude button — the operator's number there is the Unit 2 conflation. ⛔ `global-error` carries a documented zero-import rule AND the support phone is OPERATOR-EDITABLE, so hardcoding it would be the Unit 1 defect (a default nobody can update). ⚠️ The 61 email footers already carry a reachable `ReplyTo`, and they are `test:cert-c1`'s inventory — a change there is a comms-certification matter, filed not done | — | No |
