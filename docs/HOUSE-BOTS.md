...
- **Summaries.** Hourly, while the EAT minute is 1–58, for the hour just ended: the admins' summary when automatic stakes passed the per-bet bell cap, and each holder's when their stakes passed their notice cap (0 = summary only). Keys name the hour summarised (`previousHour`).
...
| `gStaffEdgeWinRatePts` | Staff edge: win rate | pts | 1 | 100 | 15 (W16) | ~~Alert when an officer's staff-chosen win rate beats the automated bots' by at least this many points in a month (10 or more settled stakes). Not set = this alert is off.~~ ⛔ **Superseded by D20 (Ali, 2026-09-17):** the staff-edge alert this threshold switched is struck (C5-SPEC rulings 218–223). The column, its CHECK and this `FIELD_META` row are commit 1 code, left as built; what becomes of them is for C5-5b or commit 7's rulings. |
| `gStaffEdgeNetTzs` | Staff edge: net | TZS | 0 | 1,000,000,000 | 100,000 (W16) | ~~Alert when an officer's staff-chosen stakes net at least this much in a month (10 or more settled stakes). Not set = this alert is off.~~ ⛔ **Superseded by D20 (Ali, 2026-09-17):** as the row above. |
...
- **Not exempt:** the two staff-edge thresholds. ~~Clearing one switches an oversight alert off, and that still needs the switch off first.~~ ⛔ **Superseded by D20 (Ali, 2026-09-17):** the staff-edge alert is struck (C5-SPEC rulings 218–223), so clearing one switches no alert off; `CLEAR_EXEMPT` is left as built (`test:house-bot-rules` still pins X-CLEAR-ON for `gStaffEdgeWinRatePts`).
