...
| I8 | An immutable `houseBotId` marker sits on the Position and on every Transaction of it. Reports filter on the row marker, never on current status. Levy identity unchanged. The marker never reaches a public payload. | `test:house-bot-migrations` (commit 1): the two marker columns and their partial indexes. `test:dal-parity` (commit 1): the memory position store's full-row write keeps the stored marker and never takes the incoming one, at source level. `test:house-bot-migrations` §d c21 (commit 1): on Postgres and on memory, a full-row write neither drops a marker nor marks an unmarked stake. | The money suite, commit 2 (a marker on every transaction path, immutable in both stores); the reports suite, commit 5 (absent from every public payload). |
