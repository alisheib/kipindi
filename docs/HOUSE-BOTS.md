...
> 🟠 Being built on branch `house-bots` (commit 1 of 8). Nothing is on production. The master switch ships OFF (D1).
...
|---|---|---|---|
| I1 | Bot bets go only through the bet service. `placeHouseBet` may only **add** refusals and markers, and only `house-bot/fire.ts` may import it (source pin). | — | The seam suite, commit 2: the import pin, and every sanctioned player-path change gives unchanged output with a null marker. |
| I2 | The engine reads only what players can see: pools, status, cutoff/open times, the public Up & Down board (`livePrice`, `openPrice`, targets), and the triggering position plus that account's own positions on that market (anti-abuse). It **never** reads Sentinel fields, staged outcomes, objections, observations or oracle modules, drafts or AI data — except `blackout.ts` and the H3 site, whose only output is `{blocked: boolean}`. | — | The info-edge suite, commit 4: an import-graph walker over the engine modules; `designation.ts`, `eligibility.ts` and `blackout.ts` are exempt by name. |
| I3 | Never bot-vs-bot. Never react to house positions, designated accounts, staff or AGENT accounts. One bot per market, one side per market. | `test:house-bot-migrations` (commit 1): `hbi_counter_anchor_uq` admits one COUNTER per trigger, and `hbt_active_market_uq` one active target per poll. | The seam suite, commit 2 (H2 `OPPOSITE_SIDE`, H3 `OTHER_BOT`); the engine suite, commit 4 (the trigger filter). |
| I4 | Exactly once. A durable intent with a stable key `hb:<intentId>` is written **before** the bet. A duplicate fire is a no-op; a lost timer only delays. | `test:house-bot-migrations` (commit 1): `HouseBotIntent_idempotencyKey_check` pins the key to `hb:` + the intent id, and `HouseBotIntent_idempotencyKey_key` / `HouseBotIntent_positionId_key` are unique. `test:dal-parity` (commit 1): the memory twin throws the same unique violations by index name. | The caps suite, commit 2: a duplicate or reclaimed fire gives one position and one alert. |
| I5 | Master switch and bot state are read fresh at fire time and authoritatively inside the bet's locks. A failed read means no bet. | — | The caps suite, commit 2 (OFF mid-burst: nothing later commits); the engine suite, commit 4 (fire re-reads). |
| I6 | **Every** cap (money and count) is enforced inside the bet's own locks, never only in the engine. | — | The caps suite, commit 2: concurrent bursts stop exactly at every cap code. The declared H2 order is pinned in the seam's anchors file, commit 2. |
| I7 | House stakes are cash only. No bonus spend, no wagering accrual, no agent commission, no cash-out, no objection standing. | — | The money suite, commit 2; the no-reward pin (R4, F9), commit 5. |
| I8 | An immutable `houseBotId` marker sits on the Position and on every Transaction of it. Reports filter on the row marker, never on current status. Levy identity unchanged. The marker never reaches a public payload. | `test:house-bot-migrations` (commit 1): the two marker columns and their partial indexes. `test:dal-parity` (commit 1): the memory position store never drops a marker once it is set. | The money suite, commit 2 (a marker on every transaction path, immutable in both stores); the reports suite, commit 5 (absent from every public payload). |
...
- **Two migrations**, hand-written, re-runnable and expand-only (S1, A23), both timestamped after `20260913120000_kyc_at_withdrawal`, and touching no KYC table:
  - `20260914120000_house_bot_tables` creates the eight tables below and seeds `HouseBotControl` `global` and `HouseBotRuntime` `global` (`ON CONFLICT DO NOTHING`);
  - `20260914120100_house_bot_markers` adds the `houseBotId` markers, the `User` password-history columns and `PredictionMarket.reopenedAt` / `reopenCount`, all nullable with no default.
...
Every house table is on the chain purge's NEVER list (`src/lib/server/chain-purge.ts`, DATA-RETENTION §7.1).
...
| `Position` | `Position_placedAt_id_idx` | `(placedAt, id)`, the sweep keyset; built before the column is added, so it takes only a SHARE lock |
...
- A key ending in an EAT day, hour, month or minute is returned as `{prefix, unit}`, never as a finished string: the claim appends `:` and the suffix inside its INSERT, from DB `now()` (§5.3).
...
| `staffEdge(officerId)` | `staff-edge:<officerId>` | month | The monthly staff-edge alert. It runs only on the first EAT day of a month, for the month just ended, so the month in the key is the month it ran in. |
...
|---|---|---|---|
| 1 · schema, migrations, DAL, pure modules, docs | `test:house-bot-rules`: every bound and cross-field rule, the timing golden rows, the clock, the constant pins. Pure, and in `predeploy`, so it cannot skip. | Every machine and CI | NOT MEASURED |
| 1 | `test:house-bot-migrations`: both migrations applied twice on a scratch Postgres (`db:scratch`), every CHECK tuple equal to the code's, the fixed index and constraint names, and the unique and CHECK refusals. Not in `predeploy`, because it needs a database. | A machine with the embedded Postgres. CI's in-memory job has none, so there it ends without a measurement. | NOT MEASURED; in CI, NOT MEASURED by design |
| 1 | `test:dal-parity`: every house store method exists in both backends, every Stored key is read and mapped with its schema.prisma kind (BIGINT included), and the unique and CHECK names mirror the migrations — at source level. Behavioural parity is `test:house-bot-migrations` §d. | Every machine | NOT MEASURED |
| 1 | `verify:house-bot-migrations-old-build`: the previous build's money path runs on the new schema. It is its own key, **outside `test:all`**: it needs a second checkout with its own install, so it runs at the commit-1 gate and again at REL-0. | The build machine, as its only heavy job | NOT MEASURED |
| 1 | The typecheck, `test:guards-exist`, `test:docs` and `test:red-anchors` | Every machine | NOT MEASURED |
| 2 | The seam, money and caps suites, and the money RED harness | — | Land in commit 2 |
| 3 | The designation suite | — | Lands in commit 3 |
