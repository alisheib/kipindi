...
| **DG-P-03** | 3 | P1 | five h1 systems, and one page with two h1s | `h1 discipline` | 🚢 **double-h1 CLOSED · §T2's named violation CLOSED · 6 skeletons adopt the kit** | this commit | ✅ **THE DOUBLE h1 AND §T2's NAMED VIOLATION ARE BOTH CLOSED — 2026-08-30.** 🔎 Re-derived over all **54** files that render an h1 (literal or via a component, comments stripped): **the double-h1 is exactly ONE page**, `/proposals` — an `sr-only` h1 three lines above a `<PageHeader>` that renders its own. Deleted; the h1 is now "Vote for markets" and `t.proposals.title` survives as PageHeader's eyebrow, so nothing is lost to a screen reader. ⚠️ `app/updown/page.tsx` has TWO `<PageHeader>` call sites and is **not** a second case — mutually exclusive branches, checked rather than counted. ⭐ **AND `DESIGN_AUTHORITY` §T2 NAMED A VIOLATION BY NAME, WHICH IS THE STRONGEST KIND OF WORK ORDER THERE IS:** the market question at `markets/[id]/page.tsx` was a hand-typed `text-[26px] md:text-[34px]`, and §T2 prescribed the remedy in the same breath — *move it onto the ladder, ⛔ never re-tune `--type-h1`.* Now **`text-title-lg md:text-display-3`** (28 → 36): the base is §T2's own page-title step, the size `page-header.tsx` gives its call sites. **§T2 is updated so the rulebook no longer carries a violation it no longer has.** 🔴 **"Five h1 systems" was 17 distinct LITERAL recipes**, and **six of them were loading skeletons hand-retyping `PageHeader`** — three of those missing `leading-tight tracking-[-0.02em]`, so the heading changed line-height AND letter-spacing the instant the data landed. All six adopt the kit: **PageHeader call sites 25 → 31, literal recipes 17 → 16.** 🔴 **The two money forms were worse than a recipe drift — they NAMED THE PAGE TWICE:** `wallet/deposit/loading.tsx` drew eyebrow "Deposit" over an h1 reading **"Loading"**, while the page says eyebrow "Add funds" over h1 "Deposit" (§L1 — one name per destination; `positions/loading.tsx` states that rule in its own header and this file was the counter-example). Both now render the page's own `PageHero` + `PageHeader` with the page's props. 🔴 **And `wallet/loading.tsx`'s header wrapper did not match its page: `flex items-end justify-between` against the page's `flex flex-col … sm:flex-row`, so below 640 the real wallet STACKS and the skeleton did not** — a layout change on the page a player opens to see their money. ⚠️ **`test:spacing-scale` then caught me spreading a defect while tidying:** adopting the kit meant copying a BackLink ghost carrying `w-16` — an INVERTED key (stock 64px against `w-12`'s overridden 128px) — into two more files, 629 → 631. Fixed in all three, ceiling **628**. ▶ **What remains:** four page-level h1s that are neither `PageHeader` nor on the ladder — `admin/totp-verify` 26px · `proposals/[id]` 19px · `admin/resolver/[id]` `text-[18px]` (a rung, hand-typed) · `updown/[roundId]` (no size class at all) — plus the error/offline/not-found family, which is a legitimately distinct system. | |
