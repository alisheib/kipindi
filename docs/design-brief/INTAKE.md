...
|---|---|---|
| **1 · Technical** | the gates below are green **and the change is the one you meant** | `tsc` · `build` exit code · the `test:*` set · plus a **falsifiable check** — the grep or probe that would FAIL if the atom had done nothing. ⛔ **FOR A CSS ATOM THAT CHECK MUST READ THE SHIPPED BUNDLE** — `npm run build && npm run qa:bundle-css`, with the atom's own expectations added to its list. Added 2026-08-07 after **E-127**: a stray star-slash inside a comment dropped an entire rule, and `tsc`, `next build` and all nine design gates were green over it, because every one of them greps **source text**. The source is not evidence |
...
| **both reduced-motion gates + the third** | ✅ **CLOSED 2026-08-07 by ATOM A — E-125 / E-126 / E-127. Read this row for what it now COSTS you, not for work to do.** M6 needs the `prefers-reduced-motion` branch, the `html.kp-reduce-motion` mirror, **and** the third gate `theme-provider.tsx:43` writes for low-end Android, `data-motion="reduced"`. ⭐ **The clamp half is now free**: one declaration set in `src/app/motion.css` serves the OS preference, the in-app switch and the `minimal` tier, and it zeroes `animation-delay` — which no copy did before, so a delayed keyframe held its *first* frame (`.market-grid > *` up to **360ms of invisible grid**; the delivery's `.needle-sweep` 600ms and `.crest-ring-reveal` 340ms would have joined it). ⛔ **The `reduced` tier half is NOT free**: it is a THROTTLE, not a clamp — full durations, ambient loops off — so **every new `infinite` animation needs a hand entry in `globals.css` §6's list**, and `npm run test:reduce-motion` rule 2.1 fails without one. It reads inline `<style>` blocks in `.tsx` too, so a loop hidden in a component is not exempt. ⚠️ **`transition-delay` is deliberately NOT clamped and a check pins it** — a hover-intent delay is intent, not motion |
