...
### 🟢 Laptop A, session 27 (2026-08-05, IN FLIGHT) — ⭐ THE BOARD REBUILT FROM THE GUIDE, AND THE OPERATOR'S HEADLINE NUMBER WAS COUNTING ROUNDS THAT PAID NOBODY

**This block is rewritten at EVERY step, not at the end** — Ali, 2026-08-05: *"make sure at every
step all docs update for future sessions."*
...
⭐ **E-90 IS THE ONE TO CARRY FORWARD, AND IT IS A MONEY-DECISION INPUT.** After the fix the same
live chain reads **`20% 1/5 paid · 4 unmatched`**. Four of five rounds on a real running chain
decided perfectly well and paid **nobody**, because only one side had money in them. The console
had been scoring every one of those as a success. That number is the measurement Ali's
**house-seeds-the-thin-side** decision needs, and until today the platform could not show it.

#### ⏭️ **RESUME AT (session 27, in flight):** ⓪ ⛔ **THE BTC 5m CHAIN `udc_2ba58e2e2c13a7f8` IS RUNNING ON PRODUCTION** and every boundary spends real provider credits — **stop it before you finish** (`/admin/updown` → the chain row → Stop → confirm inside the dialog). It was created through the console from the guide's §9, band *Smallest possible*, and it is the evidence base for E-90. ① **DONE THIS SESSION, DRIVEN LIVE:** §9 steps 1–9 (chain created STOPPED · market open · ±0.02 ·min move · started · rounds on the 6-minute lattice) · **§14.3 one-sided refund** on round #1 — alpha backed UP, the round decided **DOWN**, and the stake came back **in full**, with **21/21 checks green in EN/SW/ZH** and the ledger row reading *"One-sided refund"* · **§14.1 a real winner and a real loser** on round #2 — echo paid **870** on a 1,000 pool (13% capped commission = 130), wallet + ledger + position + round row **exact to the shilling**. ② **STILL TO DRIVE:** **§14.2 no-move refund** (widen the band to *Very wide · 0.50%* through **Edit**, wait for the NEXT round — a round freezes its band at open — then restore it) · **§14.4 operator void** with a real reason from `/admin/updown/rounds` (⛔ the modal is `role="alertdialog"`) · **§14.6 the Feed record filling in** (BTC was at `3 reads · 67% ok · not measured yet` mid-session; watch it cross the 20-reading floor and confirm the Add-chain duration list changes with it). ③ **THE REST OF THE MANDATE, NOT YET STARTED:** every page under `/admin` · the whole player side · the login flows · the consistency sweep. ④ ⛔ **`market-shut gold` CANNOT BE DRIVEN** — it is **Wednesday**; metals follow the FX week (Fri 21:00 → Sun 22:00 UTC). Do not mark it done from a suite. ⑤ 📌 **CLOSED BY OBSERVATION, not by a fix:** the previous handoff's item ③ said the result chip *"still reads VOID · REFUNDED beside OUTCOME ▲ Up"*. Driven on exactly that path today it reads **`STAKE RETURNED`** beside **`OUTCOME ▼ Down`**, with the one-sided sentence between them — **E-87 fixed it and the handoff was stale.** ⑥ ⚠️ **CARRIED, UNANSWERED:** Solana's hardcoded `cautionBelowMinutes` (§6ap) is still **Ali's call** — measurement escalates only, and `UpDownObservation` measures neither price scale nor bar seams. ⑦ 🔴 **NEW, NOT YET FILED — needs a decision:** the **GGR · this game** tile counts **open, unsettled stake as revenue**. With two rounds live it read **TZS 1,000 · hold 66.7%** while the platform had earned **nothing** (one round refunded, one still running); it self-corrects to the true **130** once everything settles. On a product that always has a round in flight the tile is permanently overstated by the open pool. ⛔ **The formula `stakes − payouts − refunds` is shared with the statutory pack, so changing it is a MONEY-REPORTING decision, not a QA fix** — the AWARKEH precedent is *fix by DISCLOSURE, not arithmetic*. 💰 **Money state:** alpha **61,040** (61,540 → −500 → +500 refund → −500 → lost), echo **24,380** (24,010 → −500 → +870). Nothing frozen. 📌 **Harness traps this session paid for, now encoded in the scripts:** the chain's grid label is **`BTC 5m`**, not the dropdown's *"5 min"* · the admin `<thead>` is CSS-**uppercase** so `innerText` reads **`FEED RECORD`** and a case-sensitive match reports a visible column as missing (and then two checks passed **vacuously** over the empty array) · Playwright's `filter({hasText})` matches **unnormalised `textContent`**, so an option's label and hint are concatenated and a `\b5 min\b` pattern cannot match `②5 minBTC has only…` · **Start has no confirm and that is the design** (Stop is the destructive one) · ⛔ **never regex a CSS colour — this design system's tokens are `oklch()`**, and scraping digits reads lightness/chroma/hue as R/G/B; it scored a bright primary button at **1.24:1** and produced three failures that were entirely the harness's own.
