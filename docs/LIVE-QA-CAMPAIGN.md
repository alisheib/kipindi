...
| **E-43** | 🔴 **OPEN** | Up & Down · notifications | **The Up & Down notification suppression is INVERTED: the only outcome a player is ever told about is the one where nothing happened.** `perEventNotificationsSuppressed` is applied at exactly three call sites — bet-placed (`market-service.ts:875`), **WIN (2311)** and **LOSS (2327)** — and is **not** applied to `notifyOneSidedRefund` (2158) or `notifyRefund` (2238). So a win is silent, a loss is silent, and a **refund fires a notification every time**. Measured on production: **0** win/loss notifications have EVER pointed at an `UPDOWN` market, against **70** refund notifications (first 2026-06-25, latest **today 17:25**). ⚠️ That inverts the stated rationale for Ali's 2026-07-24 decision — the volume concern was "forty emails an hour", but under E-32's ladder a **void is the most frequent outcome by design (~1 in 3)**, so the rule silences the two rare, meaningful results and keeps the frequent, meaningless one. It is also worse than plain silence: a player who is reliably told when a round refunds learns that no message means no round, which is precisely wrong. ⛔ Fix alongside **E-37** — they are one decision, not two. | `market-service.ts` 875/2158/2238/2311/2327 · live counts above, each with a control (221 WIN/LOSS notifications exist platform-wide, so the query is not blind) · the 17:35 round settled a real WIN and a real LOSS and produced **0** notifications to either player |
...
off the ladder at all? ⑥ **E-42** the stuck-`GENERATING` reaper.
