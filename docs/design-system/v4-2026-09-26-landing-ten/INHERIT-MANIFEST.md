...
| L21 | Signed-in hero (§4a/§4c): the player's block REPLACES the trust lines; "Your picks" always shows three figures | R4(5) puts the RG line in the hero's trust lines, and a player holding money is who it is for; three zeros are advertised emptiness (the gate's V11); two surfaces disagreeing about a position is DESIGN_AUTHORITY B6 | The trust lines stay for everyone; the player's block replaces only the CTAs. The figures are read exactly as `/positions` reads them (MARKET line, same market join, same dropped rows), so open + awaiting = its Open count; "paid this week" is a win's payout plus a cash-out's proceeds since Monday 00:00 EAT (refunds are returned, not paid). A player with no picks gets one sentence; a failed read shows nothing. The empty-balance line says "mobile money or card" (L19). `src/lib/server/landing-picks.ts`, `test:landing-mine`. |
