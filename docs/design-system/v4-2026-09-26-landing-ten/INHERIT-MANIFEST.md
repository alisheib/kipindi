...
| L18 | Phone order: lede → card → CTAs → trust lines; and "Licence · 18+ · helpline on the first screen" (its own placement map P15, ACCEPTANCE K29) | The two contradict each other: after two full-width CTAs the trust lines start below 740px at 360 | Below 1024 the trust lines come BEFORE the CTAs (one DOM, CSS `order`), still after the card, ordered licence + 18+ → mobile money → RG line + helpline → named sources. The card still ends inside 740 (V15); the header's Sign up keeps the primary action on the first screen. The licence NUMBER stays in the footer (K39 is about the page), as in the delivery's own hero. |
