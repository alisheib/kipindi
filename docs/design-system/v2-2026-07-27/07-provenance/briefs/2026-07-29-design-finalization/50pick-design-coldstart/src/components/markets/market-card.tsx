...
type Props = {
   ...
   * a centred needle, or a "TIPPING" badge (all of which read as "contested"
   * on an empty market and make the whole board look dead). Instead it shows a
   * NEW badge, an em-dash for the probability, a neutral "awaiting first bet"
   * bar, and a "be the first" invitation — honest per the no-fabrication rule.
   *
   * Optional: if omitted, the card derives it from `volume`/`predictors`, so it
   * is correct even without the caller passing it. Pass it explicitly when the
   * board already knows (e.g. from a `createdAt`/`hasBets` flag).
   */
  isNew?: boolean;
  className?: string;
};
...
export function MarketCard({
  ...
  // COLD-START: a LIVE, still-open market with genuinely no activity. Derive it
  // when the caller doesn't pass it, so the fix is correct everywhere the card
  // is used. Honest per A-5: we show emptiness, we never invent a price.
  const fresh = live && (isNew ?? (volume === 0 && predictors === 0));

  const signal = getSignalBadge(status, yesPct, volume, predictors, timeLeft, fresh, {
    hot: t.common.hot, soon: t.common.soon, tipping: t.market.tipping, new: t.market.newBadge,
  });
  ...
  // Localised category label — a Swahili player must not see "SPORTS" over a
  // "MICHEZO" filter chip (POLISH-BACKLOG §1.1). Map from the real category,
  // falling back to "Other"; never a raw English token.
    ...
    <>
      ...
      <div className="mcardp-head">
        ...
        <div className="mcardp-prob">
          {fresh ? (
            // No real crowd price yet — show an honest em-dash, not a fake 50%.
            <div className="mcardp-pct" style={{ color: "var(--text-faint)", fontSize: 22 }}>—</div>
          ) : (
            <>
      ...
      <TippingBar yesPct={yesPct} height={7} resolved={isResolved} showLabels={false} recastOnHover={false} empty={fresh} />
      {fresh && (
        <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, letterSpacing: "0.06em", textAlign: "center", color: "var(--text-faint)", marginTop: 6 }}>
          {t.market.noBetsYet}
        </div>
      )}
      ...
      {/* Trader row — rendered on every card (min-height fixed) so the grid stays
          even. Fresh markets invite the first prediction instead of showing 0. */}
      <div className="mcardp-traders">
        {fresh ? (
          <span className="t-txt" style={{ color: "var(--brand-300)" }}>{t.market.beFirst}</span>
        ) : (
          <>
            {hasTraders && (
