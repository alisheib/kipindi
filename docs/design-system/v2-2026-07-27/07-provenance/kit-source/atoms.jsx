...
const Btn = ({ variant = 'primary', size = 'md', loading, leadingIcon, trailingIcon, children, disabled, style }) => (
  <button
...
const ProgressBar = ({ value, max = 100, tone = 'teal', size = 'md', label, showValue }) => {
  ...
  const tones = {
    teal:    'linear-gradient(90deg, var(--teal-600), var(--teal-400))',
    ...
    <div style={{ width: '100%' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          boxShadow: `0 0 12px -2px color-mix(in oklab, ${tone === 'teal' ? 'var(--teal-500)' : tone === 'yes' ? 'var(--yes-500)' : tone === 'no' ? 'var(--no-500)' : 'var(--gold-500)'} 50%, transparent)`,
        }} />
      </div>
    </div>
  );
...
const CircularProgress = ({ value, size = 56, stroke = 5, tone = 'teal', label }) => {
  ...
  const colorMap = {
    teal: 'var(--teal-400)', yes: 'var(--yes-400)', no: 'var(--no-400)',
