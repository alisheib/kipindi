...
| D78 | 🔵 shipped early `3d644e4e` 2026-09-24 — SIGNED-IN surface; every tap gate was blind to it | U27 |
| D79 | 🔵 shipped early `5b3c6cf1` 2026-09-24 (live) — sort control 33×51 at 320 | U27 |
...
- 🔵 **D79 + D80 — two more controls under the 40px reach floor, both found by MEASURING rather than reading a class.** D79: the `/markets` sort control is 27×44 drawn, **33×51 reach** at 320, because
  ...
  a `Pill` inside a `Link` that contributed no box. Both live (`5b3c6cf1`) with `--tap-min` rungs, both pre-flighted against production before being written.
