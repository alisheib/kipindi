...
| D40 | 🔵 shipped `1ce994b1` 2026-09-25 — the bet panel is first in the SOURCE and carries an `<h2>` of its own; DOM order now equals visual order at 9/9 cells. Guarded by `qa:detail-order-hints` §1 + §2 | U37 |
| D41 | 🔵 shipped `1ce994b1` 2026-09-25 — a 40×40 disclosure button and the explanation in flow: the commission hint went from **23% readable to 100%**, at a layout cost of **0px**. Guarded by §3 + §4 | U37 |
