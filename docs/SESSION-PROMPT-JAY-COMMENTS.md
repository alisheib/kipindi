...
| D | Date beside every timer | #6 | Independent; touches a shared formatter, so land it before more surfaces exist | ✅ **SHIPPED 2026-08-25 (session 62) — `E-203`.** Both clocks on `/markets/[id]` name the instant they count to, on the label row, through one new **pure exported** rule: `formatDeadline` in `src/lib/utils.ts`. 🔴 **The measurement is the finding.** The obvious fix is `formatDayTime` — the helper `/positions` already used — which prints **no year**. Measured on production first: **7 of 51 LIVE markets resolve in a later year on the platform clock**, the furthest **170 days out** (`mkt_0d271bde3ae784abe12b`, 2027-02-10). Beside a three-digit DAYS cell a bare *"10 Feb"* is **the arithmetic #6 exists to remove**, so the year appears **iff** the deadline leaves the reader's platform-zone year. 🔴 **The first census said "3 of 49" and was wrong in the exact way this fix exists to prevent** — Postgres's `naive AT TIME ZONE 'EAT'` interprets rather than converts, so it missed the four markets sitting ON the boundary (`2026-12-31T21:00Z` = 1 Jan 2027 in Dar). Caught by cross-checking against what the live page rendered. Full account in `LIVE-QA-CAMPAIGN.md` §6 `E-203`; ⚠️ commit `7d55dd6b`'s message still carries the wrong figure and cannot be renamed. ⛔ **No third format** — both branches already existed, so the cross-year string is the same-year string plus a year, and the guard pins that. ⚠️ **The zone is the risk §3 named, and it is driven, not commented:** at 2026-12-31 21:30 UTC the Dar clock has already turned to 2027 while the host clock has not, so `sameZonedYear` asks Intl in `tz()` and never `getFullYear()`. ✅ **One home per fact** — the *"results expected by"* line and `/positions`' selection-close line route through the same rule; `formatDayTime` survives only as its same-year branch. **`test:timer-date` 24/0 · `red:timer-date` 7/7 caught, 0 missed** (anchors declared as data; case 5 is the positive control for a vacuous per-timer loop). |
| E | Phone prefilled from the account | #8 | Independent, small | ✅ **SHIPPED 2026-08-25 (session 62) — `E-210`.** Both money forms open with the player's own registered number instead of an empty box behind the placeholder `712 345 678`. ⛔ **A placeholder must never become a value (A-5)** — this is a real default, and the field stays editable. ⭐ **THE RULE IS NOT `sp.msisdn ?? account`, AND THAT IS THE WHOLE JOB.** Both actions carry the submitted values back on failure **only when truthy** — `withdraw/actions.ts:90` and `deposit/actions.ts:46` each omit an EMPTY msisdn — so the naive form would silently refill a field the player had deliberately **CLEARED** with their account number, **on the screen that decides where their money is sent.** Keying on the error instead is exact: a fresh visit prefills, a returning one shows precisely what came back. ⚠️ **A CONVENIENCE, NOT AN ASSUMPTION:** display changed and nothing else — the action still reads the destination from the FORM, never the session, and every existing validation still runs. The rule is the pure exported `moneyFormMsisdn` in `@/lib/phone-normalize`, beside `normalizeTzLocalDigits`, so the stored shape (`+255…`, `255…`, `0…`, bare 9) cannot matter. **`test:msisdn-prefill` 24/0 · `red:msisdn-prefill` 4/4 caught, 0 missed** — §4 pins the carry behaviour the rule depends on, because a rule resting on an assumption about another file goes false when that file changes. |
| F | Cash Back removed | #5 | **Before J** — it changes what the bonus drive must test | ✅ **SHIPPED 2026-08-25 (session 62) — `E-211`.** ⛔ **HIDDEN, NOT DELETED** — Ali's *"disabled/hidden for now until further notice"* is a feature-state. 🔴 **AND THE LIVE STATE WAS THE FINDING: production carries NO bonus row in `SystemConfig` at all, so `DEFAULT_BONUS_CONFIG` IS the live value** — the promo was rendering on `/wallet` AND `/wallet/deposit` offering *"Request a 10% cash back bonus"*, verified on production before the change. That is exactly the precedent this brief warns about. ⭐ **No new flag was invented — `cashbackEnabled` already WAS the feature-state switch**, so the change is one value. ⛔ **NO LEDGER PATH DELETED, and the blast radius is stated in full rather than asserted:** `cashbackEnabled` is read in exactly **three** places — the two display gates and ONE grant branch in `wallet-service.ts` that additionally requires `cashbackMode === "AUTO"`, and the live mode is `REQUEST`, so that branch was already inert. There is **no server action a player can call**: the promo is informational and grants are made by an operator, which is why both live `BonusGrant` rows read `source=ADMIN`. The two existing ACTIVE grants keep their remaining balance and wagering requirement, and `bonus-service.ts` never reads the switch at all. **All five bonus suites green · `test:cashback-hidden` 15/0 · `red:cashback-hidden` 5/5 caught, 0 missed.** ⭐ The mutation worth reading is `ripped-out-entirely`: it switches the WHOLE bonus system off, which hides the promo perfectly — every *"the offer is gone"* assertion passes **harder** — while taking referrals, invites, proposals and both live grants down with it. |
| G | Chain archive + safe delete | #3 | Independent; admin only | ✅ **SHIPPED 2026-08-25 (session 62) — `E-212`.** **Both halves built.** ① **ARCHIVE** — a fourth `UpDownChainState`, additive migration `20260825120000_updown_chain_archived`. The chain leaves the operator's working list and **every round it ever ran is kept**. ⚠️ Players needed **no change**: the board filters on `state === "RUNNING"`, so an archived chain is invisible by construction, exactly as STOPPED already is. ⛔ Archive is refused while the chain is RUNNING — filing a live board away while it keeps taking bets is the opposite of a safe control. Restoring returns it to **STOPPED, never straight to RUNNING**: restoring a board and starting it are two decisions, and the start path re-checks the asset and its trusted source. ② **HARD DELETE permitted only at zero rounds**, refused otherwise **with the count and the remedy**, and ⭐ **the refusal is AUDITED** (`updown.chain.delete_refused`) — that row is the evidence an operator nearly removed a board carrying settlement history, which is precisely what the Board would want to see. ⛔ **Delete is always OFFERED and usually refused, deliberately:** hiding it would teach an operator the control is missing; answering with the count teaches them why it cannot be used. ⛔ **Archiving is NOT reachable through `setChainStateAction`** — it has its own precondition and its own audit action, so letting it in through the start/pause/stop door would make a fourth radio button out of a different kind of act. **`test:chain-removal` 24/0 — it DRIVES the real service against real stores** (a source scan asserting "there is a guard" is E-4's weakest evidence): the refusal with 3 rounds, **nothing destroyed by the refusal**, and ⭐ **the positive control the acceptance asks for by name — a zero-round chain really IS deleted.** **`red:chain-removal` 6/6 caught, 0 missed**; its own positive control makes `deleteChain` refuse EVERYTHING, which passes every refusal assertion harder while the control does not work at all. |
| H | Re-categorise a market | #14 | Independent; admin only | ✅ **SHIPPED 2026-08-25 (session 62) — `E-213`.** Category was set **once, at creation** (`admin/markets/new/wizard.tsx:91`); everywhere else in `/admin/markets` it was only filtered and sorted, so a mis-filed market could be fixed only by **re-creating it** — which on a market already holding stakes is not a correction at all. Built: `recategoriseMarket` (validates against the canonical `MARKET_CATEGORY_SET`, **audits `market.recategorised` with actor, before and after**), an admin-gated action, and a `Select` on `/admin/markets/[id]` **fed the canonical list as a prop** so the console cannot offer what the server would refuse. ⛔ **THE SUBTLE HALF IS `resolvePublishCategory`, WHICH MUST NOT BE USED HERE.** It is a **coercer**: anything unrecognised becomes `other`. Right when publishing a generated poll, catastrophic here — a typed `politics` would come back as a **successful** re-categorisation to `other`, and nothing anywhere would record that the licence-excluded category had been asked for. **An operator who asks for something forbidden must be told no, not quietly given something else.** ⭐ The guard **drives the coercer itself** to prove the trap is real (`resolvePublishCategory("politics") === "other"`) before asserting the service does not use it. ⛔ **No 2FA step-up, deliberately:** `requireAdminTotp` guards the actions that move or lock MONEY; this changes a filing label and the suite pins that the write is `{ ...m, category: after }` and nothing else — the day it starts touching a pool that decision has to be revisited. **`test:recategorise` 28/0**, driving the real service; **`red:recategorise` 8/8 caught, 0 missed**, including the coercer swap and ⭐ a positive control that makes the validator refuse **every** category — the licence is perfectly safe and the correction Jay asked for does not work at all. |
| I | Selcom page: C2B + B2C | #7 | Independent; verify the API first | ✅ **SHIPPED 2026-08-26 (session 64) — `E-220`.** Both balances answered honestly on `/admin/payments`: the disbursement float live from Selcom, and the **collections balance stated as NOT PUBLISHED** with the reason, because it does not exist in the C2B contract (A-5). One statement — money in, money out, net — built from confirmed `DEPOSIT`/`WITHDRAWAL` **only**, reconciled on production against its own SQL by `qa:selcom-statement`. ⭐ **The acceptance's "a guard that fails if a ledger figure is ever labelled a rail figure" is a TYPE, not a spelling check:** every figure carries `source: "rail" \| "ledger"`, `railFloat()` is the only constructor of a rail figure, and the card reads its caption from `provenanceLabel(figure.source)` — so a ledger total cannot appear under a Selcom heading. ⭐ **The conflated figure is ON the page, named as what it is** — `BET_PAYOUT` beside the rail figure, captioned *"This did not touch Selcom"*, with the **29.7×** overstatement spelled out. 🔴 **And my own detector had the defect it was guarding against** — `red` case 8 reported NOT CAUGHT against a genuinely mutated card, because the check matched a phrase only inside a pure JSX text node while the realistic defect sits after an interpolation; the phrases now come FROM `provenanceLabel` so a rewording cannot outrun them. **`test:selcom-statement` 37/0 · `red:selcom-statement` 10/10 caught, 0 missed.** ⬇️ The session-62 measurement it was built on is kept below. 🟡 **THE "VERIFY FIRST" QUESTION IS ANSWERED — MEASURED ON PRODUCTION 2026-08-25 (session 62).** ⭐ **Selcom exposes NO collections (C2B) balance.** The only balance endpoint is `POST /v1/vendor/balance`, and it reads the **disbursement float**. Three independent measurements, not one: ① the probe inside Railway returned `resultcode=000 · TZS 26,385`; ② `data` came back as an array of **length 1** carrying a single `{balance}` — so the suspicion that `selcomFloatBalanceDetailed` was silently dropping a second account in `data[0]` is **disproven, not assumed**; ③ **the drawdown proves which account it is**: the float fell from ~TZS 100,000 (2026-07-31) to TZS 26,385, a fall of ~73,615 against **TZS 70,000 of confirmed payouts** plus charges — while **TZS 646,000 of confirmed collections passed through and never touched it.** Collections settle elsewhere. ⛔ **So the page SAYS SO** (A-5): it must not compute a number from our ledger and label it a Selcom balance. ⭐ **AND THE CONFLATION THE README WARNS ABOUT NOW HAS A NUMBER:** the rail moved **TZS 70,000** out; the internal wallet credited **TZS 2,077,191** in `BET_PAYOUT`. Reporting the second as the first overstates the rail by **29.7×** — on a page built for the regulator. **Build the statement from `DEPOSIT`/`WITHDRAWAL` only.** |
| J | Bonus proved end to end | #15 | **After F**; extends an existing drive | ✅ **SHIPPED 2026-08-26 (session 65) — DRIVEN ON PRODUCTION, END TO END, `qa:bonus-j`.** ⭐ **The whole lifecycle, and every money claim read from `Wallet` / `BonusGrant` / `Transaction`, never from a rendered number.** `locked` **14/0** (0 money moves) · `queue` **6/0** · `promote` **7/0** · `settle`+`verify` **9/0** · `withdraw` **11/1 → `payout` 6/0**. ⛔ **THE WORK ORDER HAD TWO MECHANICS BACKWARDS, and both were measured before a line was written:** ① **turnover accrues at BET PLACEMENT, not at settlement** (`market-service.ts:1266`), so *"a bet that settles, completes the requirement"* is the wrong order — the unlock happens the instant the stake is accepted; ② **cash is spent FIRST** (`:1034`), so a funded player's bonus survives the qualifying bet to be converted — ⚠️ **a player funded BELOW the stake would spend the bonus instead, and the unlock would move NOTHING while still reporting FULFILLED.** 🔴 **AND THE OBVIOUS PLAN WOULD HAVE SILENTLY FAILED:** `sequentialBonuses` is **true** in production (measured — `SystemConfig` holds no bonus row at all, so the default IS the live value), so *"grant a fresh one at 1× so a single bet completes it"* lands **QUEUED** behind the existing grant and can never fulfil. ⭐ **Turned into proof instead of worked around:** the queued grant, then `activateNextQueued` on cancelling the stale one — **§6 of the Management Bonus Rules, exercised on production for the first time ever.** ⭐ **B × J is satisfied and asserted in every leg that pays**, not promised in a comment: `fleet:01` holds **no `KycSubmission` row at all**. ⭐ **Ali's ruling honoured and now enforced by the product rather than by the script** — since `E-215` the destination is not a choice, so running as `fleet:01` IS *"the fleet number only"*; the payout went to `+255799000001`, the rails refused it (`PROVIDER_DOWN`), and the money came back in full with **hold 0**. **Three findings came out of it:** `E-222` (five bonus suites say *withdrawable* and none had ever called `withdraw()`), `E-223` (the money-out screen's commonest refusal said *"That didn't go through"* — **fixed**), and `E-224` (**a bonus cleared at zero risk — ⛔ OPEN, Ali's call**). |
| K | Customer-care surface + mailbox | #12 · #13 | One feature; largest; needs the READ_TIERS design first | 🔵 **READ_TIERS DELIVERED + PROVEN LIVE; the ticket system is NOT (2026-08-26, session 66).** ⚠️ **This row previously read "BLOCKED ON A RULING"; it is corrected here rather than duplicated.** Ali **delegated** §4's five decisions — *"you decide based on how the overall platform works and how it behaves and according to our direction"* — and they are **ruled in `docs/READ-TIERS.md` §4a**, each argued from a measured precedent: **D1** masked, movements yes and totals no · **D2** masked, with the mailbox **inheriting** it as an acceptance condition in §7 (#13 is not DONE until its ticket view asks `canRead`) · **D3** ADMIN **not** exempt, because ADMIN is the only account on production and a rule it skips has no possible witness · **D4** the reveal is audited · **D5** ⭐ **not a wall but a state to CREATE** — mint QA `SUPPORT` **and** `AUDITOR` personas on production, both, since a matrix proven at one row is proven nowhere. ⭐ **THE BUILD FOUND THE AGREED DESIGN SELF-CONTRADICTORY AND §4c RESOLVES IT:** §2.3/D3 say ADMIN is masked while §3.2 grants ADMIN `read` on all four classes — irreconcilable while `read` means *"sees it"*. The cell is now **defined** instead of the grid being changed: `read` = masked at rest **and may reveal**; `masked` = masked, **never** reveals; `—` = not rendered. **ADMIN and SUPPORT therefore render the same dots**, and what separates them is **whether a reveal control exists at all** — stronger, because an absent control cannot be reached by a modified client either. ✅ **SHIPPED `682bfcfa`:** the axis in `src/lib/server/roles.ts`, `test:read-tiers` **25/0** (every refusal paired with a positive control, **two on the SAME role**, since *"SUPPORT sees nothing"* would otherwise pass every refusal), `red:read-tiers` **5/5 caught · restored · green after restore**, `test:red-anchors` **746/0**. ✅ **THE READ_TIERS HALF IS COMPLETE AND PROVEN ON PRODUCTION — `qa:read-tiers` 18/0 (§5a).** Shipped: the `RoleReadGrant` override table (**migration verified applied 2026-08-26T08:23:17Z, 0 rows, so defaults are live**) · `<Sensitive>` + the audited reveal · the `/admin/players/[id]` header wiring · the **Reads tab** on `/admin/roles`. ⭐ **D5 CLOSED BY CREATING THE STATE, NOT BY WAITING:** `ops:mint-read-tier-personas` registered each persona on the REAL sign-up form and had a REAL ADMIN session promote it through `/admin/staff` with a reason — **SUPPORT 0→1, AUDITOR 0→1**, each with a `staff.role_changed` audit row. ⭐ **PROVEN BY REFUSAL WITH POSITIVE CONTROLS IN THE SAME RUN:** SUPPORT gets `a••••@…`, **no reveal control**, and no region; ADMIN on the same page in the same run **does** get the controls; SUPPORT keeps its desk. ⛔ **The raw address is ABSENT FROM THE SERVER'S HTML** (101,269 bytes) for SUPPORT *and* ADMIN — §5.4 asked for that on the response, not the rendered box. ⭐ **AND THE SEAL §5 DID NOT ASK FOR:** ADMIN's real reveal request, replayed from SUPPORT's own session, is refused **HTTP 200** with *"Your role cannot reveal email address (identity.contact)"* — naming the class, with the address nowhere in the response. The refused attempt wrote **no** `pii.revealed` row: a refusal is not a read. 🔴 **AND THE UNIT'S OWN PREMISE WAS WRONG — §1a:** §1 claimed SUPPORT reads five things; **three were already closed by the DOMAIN axis** (`test:player-page-reads` 15/0). The real exposure was **two fields, one role**. ⛔ **§3.2's headline cell cannot deliver its own example** — SUPPORT's `money.figures: masked` needs the transactions tab, which is domain-gated, and §6 forbids widening a route. It is a **CEILING for later, not a narrowing now**, and §1a says so. ⚠️ **THIS ROW IS STILL NOT ✅:** the **ticket system** (#12's larger half) and **`msaada@50pick.tz`** (#13) are NOT built — §7 keeps them out deliberately, and **D2 binds them**: #13 is not DONE until its ticket view resolves `identity.contact` through `canRead`. | ⚠️ §1's measurement stands unchanged: every ACTION on that page is already capability-gated and phone + document number are already masked for everybody; what is open is the **balance, lifetime deposits, email, DOB and region**. |
...
✅ **MEASURED ON PRODUCTION 2026-08-25 (session 62) — do not re-derive this.**
- The Selcom API exposes exactly **one** balance: `POST /v1/vendor/balance` → `data` is an array of
  **length 1**, `{"balance":"26385"}`, `resultcode=000`. There is **no** collections balance in the C2B
  contract — `pushussd` / `query-status` / the three callbacks are per-transaction, not per-account.
- ⭐ **The `data[0]` suspicion is disproven, not assumed.** `selcomFloatBalanceDetailed` reads only
  `data[0].balance`; the probe printed the whole array to check nothing was being dropped. Nothing was.
- ⭐ **Which account it is, proven by drawdown:** float ~100,000 (2026-07-31) → **26,385** now, a fall of
  ~73,615 against **70,000 of confirmed payouts** plus charges — while **646,000 of confirmed collections**
  passed through and never appeared in it. It is a prepaid disbursement float. Collections settle elsewhere.
- ⛔ **Therefore the page says the collections balance is unavailable from Selcom, and shows the collections
  side as OUR ledger, labelled as ours.**
- ⭐ **The conflation now has a number:** rail out = **TZS 70,000** (12 confirmed `WITHDRAWAL`); internal
  wallet credit = **TZS 2,077,191** (289 `BET_PAYOUT`). Calling the second a payout figure overstates the
  rail by **29.7×**. Rail in = **TZS 646,000** (52 confirmed `DEPOSIT`).
- ⚠️ Run the probe as `railway ssh "node scripts/selcom-probe.mjs"`, **never `railway run`** — the latter
  executes locally and every rail answers `4032 not whitelisted`, which reads as "nothing is provisioned".
