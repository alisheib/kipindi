...
| K | Customer-care surface + mailbox | #12 · #13 | One feature; largest; needs the READ_TIERS design first | 🟠 **THE PREREQUISITE IS DELIVERED, THE BUILD IS BLOCKED ON A RULING — `docs/READ-TIERS.md`, 2026-08-26 (session 65).** This row's own *"needs the READ_TIERS design first"* is now satisfied: the design exists, and **§4 is the five decisions only Ali can make** (does support see a balance at all · is the email masked · is ADMIN subject to masking · is a reveal audited · who is the first SUPPORT account). ⭐ **§1 is MEASURED against production rather than assumed** (`rbac-census.cjs`: `RoleDomainGrant` holds **0** override rows, so the code defaults ARE live): `/admin/players` is a `support`-domain route and SUPPORT holds **view+act** on it. **The good news changes the size of the job** — every ACTION there is already capability-gated (`capSupport`/`capMoney`/`capCompliance`) and the phone and document number are already masked **for everybody, including ADMIN**. 🔴 **What is genuinely open:** a support agent reads **every player's exact wallet balance, lifetime deposits, full email, date of birth and region** — none of which answers *"where is my withdrawal?"*, and three of which are the identity set an attacker wants. ⛔ **D5 IS A HARD BLOCKER, not a formality: AUDITOR and SUPPORT hold no account on production at all**, so the refusal test unit K demands cannot run until one exists — *a refusal test needs a session that is actually refused.* ⚠️ **The ticket system itself (#12's larger half) is deliberately NOT designed yet**: it is one more surface that will have to ask `canRead`, so designing it before the tier is agreed would build it against a guess. |
