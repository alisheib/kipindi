...
| 2026-07-28 | **Card shows the winning targets** (PDF/paper-prototype parity). The board card surfaced live price + pool split + open price but never the two target amounts a player must reach — added a "Target to win" band: `Up ≥ $<upTarget>` / `Down ≤ $<downTarget>` + the `± <buffer>` (base × margin). Hardened to a fixed 50/50 grid so a 6-figure asset can't overflow. **Verified by rendering the real board card at 360/768/1280 × EN/SW/ZH** (headless Chrome, seeded frozen targets): overflow 0 everywhere, band correctly absent on an awaiting-price round. Spec `D1-updown-card-spec.md` + `UPDOWN-PRICING.md` updated. | LIVE `9871607` (band) + `7d95fc4` (grid harden); tsc + build clean |
