...
| 2026-07-27 | **Finalize + routing audit**: full retest ×2 (no flakes) + final visual sweep (all surfaces × 4 widths clean); docs (UPDOWN-PROGRESS + new `PLATFORM-DATETIME-FILTER.md`); `*.rar` gitignored. Routing-engineer audit (new `scripts/routing-audit.mjs`, 54 checks): edge-protected `/updown/history` (was a soft 200) — now clean 307; verified 0 dead links app-wide, all 34 admin nav links resolve, auth/admin gating + login `?next=` bounce-back all open-redirect-safe. | LIVE `6e4ea60`, `b80626a`; routing-audit 51/54 (3 explained non-bugs) |
| 2026-07-27 | **History grain + full-flow scan**: consultants found multiple bets on one 5-min round read as a redundant cluster and the "Rounds" KPI counted positions not rounds. `/updown/history` now **groups by round** — one card per round, bets collapse to chips (max 2 + "+N"), round-level KPIs (rounds / bets sub-stat / net-positive win-rate); table → responsive card grid (`scripts/updown-history-shots.mjs`). **Stake floor** — `stakeBoundsFor` + `getBoard` clamp min at the product default so stale chain data can never surface a sub-1,000 "50" preset (test 0.1/0.2). Full-flow visual scan (live cards · board/market cards · round detail · history, EN/SW/ZH × 360/768/1280/1920): one finding — **"Awaiting read" → "Awaiting price"** (ops-jargon + cross-language mismatch; EN/ZH aligned to SW's player-friendly "price"). Untracked a 3rd stray root kit drop (`Needle Fidget Project/`). | LIVE `f2b2fdb`, `65a9d95`, `3502786`; test:all 93/93 · updown-quickbet 28/28 |
| 2026-07-27 | **Anti-tampering + platform responsive consistency**: closed a real tampering gap — the Up & Down card **displayed** per-chain stake bounds (set at /admin/updown) but `buyPosition` enforced only the GLOBAL bounds, so a client could POST a stake the card refused. Now `buyPosition` (for UPDOWN) + `getBoard` both resolve through the ONE `stakeBoundsFor` function (new `stakeBoundsForUpDownMarket(marketId)`); `getBoard` de-duped (inlined floor removed). Behaviorally proven in `updown-engine.test` §8B: a 1,500 stake on a 2k–5k chain is rejected with the chain's own message; a stale stored min 100 is floored to 1,000 on the money path; rejected bets move no money; money still conserves. Full buy-path tampering audit — one entry (`buyPositionAction`, `userId` from session), no alternate money path, dev-test env-gated. **Responsive** (the full-suite `test:responsive` surfaced pre-existing clips): global top-nav **avatar was off-screen at 1024px** (tablet-landscape) on every signed-in page — fixed by relocating the language toggle to the avatar menu at the lg–xl band (one language control per width, mirrors the balance-pill pattern); and the `/live` hero carousel **next-arrow clipped at 320px** — fixed with `min-w-0 truncate` eyebrow + `shrink-0` arrows. | test:all incl. responsive; updown-engine 66/66 |

| 2026-07-28 | **"50pick Dynamic Engine" margin pricing** (the administration's PDF). At open, freeze spot as base and set two winning boundaries `base ± (base × marginBps/10000)`, default **50 bps = 0.5%** (the "50pick factor"): reach `upTarget` ⇒ UP, `downTarget` ⇒ DOWN, between ⇒ VOID + full refund. Targets frozen per round (a config edit never re-prices a live round); legacy/null-target rounds fall back to the old `minMove` rule. Admin-tunable global default + per-chain override + a per-chain **void-rate** readout at `/admin/updown`, all with in-app instructions. Money math untouched (`buyPosition`/`settleMarket`/`payout.ts` never read a price). New doc `UPDOWN-PRICING.md`. | LIVE `7c0f807` (math) → `fd03e88` (detail hero + settlement proof) → `905bbb7` (admin controls); test:all 103/103 ×2, updown-engine 86 / config 77 |
| 2026-07-28 | **Card shows the winning targets** (PDF/paper-prototype parity). The board card surfaced live price + pool split + open price but never the two target amounts a player must reach — added a "Target to win" header (`± <buffer>` = base × margin) over **two colour-coded tiles** that echo the Up/Down bet buttons: `↗ UP · ≥ $<upTarget>` (emerald) / `↘ DOWN · ≤ $<downTarget>` (rose), arrows included per the sketch. Arrow+label stacked over the price → a fixed 50/50 grid where even a 6-figure asset stays on one line, no overflow. **Verified by rendering the real board card at 360/768/1280 × EN/SW/ZH** (headless Chrome, seeded frozen targets): overflow 0 everywhere, tiles correctly absent on an awaiting-price round. Spec `D1-updown-card-spec.md` + `UPDOWN-PRICING.md` updated. | LIVE `9871607` (band) → `7d95fc4` (grid) → tiles refinement; tsc + build clean |
