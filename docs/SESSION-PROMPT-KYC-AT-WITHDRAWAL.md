...
| 6 | `User.status` semantics | ☑ created `ACTIVE`; the migration normalised **11** `PENDING_KYC` rows at apply time (10 when measured that morning) — 0 after, all 22 users ACTIVE |
...
| 13 | Verify — suites incl. non-predeploy, browser drive | ☑ Final battery on `1699c17a`'s tree (2026-09-13 19:32–20:05): `tsc` · 38 guards incl. `test:kyc-gate` 96/0, `test:kyc-at-withdrawal` 94/0 (§B8 deposit screen says nothing about identity), `test:kyc-copy-truth` 318/0, `test:kyc-stage` 104/0, `test:refused-funds` 64/0, `test:refused-funds-race` 16/0, `test:wallet-freeze` 39/0, `test:updown-void-copy` 49/0, `test:rate-copy` 40/0, `test:rules-copy` 71/0, `test:cert-c1` 1145/0, `test:cert-c3` 1447/0, `test:dal-parity` 285/0, `test:control-gates` 281/0, `test:social-panel` 53/0, `test:stacking` 113/0 · `test:motion` 43/0 · **`qa:live` 199/0** (now also asserts no fused words in the rendered body of every public route) · `test:revoked-deadend` PASS. Red only where `c63a4668` was already red, byte-identical: `test:failure-reasons` §10.1/10.2 (agent apply toast) and `test:read-tiers` 7.1 (`/admin/agents` phone). ⭐ **Visual:** ~300 pages / ~980 viewport tiles (9 player states × en/sw/zh × 360/768/1280, signed-out legal and auth, socials, admin 390/1280) read by two full agent inspections (414 + 168 agents, 0 tiles uninspected) and four fix rounds; the final capture checked by eye. Clean-tree red harnesses: see row 14. |
| 14 | Migration applied, pushed, verified on production | ☑ `20260913120000_kyc_at_withdrawal` applied to production **20:08:42, before the push**, by `prisma migrate deploy` (read-only pre-flight: 0 conflicts under the new document-number predicate on both indexes, 11 `PENDING_KYC`, 0 frozen wallets; after: 0 `PENDING_KYC`, `Wallet.freezeReasons` present, both indexes keep a final refusal's number reserved). Pushed `c63a4668..1699c17a` 20:09:36; **`1699c17a` served at 20:12:00** (`?dpl=`). Production checks after the deploy: all 22 users ACTIVE, 0 `PENDING_KYC`; `scripts/live/kyc-at-withdrawal-prod.mjs` (signed out) **49 passed, 0 failed** — R/A not driven (no QA phone/email on this machine; the owner's address is not a test inbox) and O not driven (`ADMIN_DRIVE` revokes the owner's session); the served HTML of 14 public routes × en/sw/zh carries **0 fused words**, and `/legal/terms` no longer says "60 seconds" or "in-app + SMS". Clean-tree red harnesses: `red:kyc-gate` 10 caught / 0 missed (on `1699c17a`) · `red:refused-funds-race` 3/0 · `red:bonus-withdrawable` 9/9 · `red:install-invite` ✅ · `red:kyc-copy-truth` ✅ · `red:refused-funds` ✅ — each left 0 files dirty, run strictly one at a time · `test:red-anchors` 1638 passed / 11 failed = the pre-existing baseline (two anchors this session had drifted were repaired in `f714a6ee`). |
| 15 | `LIVE-QA-CAMPAIGN.md` §6b handoff | ☑ Session 94 block with the next `RESUME AT` — what shipped, the three money defects found in the first draft, the quiet rule and its cost, the visual pass (two full inspections, four fix rounds, the fused-words defect live on production), the verification table and the OPEN / deferred list. |
