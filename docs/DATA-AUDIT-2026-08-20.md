...
| 5 | **F-09 · AIPoll payload prune** | S | Null `rawResponse`/`trace`/`generation` after 30 days, keeping the decision fields; copy `ai-usage.ts`'s opportunistic pattern. 494 of 621 polls are already older than 30 days. ⛔ **Do NOT do the snapshot skip** — verified NOT_SAFE. |
