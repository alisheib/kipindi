...
| **Next move** | ▶ **STEP 5 — `DG-S-01…07`, ADMIN SHAPE & FORMS. It is the only stage left and it is a session of its own.** Ali's commission: admin tabs · unsaved-changes · validation that takes you to the field. Planned in [`SESSION-PROMPT-DESIGN-GATE.md`](SESSION-PROMPT-DESIGN-GATE.md) §🅢, with its craft table, form inventory and sequencing already measured. ⛔ **Read that file's `RESUME AT — session 82` block first**; the **RE-DERIVED 2026-08-30** section below it is now HISTORY for steps 3–4 and the planner rows are newer where they disagree. ✅ **SESSION 82 IS DEPLOYED AND VERIFIED ON PRODUCTION** (`78546e83`): admin drive **43 routes + a 5-route re-drive, `ovf390=no err=0 h1=28` on every one**; player anon **17 routes, all clean**, `/markets/[id]` h1=36; `qa:dg-shell` 40 routes / 77 probes / 0 failing; `qa:toggle-hit` 206 probes PASS; **`qa:install-shown` 63/0 UNMOVED** (DG-P-13 changed the copy, not the gate); `qa:e70-legal` 7/0; `redo` admin OK 49 / REDO 0. 🔴 **BUT THE 13 PAYMENTS MONEY LEVERS ARE UNWITNESSED AND CANNOT BE SEEN FROM HERE** — they render only when a retry has failed or a payout is stuck, and production reads **STUCK PAYOUTS 0**, so the controls are absent from the DOM. That is the `CardSortControl` shape exactly: *an empty production queue renders nothing.* 🔴 **And the responsible-gambling helpline panel — DG-A-21's sharpest repaint — is unwitnessed too**, because that route is authed and every player secret is rejected. Both are named in the RESUME AT block with what it would take to see them. |
