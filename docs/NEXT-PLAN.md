...
| | |
|---|---|
| **State** | ✅ **STEP 1 AND STEP 2 ARE CLOSED. STEP 3 IS UNDER WAY AND ALL 17 OF ITS OPEN ROWS ARE NOW RE-DERIVED.** Planner: **18 ☑ · 3 🚢 · 16 ☐** of 37. Closed 2026-08-30: **DG-A-11 + DG-P-06** — §T3's section eyebrow, the last item in step 2. 📐 **`type-scale` §6 arbitrary tracking 602 → 313** (−289, the largest fall that ratchet has taken), §3 761 → 756, §4 FLAT at 935. ⭐ **The population was READ, not matched: 586 uppercase-and-tracked sites in three passes** — classify, a reader briefed to REFUTE, then a third pass over every control label. **23 calls overturned on pass 2, 12 more on pass 3.** 🔴 **§T3's own table was wrong and is corrected from the read:** not “~410 · 41 · 3 · 5” but **308 · 102 · 5 · 11**, plus TWO families it never named (57 status chips, 55 data values). ⛔ A sweep keyed on the dressing would have moved 112 sites the rule does not govern. ⭐ **And the value is written ONCE**, as `.eyebrow` in globals.css beside `.amount`: converging nine values onto one would have left §6 at 602 forever, because what it counts is a value written at a CALL SITE. 🔴 **Two bugs in my own sweep, both caught by reading the diff:** it wrote `text-transform: eyebrow uppercase` into five inline styles — invalid CSS, so three transactional-email eyebrows would have STOPPED BEING UPPERCASE — and a class cannot reach an inline `letterSpacing` at all. ⚠️ **And §3 rose +291 the moment it landed**, because `type-scale`'s blessing is keyed on `uppercase + tracking-`, THE DRESSING, which the sweep had just replaced with a role marker. `isBlessedMicrolabel` now accepts `.eyebrow` — the better key. Older state: **DG-P-04** · **DG-A-09** · **DG-A-19** (prop half) · DG-P-03's double-h1 and §T2's named violation all closed 2026-08-29/30; the money wall (`.amount`, §M4) and the name collision (§T7) are ruled and guarded. 🔴 **The finding to carry: `space-y-*` IS NOT A GAP.** A sibling selector that counts DOM order, and `.sr-only` is `position:absolute` — so the correct WCAG hidden `<h1>`, written first, hands the first VISIBLE band a full rung nobody wrote (`/live` +24px, `/proposals` +32px, on production). Ruled into §S1; its corollary is that the selector is (0,3,0) and **silently flattens** every `mt-*` inside the container |
