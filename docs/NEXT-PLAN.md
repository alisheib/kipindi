...
**Door: [`SESSION-PROMPT-DESIGN-GATE.md`](SESSION-PROMPT-DESIGN-GATE.md)** — 5 steps, the gates per
step, the traps, **THE PLANNER** (**44 rows** as of 2026-08-31 — the 7 `DG-S-*` step-5 rows joined
it; one row per system, and the only tracker), and the delete-when-done list.

| | |
|---|---|
| **State** | ✅ **STEPS 1–4 ARE CLOSED; STEP 5 IS OPEN AND `DG-S-01` IS CLOSED.** Planner: **32 ☑ · 6 🚢 · 6 ☐** of **44** — and ⭐ **two of the six ☐ are blocked on ALI, not on work** (`DG-P-02`, `DG-P-09`: all six player/officer QA secrets are rejected by production, only `QA_ADMIN_PASSWORD` works). The five 🚢 each name their own remainder in their planner row. 📐 Session 82, all re-derived at the end: `type-scale` §3 **755 → 753** · §4 **934 → 924** · §6 **271 → 241** · inline fontSize flat at 36 · `spacing-scale` **625 → 562** (−63, the largest fall that ratchet has taken, and a SIDE EFFECT — every hand-rolled control that became a kit `<Button>` took its inverted-key padding with it) · eyebrow census **552 → 527** · `contrast-audit` **67 → 69 checks**. Ladder adoption 58.0%. 🔴 **THE FINDING TO CARRY: A GUARD PRINTED `PASS 7.50` OVER A PIXEL THAT RENDERS `2.02`.** §A1 says *measured on its ACTUAL surface*, and a placeholder's rendered ink is `token × alpha × opacity` — while `test:contrast` scored a token against a token. The sign-up DOB field, every `.input` and every `<textarea>` placeholder sat under the AA floor with a green gate over them. ⛔ The §M4 `isTracked = /^tracking-/` shape: a guard reading the SPELLING of a value, not the value that lands on the glass. 🔴 **AND THE FIX WAS ITSELF WRONG, CAUGHT ONLY BY ITS OWN RED CONTROL** — the first `contrastAlpha` blended in linear light and scored the real shipped defect at **5.55 PASS**; browsers composite in gamma-encoded sRGB, where it is **4.07 FAIL**. Build the control before believing the gate. 🔴 **AND: A RULING THAT MINTS A TOKEN IS HALF A RULING.** D2 minted `--success-500`/`--danger-500` on 2026-08-21 so app state would stop wearing the money ink, and named three examples; nine days later all three were still in the betting ramp — including the responsible-gambling **helpline panel**, in the colour that means *your bet won*. Nothing went red, because a guard reading a token's DEFINITION cannot see its ABSENCE. ⚠️ **Two of my own, both caught:** `.row-link` supplies `text-transform`, so putting it on four lowercase pills would have shipped *"8 PREDICTORS"* (caught by reading the diff, with `tsc` silent and every ratchet green); and a class that supplies `uppercase` removes its sites from TWO censuses at once — 8 sites read as **+8 NEW** sub-floor violations with no glyph moved, because both blessings are keyed on the DRESSING. Fixed in `eyebrow-sweep.mjs` (`CARRIER`) and `type-scale.test.mts` (`isBlessedMicrolabel`); ⛔ **the two lists must not drift**, and ⛔ `.eyebrow` is NOT a carrier — it sets letter-spacing only, and listing it inflated the census 318 → 499 by matching every `<PageHeader eyebrow=…>` prop. |
...
| **🔴 THE BIGGEST FINDING OF STEP 5** | **`cn()` WAS DELETING TYPE SIZES BEFORE THEY REACHED THE GLASS**, and it was found by driving production after a regression — not by any static gate. `cn` was bare `twMerge()` with **no config** while `tailwind.config.ts` REPLACES Tailwind's `fontSize` scale, so tailwind-merge filed `text-body-sm` as a *colour* and `text-text` **deleted** it. Not overrode — removed; the element then inherits `body`'s 15px and **no rule anywhere says so**, which is why no cascade probe could find it. 📐 **Measured on production, `/auth/login`: the `PHONE` and `PASSWORD` field legends rendered 15px at 2.1px tracking beside a `SIGN IN` eyebrow at 11px and `FORGOT PASSWORD?` at 10px** — three sizes of one recipe on the sign-in card, in `FieldLegend`, the kit's *own* eyebrow, on the page every player signs in through. ⛔ **§M4/§A1's shape a third time:** `test:type-scale` counts the class at the call site as *"on the ladder"*, so a class that never survived to the browser was scoring as **adoption** — and an arbitrary `text-[13px]` never had the bug, so the ladder migration was replacing something that worked with something that did not. ✅ Fixed at one definition site and held by **`test:bridge` §8** (set equality **plus** a behavioural half, both RED controls proved). ✅ **Re-measured live: PHONE/PASSWORD now 10px at 1.4px.** 🔴 **Blast radius 10 call sites, SEVEN pre-dating step 5 — every one a rendered change that still needs eyes**, two of them money-adjacent (`wallet-balance-pill`, `updown-stake-controls`). Re-derive with **`npm run test:cn-collision`** and **`npm run red:cn-collision`**, both TRACKED and in the pipeline — ⛔ this line used to name `.qa-design-gate/cn-collision2.mjs`, which was GITIGNORED and therefore existed on exactly one machine, the third time this programme handed a future session a file nobody else could have. |
| **▶ STEP 5 — 2026-08-31** | ✅ **`DG-S-01` IS CLOSED AND IT IS LAW: [`DESIGN_AUTHORITY.md`](DESIGN_AUTHORITY.md) §K rule 7,** *"THERE IS ONE SECTION RAIL, AND A TAB IS NOT A FILTER"* — 7a when a page earns tabs · 7b the primitive and what A5 does and does not forbid · 7c geometry/paint/type/motion · 7d what may never go behind a click · 7e the consoles that are NOT tabbed · 7f what tabbing does to the gate population · 7g the gate. 🔴 **THE FINDING: THE ADMIN CONSOLE WAS ALREADY TABBED, TWICE, AND THE STEP-5 PLAN'S OWN PROBE COULD NOT SEE IT** — it grepped `role="tab"`/`aria-selected`, which is **exactly the ARIA `tabs.tsx`'s A5 ruling FORBIDS**, so it returned a true and useless ZERO while `roles/page.tsx:60` and `players/[id]/page.tsx:302` both shipped URL-backed `?tab=` rails with a **byte-identical container class** and divergent items (40 vs 52px · `<Link>` vs raw `<a>` · `aria-current` vs **nothing**). A true measurement over the wrong population, inside the plan itself. 🔴 **Three more plan claims struck at source:** `/admin/roles` is not a page to refrain from tabbing (it is already tabbed, on permission AXES, with its own dated ruling) and its *"50 cards"* is a probe artefact; the payment **kill-switches are not in the control-plane** (`payments/page.tsx:288`, ~4,000px down); and *"only 5 import zod"* is **1** under `src/app/admin`. 🔴 **AND `ui/tabs.tsx` HOLDS THE 4th AND 5th HOMES OF `--pill-active`** (`:99` `oklch(40% 0.08 264 / 0.55)` — a different chroma, hue *and* alpha, so a different answer rather than a copy, which is why `hardcoded-pill-active` cannot see it), **both on variants with ZERO call sites** — so the repair is free today and expensive the moment the primitive is adopted. ✅ **`DG-S-02` IS ALSO CLOSED (2026-08-31):** the primitive is repaired alone, before any adopter — `--pill-active` in both spellings, every hand-typed size onto the ladder, the off-rung `duration-100`/`duration-150` onto `--t-quick`, the count pip hand-written three times and uncapped into the kit `<CountBadge>`, and the header's two false facts. Ratchets lowered in the same commit: `design-frozen` 160 → 159 values, `type-scale` §4 924 → 920 and §3 753 → 751. 🔴 **AND THE COMMIT'S OWN "MOVES NO RENDERED PIXEL" CLAIM WAS FALSE — an A/B bench in the real production fonts refuted it before it shipped:** the `line` variant DOES ship on `/wallet`, and `text-[13px]` → `text-body-sm` carries that rung's `-0.05px` tracking and 18px line-height, so the label measured **92.44 → 92.03px**. ⭐ **Moving a hand-typed size onto its rung is never only a rename — the rung brings tracking and line-height with it** (now §K rule 7c). ✅ **`DG-S-03` PART ONE IS SHIPPED (2026-08-31):** `TabItem` gains `href`, link mode renders `<nav data-section-rail>` + `<Link aria-current="page">`, and `/admin/roles`'s hand-rolled rail is deleted INTO the primitive — rail 41 → 45px with its skeleton moved in the same commit, item 40 → 44px, and the first label's inset 21 → 17px, which now aligns with the card's own content column where the hand-rolled rail never did. 🔴 **AND THE REFACTOR BLINDED A GATE BEFORE IT WAS CAUGHT:** routing all three variants through one shared control moved every height out of its own `<button>` open tag, and `tap-target` §3 reads heights *declared inside an interactive open tag* — so it went red claiming the 36px segment was "fixed (or moved)". It was neither. Link mode is `line`-only now, which §K rule 7c says anyway. ⭐ **A refactor that looks like tidying is how a gate gets one level too shallow.** ▶ **RESUME AT `DG-S-07`, then `DG-S-03`'s tail:** `/admin/players/[id]` cannot convert until `TabItem` carries a `glyph` and a status tone (its rail holds the KYC signal an officer relies on), and the `line` variant needs a scroll affordance — re-measured at 390 through the kit recipe, **576px of content in a 356px box with `elementFromPoint` finding 4 of 6 options reachable** and nothing saying the other two exist. |
| **Next move (superseded detail)** | Planned in [`SESSION-PROMPT-DESIGN-GATE.md`](SESSION-PROMPT-DESIGN-GATE.md) §🅢, whose craft table, form inventory and sequencing were **rewritten 2026-08-31** against the findings above. ⛔ **Read that file's `⏭️ START HERE` block first**; the **RE-DERIVED 2026-08-30** section below it is now HISTORY for steps 3–4 and the planner rows are newer where they disagree. ✅ **SESSION 82 IS DEPLOYED AND VERIFIED ON PRODUCTION** (`78546e83`): admin drive **43 routes + a 5-route re-drive, `ovf390=no err=0 h1=28` on every one**; player anon **17 routes, all clean**, `/markets/[id]` h1=36; `qa:dg-shell` 40 routes / 77 probes / 0 failing; `qa:toggle-hit` 206 probes PASS; **`qa:install-shown` 63/0 UNMOVED** (DG-P-13 changed the copy, not the gate); `qa:e70-legal` 7/0; `redo` admin OK 49 / REDO 0. 🔴 **BUT THE 13 PAYMENTS MONEY LEVERS ARE UNWITNESSED AND CANNOT BE SEEN FROM HERE** — they render only when a retry has failed or a payout is stuck, and production reads **STUCK PAYOUTS 0**, so the controls are absent from the DOM. That is the `CardSortControl` shape exactly: *an empty production queue renders nothing.* 🔴 **And the responsible-gambling helpline panel — DG-A-21's sharpest repaint — is unwitnessed too**, because that route is authed and every player secret is rejected. Both are named in the RESUME AT block with what it would take to see them. |
| **Blocked** | 🔴 **Only the authed PLAYER half, and it is the ONLY thing stopping a 37/37.** All six player/officer QA secrets are rejected by production; only `QA_ADMIN_PASSWORD` works. That blocks `DG-P-02`'s re-measure and makes `DG-P-09` unreprovable — no `auth-login` record was retained and the register's `auth-flash.tsx` hypothesis is refuted (227 client files scanned, zero conditional hooks). ⭐ Everything else that was blocked is gone. |
| **Instruments** | `scripts/design-gate/` — ⚠️ **there ARE npm aliases now**: `qa:dg-type` (the type bench, which runs INSIDE a live production page) · `qa:dg-shell` (the admin landmark/drawer seal, 40 routes) · 🆕 **`qa:dg-rhythm`** (the §S1 band probe — every `[data-measure]` container's computed per-band `margin-top`; `VERBOSE=1` prints the stack, which is what turned *"seven section gaps"* into line numbers) · `qa:dg-money` / `qa:dg-eyebrow` (the sweeps, each REFUSING what it must not decide — and `qa:dg-eyebrow` now LISTS its refusals, which the handover had claimed it did while it printed only a count) · `qa:dg-measure/overlays/shots/analyze/redo`. 🆕 **`npm run test:eyebrow-roles`** — `qa:dg-eyebrow --tracking`, and it is IN `test:all`, because a gate that is not in the pipeline is not a gate. It holds §T3's invariant: every uppercase-and-tracked site in `src/` either carries `.eyebrow` or its role has been READ and written down (`scripts/design-gate/eyebrow-roles.mjs`, 586 sites). ⛔ Its declarations are keyed on the line's CONTENT plus what the element renders — a `:line` key is correct for exactly one commit, and a gate that reports itself stale on every unrelated edit gets deleted. Proved RED both ways: a NEW undeclared uppercase site exits 5, and so does an eyebrow that loses its class. `session.mjs` shares ONE sign-in across every instrument — each login REVOKES the previous, which is the revocation this rig has been fighting. 🔴 **`redo.cjs` reported `OK 48` over EIGHT OFFLINE pages** (HTTP 200, h1 "You're offline") because its only poison test was `/auth/`; it also held a second, drifted copy of the route list. Both fixed |
