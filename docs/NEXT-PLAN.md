...
| | |
|---|---|
| **State** | ✅ **STEP 1 CLOSED. STEP 2 IS DOWN TO ONE SWEEP (its last decision is RULED). STEP 3 IS UNDER WAY AND ALL 15 OF ITS OPEN ROWS ARE RE-DERIVED.** Planner: **16 ☑ · 5 🚢 · 16 ☐** of 37, `test:all` **272/272**. Closed 2026-08-29/30: the 139 off-ladder eyebrows · the 14 off-ladder amounts · **DG-P-04** · **DG-A-09** · **DG-A-19** (prop half) · DG-P-03's double-h1 and §T2's named violation. 📐 §4 **1109 → 935** · §3 **763 → 761** (the first time §3 moved in three sweeps — it falls only when a GLYPH gets more legible) · adoption **50.5% → 58.0%**. ✅ **Verified on production after the deploy:** `qa:dg-rhythm` 16 routes / 0 phantom rungs · `qa:dg-shell` 40 routes / 78 probes / 0 failing · anon drive 17 routes with **`ovf390=no` and `err=0` on every one**. 🔴 **The finding to carry: `space-y-*` IS NOT A GAP.** It is a sibling selector that counts DOM order, and `.sr-only` is `position:absolute` — so the correct WCAG hidden `<h1>`, written first, hands the first VISIBLE band a full rung of margin nobody wrote (`/live` +24px, `/proposals` +32px, measured on production). Ruled into §S1. ⚠️ Its corollary is the next trap: that selector is (0,3,0) and beats any `mt-*` (0,1,0), so adding a rhythm **silently flattens** every per-element margin inside the container. Older state: the TWO DECISIONS (money wall · name collision) are ruled and guarded. 📐 The type ladder crossed half: §4 arbitraries **1809 → 1109**, adoption **19.8% → 50.5%**, and ⭐ **§3 held at 763 through every sweep** — the pair of numbers proving none of them bought a ratchet without moving a glyph. Verified on production (admin 40-route seal + 44-route drive, player anon 17) with **0 overflow and 0 console errors**. ⭐ **THE MONEY WALL:** every Tailwind rung emits letter-spacing, so §M4 had no legal rung — ruled as `.amount` (a doubled selector, because a responsive variant beats a single class). 🔴 The find behind it: `type-scale` §2 matched the WORD `tracking-`, so a money element written `text-micro` was tracked out **+6.67%** and the guard printed PASS over **ten live violations**. ⭐ **THE NAME COLLISION:** not three but **FIVE**, agreeing on no value — frozen as **§T7** with a guard that fails on a sixth. And DG-A-11's blocker was asked of the wrong ladder: `text-micro` IS 10px, so the eyebrow was already on a rung and the "+1px on 254 labels" is never paid. ✅ Also closed: **DG-A-18** (the drawer the "shipped" fix never reached, and 2 routes with no `<main>`) and **DG-A-10** (both parts) |
