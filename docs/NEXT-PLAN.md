...
| | |
|---|---|
| **State** | ✅ **STEPS 1 AND 2 CLOSED. STEP 3 IS UNDER WAY AND ALL 17 OF ITS OPEN ROWS ARE RE-DERIVED.** Planner: **20 ☑ · 3 🚢 · 14 ☐** of 37. ✅ **DG-A-06 closed 2026-08-30 and VERIFIED ON PRODUCTION** — 70 controls over 12 rails and 8 routes, none of which the audit could see: the register's rails were already converted, and the recipe had survived on a DIFFERENT rail behind `{queue.length > 0 && …}`, which an empty production queue renders as nothing. `filter-language` **111 → 183 assertions · 2 → 9 admin rails**, §6.6 re-keyed BEFORE any rank was passed and proved falsifiable ("2 dense of 3 controls"). Admin drive: **44 routes · 0 overflow · 0 console errors**. 🔴 An adversarial pass then found FOUR regressions the conversion had introduced — including §6.7 convicting an innocent `<input>` and the "Custom" chip losing `aria-pressed` — and a fifth instrument that **exited 0 having reached ZERO surfaces**. ✅ **DG-A-14 closed 2026-08-30** — 📐 §3 756 → 804 → 756 · §6 313 → 274. ⭐ **The instrument was narrowed FIRST, which is the whole row:** §3's blessing was keyed on the DRESSING, so 48 sentences sat 2-4px under §T4's floor and counted as labels. ⛔ The register's own guard would have missed 92% of them, and so would a word count — so the population came from the §T3 ROLE READ, which §3 now imports (one list, §0a). 🔴 **9 of the 48 came off the ratchet WITHOUT a glyph moving** — a fixer refused 12 as misclassified, a third reader adjudicated all 12 (8 upheld · 3 overturned · 1 a different defect), and that half of the number is stated rather than folded in. Closed 2026-08-30: **DG-A-11 + DG-P-06** — §T3's section eyebrow, the last item in step 2. 📐 **`type-scale` §6 arbitrary tracking 602 → 313** (−289, the largest fall that ratchet has taken), §3 761 → 756, §4 FLAT at 935. ⭐ **The population was READ, not matched: 586 uppercase-and-tracked sites in three passes** — classify, a reader briefed to REFUTE, then a third pass over every control label. **23 calls overturned on pass 2, 12 more on pass 3.** 🔴 **§T3's own table was wrong and is corrected from the read:** not “~410 · 41 · 3 · 5” but **308 · 102 · 5 · 11**, plus TWO families it never named (57 status chips, 55 data values). ⛔ A sweep keyed on the dressing would have moved 112 sites the rule does not govern. ⭐ **And the value is written ONCE**, as `.eyebrow` in globals.css beside `.amount`: converging nine values onto one would have left §6 at 602 forever, because what it counts is a value written at a CALL SITE. 🔴 **Two bugs in my own sweep, both caught by reading the diff:** it wrote `text-transform: eyebrow uppercase` into five inline styles — invalid CSS, so three transactional-email eyebrows would have STOPPED BEING UPPERCASE — and a class cannot reach an inline `letterSpacing` at all. ⚠️ **And §3 rose +291 the moment it landed**, because `type-scale`'s blessing is keyed on `uppercase + tracking-`, THE DRESSING, which the sweep had just replaced with a role marker. `isBlessedMicrolabel` now accepts `.eyebrow` — the better key. Older state: **DG-P-04** · **DG-A-09** · **DG-A-19** (prop half) · DG-P-03's double-h1 and §T2's named violation all closed 2026-08-29/30; the money wall (`.amount`, §M4) and the name collision (§T7) are ruled and guarded. 🔴 **The finding to carry: `space-y-*` IS NOT A GAP.** A sibling selector that counts DOM order, and `.sr-only` is `position:absolute` — so the correct WCAG hidden `<h1>`, written first, hands the first VISIBLE band a full rung nobody wrote (`/live` +24px, `/proposals` +32px, on production). Ruled into §S1; its corollary is that the selector is (0,3,0) and **silently flattens** every `mt-*` inside the container |
...
| **Next move** | 🔴 **READ `SESSION-PROMPT-DESIGN-GATE.md`'s RE-DERIVED 2026-08-30 SECTION FIRST — all 15 open step-3/4 rows were re-derived and NONE reproduces as written** (14 PARTLY_REAL · 1 REAL_AND_ACTIONABLE), and 14 of the 15 derivations were themselves refuted on a second pass. ⛔ **DG-A-13 and DG-A-21 did not complete and are still un-re-derived.** Two traps to know before opening anything: **DG-A-06's obvious first edit turns a green 111-assertion suite RED** (`filter-language` §6.6 counts `<FilterPill` against `rank="dense"` in one file), and **DG-A-14's 45 sites move §3 by ZERO** unless its blessing test is narrowed first. ▶ Then: **STEP 2's last item is a SWEEP, not a decision** — the eyebrow tracking is ruled at 0.14em (§T3) and what remains is the role-aware pass over ~410 section eyebrows. ✅ The 139 off-ladder eyebrows, the 14 off-ladder amounts and **DG-P-04** all closed 2026-08-29. ▶ What is left is **the eyebrow RECIPE — one decision for DG-A-11 and DG-P-06 together**, and ⚠️ the handover's list was the wrong population: not *"tracking varies 0.12/0.14/0.16/0.20em"* but **106 elements in 74 files, 38 distinct recipes, 23 used exactly once, NINE tracking values** — and the biggest group (0.1em, 32 sites) is spelled two ways, so a text-matching guard reads them as different. 🔴 **`FieldLegend`'s own recipe is not the platform's modal one**, so "adopt the component" is a visible change at most sites. No law names a tracking value → taste, and it needs a screenshot (`qa:dg-type`). Then steps 3 and 4, ⛔ reading RESUME AT + RE-DERIVED first. 🆕 ⛔ **STEP 5 IS PLANNED AND MUST NOT BE STARTED** — Ali's commission (admin tabs · unsaved-changes · validation that takes you to the field), **DG-S-01…07**, a session of its own after 1–4 |
