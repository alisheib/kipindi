...
## ▶ 0 · MOBILE VISUAL PLAN — `MOBILE-VISUAL` · 🟠 **APPROVED 2026-09-15 (v3) · START HERE** · 12/42 units ✅ · 16/90 defects ✅ (D28 · D49 · D33 · D51 · D56 · D31 · D32) · ⛔ **NOT DONE, and the unit count is the OPTIMISTIC number: 29 units not started and 5 of §1a's 9 closure conditions HAVE NOT BEGUN** — U31's 379-item inspection backlog, §11's final production re-measure, U30 on real phones (needs Ali's handsets), the critics re-run at the Seal, and Ali's sign-off on the contact sheet. ⚠️ The register GREW on 2026-09-25 (6 closed, 6 new), which is the programme working rather than failing: 4 of the 6 were invisible to every gate in the repo. ⭐ The surfaces a player meets FIRST are done and production-measured — board, market card, discovery bar, header, home, chat bubble, sign-up funnel, /fairness, signed-in header cluster, market detail. What remains is the depth behind them: overlays (U11 gates 7 units), touch behaviour, short screens and landscape, the keyboard, safe areas, large text, empty/error/offline states · 🔵 shipped early and live: D57 · D63 · D65 · D30 · D7 · **D71–D81** (`371690b6` + follow-up 2026-09-24 — /live's hero resized on a timer for 0.0796 un-input CLS, and three skeletons promised the wrong page) · **D36 · D37 · D45 · D52** (`a7da5f89` + `c6ebbedf` 2026-09-24 — the board priced a settled round at today's quote; the history strip's money moved when you turned the page; Up & Down sat on a 20px gutter where the platform uses 16; the settled pod stranded its arrow at a line end. **D36, D45 and D52 verified on production `4b30a069`.** ⛔ U35 is NOT closed on ONE thing: D37's data path is proved by unit (26 assertions, 6/6 mutations) and its FIT is proved on production's own stylesheet as a DELTA — the old markup spilled 72/52/26px at 320/360/412, the new spills 0 — but nobody has watched the real strip, fed by real rows, hold still while the player pages from 1 to 2. ~~🔴 React does not hydrate in `next dev` on this machine — both dev servers, every route, 4 hydrated elements of ~500 — so no local drive can place a bet.~~ ⭐ **STRUCK 2026-09-25 — DEV HYDRATES** (775/919, 1123/1286, 319/457, 781/931, three reads each; a real click flips React state), so **U35's last step is startable locally again** — seed the store first (`POST /api/dev-test/seed-markets`; the board is empty on boot) and sign in via `/auth/demo`. ⚠️ Why it read 4 on 09-24 is unknown, so probe it every session. Detail in MOBILE-VISUAL-PLAN §0) · **D40 + D41 shipped and verified on production `ee55a509` 2026-09-25** — the bet panel was announced last and had no heading at all; its commission explanation was 1392.8px on one line in a 320px viewport, 23% readable and the rest unreachable behind `overflow-x: clip`. Now 100% readable at a layout cost of 0px, guarded by `qa:detail-order-hints` (171 failures pre-deploy, four RED controls each breaking only its own section). · **D84 filed**: `/leaderboard` still has that defect through the same `Tooltip` atom (14 triggers at 22×22, popovers up to 72px past the right edge) · ◐ half: D31 · D35 · D54 · D30 and D7 shipped early and live (🔵 `a25c127b`) · D31 HALF closed — the mask overflow only
...
| **Shape** | 40 units: **[Compact]** market-board density behind a "Card spacing" switch; **[General]** phone fit for everyone (header, home, chat bubble, popups, questions, toasts, celebrations, notices, notifications, page shells, footer); the hidden phone conditions (short screens/landscape, keyboard, safe areas, large text, skeletons, empty/error states, touch, low-end motion, browser floor, real devices); and the element-inspection units U31–U40. Defects D1–D53. Scored 10/10 through seven lenses in the plan's §13 |
...
## ▶ 0a · MARKETING CAMPAIGN & CONTACTS SETUP — `MARKETING-CAMPAIGN-AND-CONTACTS-SETUP` · 🟢 **BUILDING — S1–S7 done 2026-09-26; U1–U12 are live** · 12/52 units ✅ · 12/25 defects ✅ · 🔴 U9's premise failed (there was no send loop — it is U43's), so it shipped the loop's dispatch step and the contract U43 must pass; U10 found U7's "deciding must not write" fix was half a fix, and that a one-hour break had been refusing marketing for ever under the wrong reason
...
| | |
|---|---|
| **Which machine, which session** | Any PC with a 50pick checkout — find it with `hostname && git rev-parse --show-toplevel && git worktree list` (never copy a path from a doc; a session whose checkout is on another branch or carries another session's edits works in its own worktree off `origin/main` and pushes `HEAD:main`). §0a is the whole contract: the plan file is the planner, and its §0 + §1 are the progress tracker, guarded by `npm run test:marketing-setup-plan` — so a fresh session on a fresh machine knows where the programme stands without memory. ⛔ Not the House Bots checkout, which runs its own programme in parallel |
| **Why** | Marketing needs an admin address book (single + bulk import, duplicate detection, a pre-flight that writes nothing, a progress bar that counts rows written, export) and an SMS broadcast engine. The SMS rail went live on Blackball on 2026-09-16. When this plan was written nothing on the platform checked marketing consent, suppression, opt-out or responsible-gambling exclusion before an SMS; since 2026-09-25 the consent ledger, the suppression list, the one gate, the `/s/<token>` opt-out, the RG standing and the dispatch step that asks the gate per recipient are built (read the plan's §1 for which are live). **No marketing SMS can be sent yet**: the campaign engine is not built. (No Gaming Board approval is needed — Ali, 2026-09-26, OQ1) |
...
| **Law** | ✅ **No Gaming Board approval gate** — Ali, 2026-09-26: the Board says marketing SMS is not part of its approval; PDPA registration is not needed; the helpline is ours (`COMPLIANCE-DECISIONS.md` 2026-09-26 second). What still binds every marketing SMS is consent (ETA s.32, EPOCA reg 7(4)) and no promotion to the self-excluded (GN 478T reg 49(3)) — the gate the engine asks per recipient |
| **Next** | ⛔ Read it from the plan's §0 ▶ NEXT, which is the only authority — this cell said "Session S1 → U1 and U2" for five sessions after S1 had shipped (corrected 2026-09-25). At S7's close (2026-09-26): **S8 → U13 (the send window) and U14 (the frequency cap) — both premises flagged in the plan's §0 before any code**. S7 repaired U6's dropped search indexes (live), closed D10, and shipped U11 + U12 on Ali's delegation. Two units per session |
...
## ▶ 0b · THE LANDING PAGE'S TEN — `LANDING-TEN` · 🔨 **REOPENED 2026-09-26 FOR THE v3 BUILD** · v3 build 0/32 ✅ · the 2026-09-24 gate work below stands
...
| | |
|---|---|
| **v3 build** | The v3 delivery is filed raw at `docs/design-system/v4-2026-09-26-landing-ten/`; its `INHERIT-MANIFEST.md` holds Ali's four rulings of 2026-09-26 (R1 Wallet sheet · R2 pick slip · R3 no estimate on cards · R4 eight small calls) and the fifteen places repo law beats the delivery. Read the next move from `LANDING-TEN.md` §0, never from this cell. **Rule:** every session rewrites §0 and ticks §1 in its own commits and updates this row's count in the same commit — `npm run test:landing-ten-plan` asserts the two agree |
...
## 1 · THE DESIGN GATE — `DESIGN-GATE-2026-08-28` · ✅ **CLOSED 2026-08-31**

**All 45 systems are ☑.** The programme ran to completion and deleted its own door file
(`SESSION-PROMPT-DESIGN-GATE.md`) per its closing ceremony, because that file said it was the
only home of the state and must not outlive the last open row.

⭐ **Everything still TRUE from it is in [`DESIGN-BASELINE.md`](DESIGN-BASELINE.md)** — what the
design system is now, which guard holds which rule and how to make one go RED, what is
deliberately left hand-typed and why, the blind spots no gate can see, and the method for
deciding any value. ⛔ Read it before adding design work.

The two measured registers are kept and marked ⚪ SPENT:
...
done; the measurements stay readable, the instructions no longer apply — and several of their
findings were overturned by re-derivation rather than fixed, each struck at its own entry.

⚠️ **One thing that outlived the programme:** `design-brief/design-gate-2026-08-28/` holds an
OUTBOUND commission that was **never sent** (Ali, 2026-08-31). It is the only copy of that
request, so the ceremony's "delete it" step was deliberately NOT run. ⛔ Do not tidy it away.
...
| **320px account menu** | 🔴 **RE-MEASURED ON PRODUCTION 2026-09-11 AND IT IS WORSE THAN "A TIGHT FIT" — THE ACCOUNT MENU IS OFF-SCREEN AND UNREACHABLE.** Signed in at 320px: the right cluster (`div.shrink-0 flex items-center gap-1`) renders **273px wide at left=66, right=339** in a 320px viewport. The avatar button sits at **left=299 → right=339 — 19px past the edge.** ⛔ **AND `document.scrollWidth === clientWidth === 320`, so the page does NOT scroll horizontally: there is no gesture that reaches it.** Contents, in order: the language control (EN/English/Kiswahili/中文), the balance capsule (`TZS 423,857`), the avatar. Confirmed by `qa:fit` on **17 routes** (`/markets`, `/live`, `/results`, `/leaderboard`, `/updown`, `/help`, `/fairness`, every `/legal/*`, `/proposals`) — one signature, every one of them. ⭐ **This is the same CLASS as `E-190`, which was fixed**: E-190 severed the menu at 1024–1090, this severs it at 320. The board's old figures ("275px in 238px") were RIGHT and were still live. ✅ **FIXED, SHIPPED AND VERIFIED ON PRODUCTION 2026-09-11 (`ee3cc696`) — cluster 273 → 255px, avatar right edge 339 → 317, `overBy −3`, `docScroll === 320`.** ⚠️ **CORRECTION: the pre-deploy simulation said 313 / 7px clear and it was WRONG — the live figure is 317 / 3px.** The injected CSS used a literal `padding: 8px` for `px-2`, but **this repo OVERRIDES Tailwind's spacing scale** (`tailwind.config.ts`): `px-2` is **12px** here and `px-3` is 16px, so the simulation credited the change with 8px it never had. ⭐ **The same override that makes `h-9` render 64px — the warning written in half a dozen comments in this tree — and a simulation is exactly where it bites, because injected CSS does not go through the scale.** The measured-on-production number is the one that counts. ⛔ **AND A FALSE ALARM WORTH RECORDING:** a 7-figure balance was measured at `overBy +10` and looked like a regression — but that test injected raw text and **bypassed `formatBalancePill`, which compacts at exactly 1,000,000**. Re-tested with strings the app can ACTUALLY produce: `TZS 999,999` is the widest possible (11 chars, `tabular-nums`) and measures **identical to today, −3**; every compact form is narrower (`TZS 1.2M` → −12). The pill's width is bounded by design and −3 is the true worst case |
| | 🔎 **How it was proven, since "it fits on my screen" is not proof:** signed-OUT discriminators first (no login, no session revoked) — language trigger **58 → 44px**, caret `display:none`, bar padding **16 → 12px** — then signed in at 320 for the cluster. A BEFORE baseline was taken against the still-old container so the AFTER could discriminate | ⭐ **AND NOTHING PROTECTED YIELDED.** The 14px came from `LanguageMenu` keeping its OWN promise: that file calls itself a **44×44** control in three places and rendered **58** — `minWidth: 44` is a floor the caret overflowed. The caret hides below `sm` (the control is bordered, is a `<details>` and reads `EN`, so the affordance survives), the avatar's duplicate `ml-1` becomes `sm:ml-1`, and the bar's gutter is `px-2` below `sm`. ⚠️ **Verified by SIMULATING the three edits on the live page and re-measuring** — not by arithmetic |
