...
| **Next move** | **Step 2 (tokens & classes)** — the ladder ruling, the three label classes, card rungs, the shell pass, section rhythm. ⛔ **Read `SESSION-PROMPT-DESIGN-GATE.md`'s RE-DERIVED section first**: nine step-3/4 register claims do not reproduce, eight more contradict a guard, and **DG-A-01's filed cause is wrong** — the 88s is a ~13,000-query N+1 in `report-money.ts:392-396` that also slows `/admin/insights`, and the primitive to fix it already exists |
