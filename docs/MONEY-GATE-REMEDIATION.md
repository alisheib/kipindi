...
- ~~**`HOUSE:TAX` is captioned "RETIRED — historical rows only" on the owner's money screens while
  ...
  includes money owed to the state.~~ ✅ **FIXED §7.15** (`LEAD-F.1`/`F.2`, `781f397e`).
- ~~The statutory levy report derives from window GGR rather than `levySplit`.~~ ✅ **FIXED
  2026-09-25** (`LEAD-F.3`) — the /admin/finance tile and `buildDailyOps` READ the booked
  `HOUSE:TRA_LEVY` + `HOUSE:GBT_LEVY`.
...
| `LEAD-A.2` | 🟠 | ❌ **REFUTED §7.5 (3/3)** — closed from the ledger side by §6.1. ⭐ Re-confirmed 2026-09-25 at `360935a3`: the payload's `levies` IS `settleLevies`, and the ledger books `allocateFeeShares(…, settleLevies.traLevy / .gbtLevy)`, which sums to it exactly — one `levySplit`, one figure; `test:levy-allocation` 26 ok / 0 failed, `red:levy-allocation` 7/7 caught. ⛔ Do not "fix" it — ~~The tamper-evident audit chain records a levy figure the ledger never booked — two authoritative records of the same statutory liability, disagreeing by construction~~ | `src/lib/server/market-service.ts` → `settleMarket — the `market.resolved` audit payload` | certain · yes |
...
| `LEAD-F.1` | 🟠 | ✅ **FIXED §7.15** (`781f397e`, CONFIRMED 3/3) — `tax` joined `HouseAccounts` and `owedToOthers`; re-confirmed 2026-09-25, `test:house-solvency` 21/0 — ~~The solvency line omits HOUSE:TAX, so unremitted VAT and withholding tax are presented to the owner as his own free cash~~ | `src/lib/house-book.ts` → `housePosition` | certain · yes |
| `LEAD-F.2` | 🟠 | ✅ **FIXED §7.15** (`781f397e`, CONFIRMED 3/3) — both captions now read "Statutory tax held — owed to the state, NOT ours"; and since `64d0ee80` (2026-09-25) the `finance-window` export prints every `HOUSE:*` balance, `HOUSE:TAX` included, so the "no report reads it" half is closed too — ~~HOUSE:TAX is captioned "RETIRED — historical rows only" on both owner money screens while two live paths credit it, and no report in the repo reads the account at all~~ | `src/app/admin/house/page.tsx` → `ACCOUNT_NOTE (and HOUSE_ACCOUNT_NOTE in src/app/admin/financ` | certain · yes |
| `LEAD-F.3` | 🟠 | ✅ **FIXED 2026-09-25** (both sites) — The statutory TRA/GBT figure is levied on window GGR instead of on the booked fee, so it charges the whole pool as tax in the window the bets are placed and zero in the window they settle. **Measured on production:** GGR 803,675 against 8,796 of levy actually booked — the tile printed 120,551, ~14×. ⚠️ And the obvious repair was ALSO wrong: `HOUSE:COMMISSION` is already NET of the levies (each settlement credits the fee then debits `SETTLEMENT_TRA_LEVY`/`SETTLEMENT_GBT_LEVY` back out of it — all-time credits 63,651, debits −9,523 = TRA 6,320 + GBT 3,203 exactly), so levying that balance understates by 15%. No formula reproduces the booked number at all: each settlement rounds its own levy, and `WITHDRAWAL_FEE` sits in the same account carrying none. **Both the tile and `buildDailyOps` now READ `HOUSE:TRA_LEVY` + `HOUSE:GBT_LEVY` movement for the window** and compute nothing; a failed ledger read OMITS the levy lines rather than printing 0. ⚠️ **Guard status corrected 2026-09-25:** this row used to say it was "guarded by `test:report-cells` and the de-vacuumed `reports-verify-live` daily-ops check". Neither guards it — report-cells never touches a levy, and the verify-live check is CIRCULAR (the builder computes net from the same three figures it then compares, so it would also pass on the computed-levy code). The ledger-read assertion is `verify:reports-live` — see `SESSION-PROMPT-FINANCE-SEAL.md` §2. | `src/app/admin/finance/page.tsx` → `AdminFinancePage (and buildDailyOps in src/lib/server/report` | certain · yes |
...
| ~~`LEAD-F.3`~~ | ~~TRA/GBT presented on window GGR instead of on the booked fee~~ | ✅ **FIXED 2026-09-25** — see the `LEAD-F.3` row in the findings table |
