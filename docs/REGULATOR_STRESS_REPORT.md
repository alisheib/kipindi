...
| Two-officer resolution (the default) | Stage 1 + stage 2 must be different officers. The codepath enforces it. This is the default resolution mode (`resolutionMode: "human"`). An operator may switch a market — or the platform default — to **auto** mode on `/admin/resolver-queue`, in which case the AI seals the outcome without the ceremony, but **only** when its confidence clears the configured floor (`resolveConfidenceThreshold`, default 90); low confidence or UNKNOWN always falls back to the human ceremony. An auto seal is stamped to a named system actor and audited as `market.autoresolved`, and it changes **only who seals the outcome** — the objection window, the winner-floor and every money gate below are unchanged. Owner decision of 2026-07-24, recorded in `docs/COMPLIANCE-DECISIONS.md`. |
| 24-hour objection window | Implemented in `src/lib/server/market-service.ts`. Settlement is genuinely gated on `objectionsClosedAt`: adjudication (stage-2 of the ceremony, or an auto seal) records the verdict but moves **no money** — every position stays OPEN and the pool stays whole. Payment is **per-market and timer-driven**: adjudicating a market arms that market's own settlement timer for its `objectionsClosedAt` (`src/lib/server/market-scheduler.ts`), and when the timer fires `settleMarket()` pays only once the window has elapsed **and** no objection is standing against it — both re-checked under the market lock, alongside the winner-floor, exact conservation and the `settledAt` idempotency stamp, so a re-fire (or a second instance) can never pay twice. A ~5-minute reconciler re-arms any market whose timer was dropped, `/admin/system` shows the live scheduler health (timers armed, next fire, anything stuck in "Ready to settle"), and `/admin/settlement` remains the human fallback where an officer can settle a due market by hand. A player who disputes a verdict therefore does so while the money is still in the pool, and an upheld objection can VOID or REVERSE the outcome before anyone is paid. Window length is admin-configurable at `/admin/config` (`objectionWindowHours`, default 24). Proven end-to-end by `scripts/settlement-gate.test.mts`. |
...
6. **The two-officer settlement** demo — show how the resolver-queue requires different officers for stage 1 and stage 2. This is the default (`resolutionMode: "human"`); if auto-resolution has been enabled for the platform or for a market, also show the mode control on `/admin/resolver-queue`, the confidence floor that must be cleared, and a `market.autoresolved` entry in the audit chain — and be clear that an auto seal changes only who seals the outcome, not the money gates in point 7.
7. **The 24-hour objection window** — show how a resolved market sits in a public-objection state, with its pool intact and every position still OPEN, **before** paying out: resolve a market in `/admin/resolver-queue` (the toast says "pays out \<when\>, unless a player objects"), show the player's `/markets/[id]` panel reading "Payout is on hold", file an objection from that player, then show `/admin/objections` reporting the money frozen and the market refusing to settle until an officer rules. Also show `/admin/system`, where each adjudicated market's own settlement timer is listed with its next fire — payment is per-market and time-based, not a global switch an operator can hold open or force early.
