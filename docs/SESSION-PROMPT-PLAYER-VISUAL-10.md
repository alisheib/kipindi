...
| 3 | PV-13a · eye toggle off `h-[42px]` | 13 | Code | ☑ | `wallet-balance-pill.tsx` (not `top-app-bar.tsx` — filed one file off) | `test:tap-target` **§6** (new) · `red:tap-rung` | `e14394d7` | `.qa-shots/pv13-local/` | ✅ **LIVE — and it had been live since `e14394d7` while three handoffs said otherwise.** The `pv10/rows-3-4-6` branch was **already in `main` by content**; its only remaining `src/` delta was the needle files, where `main` was NEWER, so merging it would have **reverted 127 lines** of the parallel session's work. Checked by content, never by the branch's name or its ahead/behind count |
| 4 | PV-13b · `mcardp-info` → 44 | 13 | Code | ☑ | `globals.css .mcardp-info` | `test:tap-target` **§6** (folded in with row 3, not `test:design-frozen` — see session-6 note) | `e14394d7` | `qa:card-geometry after-deploy` | ✅ **LIVE, and the owed measurement is taken.** `qa:card-geometry` against `50pick.tz` 2026-09-04 reads `heights=[278,320,**347**,354]` at 360/1280/1920 — the `MARKET_CARD_H` **349 → 347** this row predicted, confirmed on production rather than inferred |
| 5 | PV-13c · the chip family's ONE definition | 13 | Code | ☑ | `chip.tsx` (the component) — the `.chip`/`.chip-*` CSS family is DELETED | `test:chip-contract` · `test:design-frozen` (−3) · `test:type-scale` (−2) | `4732e602` → `05fd993f` | `.qa-shots/pv13c-local/` | ✅ **LIVE on `05fd993f`, verified on production 2026-09-04** (uptime reset 62,466s → 5s, never a 200).  **Ali's ruling, 2026-09-03: finish the migration, no visual change.** ⭐ Census CORRECTED first: not "7 chaotic heights" but **3 tiers** (sm 18/20 · xs+md 21/23 · lg 25/27), and the record's 7th value was `.tier-badge` — a **circular 22px rank medallion**, not a pill. 🔴 **The real defect was a DRIFT the audit only found because the migration forced it:** `chip.tsx`'s `signal` variant still held the pre-D6 **AQUA** (§B4 bans aqua on anything semantic) while the CSS class had been corrected to ROYAL on 2026-08-21 — zero call sites, so nothing rendered it, so nothing caught it. Migrating the market card's tipping flag onto it would have silently put banned aqua back on the player board. ⛔ **NOT proven on production** |
| 6 | PV-10 · `@pct%`/`×N` suffix contrast | 6 | Code | ☑ | `market-card.tsx` · `side-picker.tsx` · `updown-card.tsx` · `updown-stake-controls.tsx` · `conviction-dial.tsx` (filed as 4 sites, was **9**) | `test:contrast` **§P-u2** (new) · `red:contrast-callsite` | `e14394d7` | `.qa-shots/pv10-local/` | ✅ **LIVE since `e14394d7`** — same correction as rows 3/4: that branch was already in `main` by content.  guard population gap WAS the finding, confirmed; guard's own sweep found a 9th site (the bet-panel commit button). ⛔ **NOT proven on production** |
| 7 | PV-14 · timing correctness — **the ladder's ratchet had never read a stylesheet** | 14 | Code | ☑ | `chat-tokens.css:82-85` (the `--cm-*` aliases) + 5 call sites in `chat-styles.css` — ⛔ **NOT** the Input atom, and **no** `test:motion-timing`: see the note | `test:motion-ladder` **§2+§3** (extended, not a new file) · `red:motion-ladder` (new — the guard had **no** control) | `05fd993f` | `.qa-shots/pv14-local/` (gitignored, local-only) | ✅ **LIVE, re-verified on production 2026-09-04: every `--cm-*` computes to the literal read out of `motion.css` in the same run, and `--cm-ease-sink` is gone from the cascade** (prod drive **72/0**, EN/SW/ZH × 390/1280, locale proven by discrimination). Guarded, **RED-proven 6/6**, browser-verified at 390+1280 × EN/SW/ZH. ⭐ The filed census was stale ×124 (`duration-150` was **3**, not 373; bare Tailwind curves **0**, not 391) and the real defect was a **corpus hole** — `walk()` took only `.tsx`/`.ts`, so a **fourth motion vocabulary** (8 hand-typed values) lived in `src/styles/chat/`. New law: Authority **§E9/§E10**. Ratchet moved **4 → 5 → 4** (`needle.css` in, scheduling; `spinner.tsx` out, §E10 covered it) |
| 8 | PV-03 · narrow detail pages — **re-derived, fixed, and the class is 4× the filed size** | 3,5,10 | Code | ☑ | `empty-state.tsx` — new opt-in **`fill`** prop + a `data-empty-state` contract. ⛔ **NOT** `detail-layout.tsx` — never build it (§b2, struck) | `responsive-audit.mjs` **B7's LOWER bound** (new) — the filed ~65%-of-tier rule is **REFUSED with arithmetic** | `05fd993f` | `.qa-shots/prod-2026-09-04/` | ✅ **LIVE — production reads `variant="fill"` with delta 0 at both widths in all three locales.**  ✅ **`/wallet/deposit` OVERTURNED** — `tier="form"` (640px) is CORRECT; its "wasted 53%" is what §B7 exists to produce. 🔴 **`/positions` was ALIGNMENT, not measure**: headings left at x=132, card centred at 460 — **delta 328px → 0**, verified at 1280 and 390. ⭐ **The guard then found the class was 9 routes, not 2** — `/positions/performance` fixed too; the other 7 are **page-level** empty states (the card IS the page) and are held in a named `EMPTY_ALIGN_PENDING` ratchet with a stale-check, pending a design ruling, rather than 7 player routes being restyled to satisfy a guard |
...
| 10 | PV-05 · dial weight + term unification | 5,9,13 | Code | ☑ | `docs/design-brief/player-visual-2026-09/handover/` | `test:motion-ladder` §4 · `test:docs` | `05fd993f` | the handover's three docs (LIVE) | ✅ **Part 2 DELIVERED — the commit-sequence motion spec** (`MOTION.md`), five beats each naming its real travel distance and its rung, with §d's **mechanical gate RUN FIRST and passing 14/14** (`TOKENS-USED.md`). ⭐ Four of five beats were **already correct** and are documented, not changed. 🔴 The fifth was a real §M8 breach — `orm-pop` on `--m-pivot` — **fixed to `--m-settle` and now guarded by `test:motion-ladder` §4** (the reservation had **no guard at all**; the handoff's belief that `test:needle` owned it was wrong). ✅ **Part 1 DECIDED 2026-09-04 — and BOTH halves are OVERTURNED by measurement (`DECISIONS.md` D6/D7).** *"Three registers for one concept"* names **three different things**: `yourPick` labels the SIDE (it sits beside a `sideWord` chip in both call sites), `conviction` labels the DIAL, `21.08×` is the multiplier — and the panel already reads *"Choose a side to set your conviction and place your bet"*. *"A thin 5px-ish slider with a small knob"* measures **`trackH = 12` and `knobR = 28`** — a 12px track and a **56px** knob, the largest control on the panel and well above §A2's 44px floor. ⛔ **No repaint, no artboards commissioned**: the adjective is off by 2.4× on a control PV-12 already proved physically sound |
| 11 | Lens 12 · `/markets` filter correctness — **3 of 4 were already true; the 4th is now guarded** | 12 | Code | ☑ | ⛔ **NOT** a new drive — the home is **`qa:filter-stress`**, which §5 never names | `qa:filter-stress` **+ sort monotonicity** (new section, with its own control) | `05fd993f` | `qa:filter-stress` GREEN **on production** | ✅ **Run against `50pick.tz` 2026-09-04: 288 combinations, hostile params, idempotence and the new monotonicity check all pass.** ⭐ **And the new check earned its keep on its FIRST production run — by catching MY reader.** It reported `?sort=pool` non-monotonic; three of twelve live cards carry TWO `TZS` figures (a market's own question can contain one), and the regex took the FIRST. Real volumes are perfectly descending. ⛔ Fixed to take the LAST match — the volume is the card's final money figure, in the meta row before the countdown. **A false sort-defect report against a live money product, avoided by re-deriving instead of believing.**  ⛔ Both instruments §5 names are wrong (`qa:filter-scan` is a filter-*language* check). ✅ **combined-filter intersection** was already covered — `qa:filter-stress` drives all **288** status×sort×odds×pool boards. ✅ **count==rendered** was already asserted there (`resultCount` vs `cards`) and holds **5/5 exact** incl. the zero case. ✅ **URL-backed OVERTURNED — it IS**; §5's doubt came from clicking **"Open"**, the *default*, which correctly writes nothing. 🔴 **The one real gap: all six sorts were driven and NOTHING checked anything was sorted** — a sort control that silently did nothing passed every check. Now asserted on `pool`, the only key a card prints, **with a control that the volumes are not all identical**, and proven to DISCRIMINATE (`?sort=new` reads non-monotonic, `?sort=pool` monotonic) |
...
## 🏁 **PLAYER-VISUAL-10 IS CLOSED — 2026-09-04. ALL 13 ROWS ☑, LIVE ON PRODUCTION.**

**Nothing is pending.** Ali took the last three open questions off the board on 2026-09-04
(*"take the decision based on what seems more reasonable… what matters is perfection"*), and all
three were decided **by measurement**, not by taste — two of them **overturning the record**:

| open question | ruling | on what evidence |
|---|---|---|
| The 7 page/tab-level empty states | **stay centred — RULED CORRECT** | Looked at, at 1280: on `/wallet` the card belongs to the **Activity tab**, not the `h1` the probe paired it with; a 1016px dashed box around one sentence is worse, not better |
| The dial's term unification | **OVERTURNED — no change** | `yourPick` labels the **SIDE** (beside a `sideWord` chip at both call sites), `conviction` the **DIAL**, `21.08×` the **multiplier** — three things. The panel already says *"Choose a side to set your conviction"* |
| The dial's visual weight | **OVERTURNED — no repaint, no artboards** | `trackH = 12`, `knobR = 28` — a **12px track and a 56px knob**, not "5px-ish and small". The adjective is off by **2.4×** on a control PV-12 already measured sound |

⭐ **AND THAT IS THE PROGRAMME'S FINAL PATTERN, STATED ONCE: the record's unmeasured adjectives
were wrong every single time they were checked.** `duration-150` ×373 → **3**; bare curves ×391 →
**0**; "seven chip heights" → **three tiers**; "eight narrow routes" → **one** real defect; "three
words for one idea" → **three correct words**; "a 5px slider" → **12px**. ⛔ The lesson is not that
the record was careless — it is that **a number read off a browser and an adjective read off a
screenshot are both hypotheses**, and this programme's whole value was converting them into
measurements. Six of the twelve findings shrank or vanished under that; the ones that survived
were real, and two of them (the ladder's blind corpus, the `--m-pivot` breach) were **never in the
record at all** — they were found by building the guards the record asked for.

**The deploy was verified the way this repo requires — an UPTIME RESET, never a 200.**
`62,466s → 5s`, clean boot, 93 live markets, public routes 200 and gated routes 307.

**The production drives, all re-run against `50pick.tz` after the deploy:**

| drive | result |
|---|---|
| the row 7/8/13 verification drive | **72 / 0** — 390 + 1280 × EN/SW/ZH, locale proven by **discrimination** |
| `qa:side-words` (row 1) | **28 / 0** |
| `qa:cold-start` (row 2) | **36 / 0** |
| `qa:card-geometry` (row 4's owed measurement) | `heights=[278,320,**347**,354]` — the `349 → 347` this row predicted, **confirmed live** |
| `qa:filter-stress` (row 11) | **GREEN** — 288 combinations, hostile params, idempotence, and the new monotonicity check |

⭐ **THE LAST FINDING OF THE PROGRAMME WAS MY OWN INSTRUMENT, ON PRODUCTION, AND IT MATTERS MOST.**
The new monotonicity check reported `?sort=pool` **non-monotonic** on its first production run —
a sort defect on a live money product. It was not. Three of twelve live cards carry **two** `TZS`
figures (a market's own question can contain one: *"…closes above TZS 2,850…"*), and the regex took
the **first**. The real volumes were perfectly descending. ⛔ Fixed to take the **last** match — the
volume is the card's final money figure, in the meta row before the countdown. **A false defect
report against production, avoided by re-deriving instead of believing a green-turned-red guard.**

⚠️ **That was the EIGHTH instrument bug of this programme's final two sessions, and not one was
found by reading code.** Every single one was caught by a control, a screenshot, or a number that
did not survive being printed. The complete list is in the session-8 block below; if you read only
one thing from this programme, read that list. **The instruments were wrong more often than the
product was**, and the product was only ever proven by the runs that could tell the difference.

**⛔ WHAT IS NOT DONE, stated plainly rather than counted clean:**
- **Row 10's artboards** — the dial's visual *weight*. `DECISIONS.md` §3 bounds the commission.
- **Three one-sentence rulings for Ali** — the dial's single word; whether the 7 routes in
  `EMPTY_ALIGN_PENDING` fill their measure or stay centred; and commissioning the above.
- **`chat-styles.css`'s stagger delays** (`animation-delay: 100ms/180ms`) are off `--m-stagger` —
  filed in `MOTION.md` §3, invisible to `test:motion-ladder` by its written scope.

**⛔ AND THE CORRECTION THAT THREE HANDOFFS GOT WRONG, so it is not inherited a fourth time:**
`pv10/rows-3-4-6` was **never merged and never needed to be** — its code was already in `main` as
`e14394d7`, and its only remaining `src/` delta was the **needle files, where `main` was NEWER**.
Merging it would have reverted 127 lines of the parallel session's work. **Rows 3, 4 and 6 had
been live all along** while three consecutive handoffs said *"NOTHING on production"*. ⭐ Check a
branch by its **content** (`git diff origin/main <branch> -- src/`), never by its name or its
ahead/behind count.
