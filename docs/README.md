...
| [`SESSION-PROMPT-AI-CYCLES.md`](SESSION-PROMPT-AI-CYCLES.md) | 🔵 **LIVE — NOT STARTED** | ⭐ **AI SPEND MEASURED IN CYCLES, SO THE PLATFORM CAN SAY WHAT A POLL RESOLUTION COSTS AND WHAT TO CHARGE FOR IT.** Ali, 2026-08-23: *"our current approach is perfect but we have to plan another approach based on cycles… every cycle we monitor — that way we know how many cycles spent per year, to know what to charge"*, with a **custom cycle size**. ⛔ **Additive: the USD meter stays.** `assertAiBudget()` is the live hard cap on real money and Ali called the existing approach perfect. ⛔ **The word `cycle` ALREADY means something else in `ai-usage.ts`** — `CreditConfig.cycleStartIso` is the *top-up window*, not a spend quantum. Two meanings of one word in one file is how `E-179` happened; the prompt requires renaming one of them in the same commit. 🔴 **The blocking prerequisite is ATTRIBUTION:** `AiUsageEvent.detail` is free text, so nothing can currently divide spend by "resolutions" — a structured `subjectType`/`subjectId` has to be threaded through every `recordAiUsage()` call site first, in one commit, because a half-threaded attribution under-counts and an under-count here reads as *cheaper than it is*. ⭐ Its §10 is a **20-row table of the ways this number can be confidently wrong** — retroactive size changes making "cycles per year" meaningless, a single call spanning several cycles, the 180-day retention pruning the evidence the ledger is derived from, projecting a year from three hours, an invisible Haiku→Opus switch moving cost-per-resolution ~5×, and two caps that can disagree. **Work that table before writing code; each row is a test case.** ⚠️ **Five questions are open for Ali (§14) and §3.4 cannot be built until #1 is answered:** what the billable event actually is. |
