...
| [`SESSION-PROMPT-LANDMARK-AND-MEASURE.md`](SESSION-PROMPT-LANDMARK-AND-MEASURE.md) | 🔵 LIVE | ⭐ **ONE `<main>` PER PAGE — and the B7 migration that finishes with it.** Written 2026-08-22 (session 56). **Measured, not inferred: 6 of 8 sampled production routes render TWO `<main>` elements, one nested inside the other**, because `AppShell` renders `<main id="main-content">` in the ROOT layout and 44 files under `src/app` render their own. Nested `main` is invalid HTML, gives a screen reader two "main content" landmarks, and the skip-link resolves to the outer one. ⭐ **The insight that makes it one job instead of two:** those same 44 files are the population **B7** is still migrating off hand-typed widths — so `<PageContainer tier>` fixes the landmark, the width and the `loading.tsx` tier parity in one edit, and `test:measure`’s ratchet falling is the proof. ⛔ It names the mechanical `<main>`→`<div>` sweep as the WRONG fix (cheapest, and it burns the one pass anyone will make over these files), and requires a **behavioural** guard in `responsive-audit.mjs` — exactly one `<main>` per rendered route — red-proven before the sweep so it doubles as the progress bar. |
| [`SESSION-PROMPT-VISUAL-SWEEP.md`](SESSION-PROMPT-VISUAL-SWEEP.md) | 🔵 LIVE | ⭐ **A LIVE COMMISSION** (Ali, 2026-08-15): every pixel, control, width and language against the design kit; nothing technical. ⛔ **Seven spent prompts were DELETED on 2026-08-16** at Ali’s instruction, so the tree keeps only prompts that still do work. ⚠️ **This row went on to say “this one and `SESSION-PROMPT-JAY-COMMENTS.md`, and no more” — corrected 2026-08-21 by LISTING rather than remembering:** `docs/` holds **six** `SESSION-PROMPT-*.md` (re-counted 2026-08-22 by running the `ls`, as this row instructs). 🔵 LIVE: `DESIGN-PERFECTION`, `JAY-COMMENTS`, `VISUAL-SWEEP` (this one), `DATA-FINALISE` and `LANDMARK-AND-MEASURE`. ⚪ SPENT but deliberately kept: `KYC-ID-OPTIONS`. Every one of them is indexed in this section. Run `ls docs/SESSION-PROMPT-*.md` before quoting a number — “and no more” was written on the day it was true and was false within five days. Their substance was already in the files that own each subject — `DESIGN_AUTHORITY` §F/§L, `FAILURE-INVENTORY` §6/§7, `LIVE-QA-CAMPAIGN` §6b — and git history holds the originals. ⚠️ One item outlived its prompt: **unit H** (`qa:refusal-frames`’ drive half), recorded in `LIVE-QA-CAMPAIGN` §6b, session-48 block. |
