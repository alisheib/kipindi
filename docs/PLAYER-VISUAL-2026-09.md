...
4. ✅ **RULED 2026-09-03 — the "Invite & Earn" gold button on the zero-bonus card** ([wallet-client.tsx:314](../src/app/wallet/wallet-client.tsx#L314) `btn-gold`). Ali: **confirmed correct** — re-verified `test:gold-is-money` **15/0** the same day. It leads to real referral money; gold on a genuine money surface, not an inducement. Ruling recorded, no code change. ⛔ **SUPERSEDED — do not cite this ruling for any invite surface.** The button is gone: removed 2026-09-06 with the invite + bonus withdrawal (`62ba00a7`; `wallet-client.tsx` now reads "THE INVITE CTA IS GONE FROM HERE"). And since 2026-09-25 the ruling's premise — "it leads to real referral money" — is false: the player invite pays nothing (`inviteRewards` WITHDRAWN, `docs/PLAYER-INVITE-UNPAID.md` §2), so no invite entry point may wear gold (`RULES.md` §2.10a, `DESIGN_AUTHORITY` §M3). This ruling does not license gold on the unpaid invite.
