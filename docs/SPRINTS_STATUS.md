...
| Suite | Pass | Notes |
|---|---|---|
| Sprint 18 regulator-readiness (NEW) | **33 / 33** | /api/health JSON · /fairness page + commit-reveal · cryptographic verifier (synthetic + live) · public footer 18+/helpline/license on 5 routes · /games RTP disclosure · /admin/privacy DSAR queue · /admin/retention schedule · /admin/aml suspicious-bet detector |
...
| **Combined** | **290 / 291** | Sprint 18 — provably-fair + DSAR + retention + suspicious-bet added. One known Mapigo-parallel-race timing flake remains; back-to-back full-suite runs trigger ~10 wallet-state assertions to fail (each suite passes in isolation). |
