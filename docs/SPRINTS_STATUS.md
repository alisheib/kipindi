...
| Suite | Pass | Notes |
|---|---|---|
| Sprint 19 markers + 2-person AML + webhooks + responsiveness (NEW) | **11 / 11** | Player-safety markers panel on /admin/compliance · two-person AML threshold visible on /admin/aml · webhook signing (unsigned/wrong/valid/unknown-provider/stale-timestamp) |
...
| **Combined** | **331 / 332** | Sprint 19 — markers + 2-person AML + webhooks + responsiveness fix + i18n audit added. One known Mapigo-parallel-race timing flake remains; back-to-back full-suite runs trigger wallet-state assertions to fail (each suite passes in isolation). |
