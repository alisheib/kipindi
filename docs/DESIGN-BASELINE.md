...
| `test:withdrawn-features` | a feature state reaches **every** entry point. §7 judges a **POSITION** — each `/profile/invite` link must sit beside the gate being consulted, because the first version of this rule asked only *"does this file mention `inviteIsLive`?"* and passed over two surfaces gone silently live with the import still present · §8 the page consults the gate **BEFORE** minting a referral code · §5h the page may not PROMISE a wallet the payer does not use | `red:withdrawn-features` (3/3) |
| | ⚠️ **CORRECTED 2026-09-06.** This row read `test:invite-coming-soon` · `red:invite-coming-soon` (4/4) until a cleanup audit checked it. **Both were deleted on 2026-09-06** — Invite is WITHDRAWN, not coming soon, so that suite failed on its own premise and was retired, its §1–§3 intent PORTED into `withdrawn-features` §6–§8. A register that names a gate which no longer exists is the same defect as a gate that is not in the pipeline, read from the other end: **this table is the one place that answers "what holds this rule?", so a stale row here is a rule nobody is holding.** ⚠️ The lesson in §6 below still names the old suite ON PURPOSE — it is history, and the defect it records really did happen under that name. | |
