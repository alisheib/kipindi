...
const G="#d9b23f",GD="#b8902a",N="#232a6b",BG="#0d0f2e",C="#141640",B="#2a2d5e",L="#3346c8",T="#f0eff4",TM="#8a90ad",TS="#5a607e",Y="#199a5b",R="#d9404a";
...
const wrap=(body)=>`<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="margin:0;padding:0;background:${BG};font-family:Segoe UI,Helvetica,Arial,sans-serif"><table cellpadding="0" cellspacing="0" width="100%" style="background:${BG}"><tr><td align="center" style="padding:32px 16px"><table cellpadding="0" cellspacing="0" width="100%" style="max-width:560px"><tr><td align="center" style="padding:0 0 24px">${mk}<div style="margin-top:10px;font-size:20px;font-weight:800"><span style="color:${T}">50pick</span><span style="color:${TM};font-weight:500;font-size:14px">.tz</span></div></td></tr><tr><td><div style="height:3px;background:linear-gradient(90deg,${G},${GD},${G});border-radius:3px 3px 0 0"></div></td></tr><tr><td style="background:${C};border:1px solid ${B};border-top:none;border-radius:0 0 12px 12px;padding:28px">${body}</td></tr><tr><td style="padding:28px 0 0;text-align:center"><div style="width:42px;height:2px;background:${G};border-radius:2px;margin:0 auto 16px"></div><p style="margin:0;font-family:monospace;font-size:10px;letter-spacing:0.16em;text-transform:uppercase;color:${GD}">50pick.tz <span style="color:${R}">&middot;</span> <span style="color:${TM}">Soko la Utabiri</span></p><p style="margin:10px 0 0;font-size:11px;color:${TS};line-height:1.7">18+ &middot; Licensed by Gaming Board of Tanzania<br>Helpline +255 22 211 5811 &middot; <a href="mailto:support@50pick.tz" style="color:${L};text-decoration:none">support@50pick.tz</a></p></td></tr></table></td></tr></table></body></html>`;

const ey=(en,sw)=>`<p style="margin:0 0 6px;font-family:monospace;font-size:10px;text-transform:uppercase;letter-spacing:0.16em;font-weight:700;color:${GD}">${en}</p>`+(sw?`<p style="margin:0 0 2px;font-size:11px;font-style:italic;color:${TS}">${sw}</p>`:"");
const h1=(t,c)=>`<h1 style="margin:0 0 10px;font-size:22px;font-weight:700;color:${c||T};line-height:1.2">${t}</h1>`;
...
const rows=(r)=>`<table cellpadding="0" cellspacing="0" width="100%" style="border-top:1px solid ${B};margin-top:16px">${r.map(d=>{const vc=d.t==="good"?Y:d.t==="bad"?R:T;return`<tr><td style="padding:10px 0;border-bottom:1px solid ${B};font-family:monospace;font-size:10px;text-transform:uppercase;letter-spacing:0.14em;color:${TS}">${d.l}</td><td style="padding:10px 0;border-bottom:1px solid ${B};text-align:right;font-family:monospace;font-size:14px;font-weight:700;color:${vc}">${d.v}</td></tr>`;}).join("")}</table>`;
const cta=(href,label)=>`<table cellpadding="0" cellspacing="0" width="100%" style="margin-top:24px"><tr><td align="center"><a href="${href}" style="display:inline-block;padding:14px 36px;background:linear-gradient(135deg,#c49a2e,${G});color:${N};font-size:14px;font-weight:700;border-radius:999px;text-decoration:none">${label}</a></td></tr></table>`;
