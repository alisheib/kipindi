...
| `leaderboard-product-mix.cjs` | `/leaderboard` ranks on ONE ROI with no product filter — this measures how much of that population is Up & Down, and how far mixing moves a player's own ROI | ⚠️ counts **POSITIONS**, not market rows. The "~12:1" above and the "~20×" in `market-dal.ts` are about the MARKET table, where a chain emits a row every few minutes; they say nothing about how players actually bet. ⭐ Measured 2026-09-09: **72% of ranked positions are Up & Down**, 17 of 41 board rows mix both, worst per-player gap **145.91pp** |
...
| `levy-divergence.cjs` | does the ledger book the TRA/GBT that `levySplit` says the market owes | ⭐ **the 2026-09-09 money-gate finding.** The ledger derived each winner levy with its own `Math.round` against ONE `levySplit`; every group still summed to zero, so postLedgerEntries, the trial balance and `ledger.test.mts` were all green over it. ⛔ **Read the BY-WINNER-COUNT table, not the headline** — 0% of one-winner markets can diverge by construction, so a run dominated by them looks clean. **Baseline 2026-09-09, post-fix: 43 of 203 historical markets diverge (+20 TRA, −7 GBT), NOT backfilled. That number must never GROW.** |
