/**
 * red:confirm-gate — proves `test:confirm-gate` catches the defects it names, each on its OWN
 * assertion. Same discipline as `red:backup-order` / `red:timer-date`: reintroduce the REAL
 ...
 *
 * ⭐ THREE OF THE FIVE NEVER TOUCH A CALL SITE, and they are the ones worth reading.
 * `the-union-reverts-to-two-optionals` and `the-gate-fails-open-again` undo the PRIMITIVE
 * while every caller still passes its word — so the call-site rule stays green, the tree
 * still builds, and only a suite that reads modal.tsx itself notices. That is how S-17
 * arrived in the first place. `the-element-reader-goes-blind` empties the population §1
 * iterates, because a rule that parses nothing reports "every gate is armed" in exactly the
 * same words as "I could not read a single element".
 *
 * ⭐ THE MATCH IS ON `FAIL <label>`, NOT ON A SECTION NUMBER, so a defect caught for the
 * wrong reason cannot print PASS.
 *
 * ⛔ The anchors are DATA, in `scripts/anchors/confirm-gate.anchors.mjs`, so `test:red-anchors`
 * can audit that every one still resolves exactly once WITHOUT running this file.
 *
 * Run: npm run red:confirm-gate
 */
import { readFileSync, writeFileSync } from "node:fs";
import { execFileSync } from "node:child_process";
import { injectDefect } from "./red-anchor.mjs";
import { MUTATIONS } from "./anchors/confirm-gate.anchors.mjs";
...
const GATE = "scripts/confirm-gate.test.mts";
...
const base = runGate();
if (base.code !== 0) {
  console.error("REFUSING: test:confirm-gate is already RED on the untouched tree.");
