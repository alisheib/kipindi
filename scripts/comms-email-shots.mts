...
const PAGES: { name: string; html: string }[] = [
  ...
  // House bots (build commit 3) — the officer alert with a real-length bot id (no holder letter exists: D19c, C4 ruling 149).
  { name: "houseBotErasureBlockedAdminHtml", html: E.houseBotErasureBlockedAdminHtml({ botId: "hb_a1b2c3d4e5f6a7b8c9d0e1f2", holder: "Player #A3F2K8", botUrl: "https://www.50pick.tz/admin/desk/hb_a1b2c3d4e5f6a7b8c9d0e1f2" }) },
  // Build commit 4, step 9: the one parametrised admin letter, in each of the four shapes an admin actually gets.
  { name: "houseBotAdminHtml.paused", html: E.houseBotAdminHtml({
    eyebrow: "House bots · paused",
    heading: "A running house bot stopped: the password changed",
    subtitle: "Player #A3F2K8 changed their 50pick password in their account settings at 14:02 EAT. The bot stopped and cancelled 2 queued stakes. No bet will be placed until you enter their new password.",
    rows: [{ label: "Bot", value: "Dar liquidity A" }, { label: "Holder", value: "Player #A3F2K8" }, { label: "Stakes cancelled", value: "2" }],
    cta: { href: "https://www.50pick.tz/admin/desk/hb_a1b2c3d4e5f6a7b8c9d0e1f2?reverify=1", label: "Enter new password" },
  }) },
  { name: "houseBotAdminHtml.switch", html: E.houseBotAdminHtml({
    eyebrow: "House bots · master switch",
    heading: "House bots are off",
    subtitle: "Switched off. A bet already in its final step may still complete. Cause: MANUAL. 3 queued stakes were cancelled.",
    rows: [{ label: "Cause", value: "MANUAL" }, { label: "By", value: "Juma M." }, { label: "Stakes cancelled", value: "3" }],
    cta: { href: "https://www.50pick.tz/admin/desk", label: "Open the desk" },
  }) },
  { name: "houseBotAdminHtml.money", html: E.houseBotAdminHtml({
    eyebrow: "House bots · holder money",
    heading: "Money moved on a live house-bot account",
    subtitle: "Player #A3F2K8 withdrew TZS 50,000 at 14:06 EAT on the account behind house bot \"Dar liquidity A\".",
    rows: [{ label: "Bot", value: "Dar liquidity A" }, { label: "Transaction", value: "txn_9f3a2b7c1d8e" }, { label: "Balance now", value: "TZS 120,000" }],
    cta: { href: "https://www.50pick.tz/admin/transactions?q=txn_9f3a2b7c1d8e&range=all", label: "Open the transaction" },
  }) },
  { name: "houseBotAdminHtml.alert", html: E.houseBotAdminHtml({
    eyebrow: "House bots · urgent",
    heading: "Settlement is blocked: house bot \"Dar liquidity A\" has no holder wallet",
    subtitle: "TZS 240,000 of open house stakes cannot settle, which holds up every player on those markets. Restore the wallet, or remove the bot and settle by hand.",
