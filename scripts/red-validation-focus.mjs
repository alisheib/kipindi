/**
 * red:validation-focus — THE CONTROL FOR `test:validation-focus` (DG-S-05+DG-S-06, §K rule 7d).
 *
...
import { MUTATIONS } from "./anchors/validation-focus.anchors.mjs";

const here = dirname(fileURLToPath(import.meta.url));
const REPO = join(here, "..");
const GATE = join(here, "validation-focus.test.mts");
...
console.log("──────────────────────────────────────────────────────────────────────");
console.log("red:validation-focus — the control for DG-S-05+DG-S-06");
console.log("──────────────────────────────────────────────────────────────────────");
...
for (const mut of MUTATIONS) {
  ...
  if (res.missing || res.rot) {
    console.log(`  🔴 ${mut.name}\n       ${res.missing ? "FILE NOT FOUND" : "ANCHOR ROT — re-anchor in scripts/anchors/validation-focus.anchors.mjs"}: ${mut.file}`);
    bad = 1;
    continue;
  }
