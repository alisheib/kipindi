/**
 * red:recategorise — proves `test:recategorise` catches the defects it names, each on its OWN
 ...
 *
 * ⛔ The anchors are DATA, in `scripts/anchors/recategorise.anchors.mjs`, so `test:red-anchors`
 * can audit that every one still resolves exactly once WITHOUT running this file.
 *
 * Run: npm run red:recategorise
 */
import { readFileSync, writeFileSync } from "node:fs";
import { execFileSync } from "node:child_process";
import { injectDefect } from "./red-anchor.mjs";
import { MUTATIONS } from "./anchors/recategorise.anchors.mjs";
...
const GATE = "scripts/recategorise.test.mts";
...
const base = runGate();
if (base.code !== 0) {
  console.error("REFUSING: test:recategorise is already RED on the untouched tree.");
