/**
 * `npm run red:rg-doors` — DOES `test:rg-doors` ACTUALLY CATCH ANYTHING?
 *
...
import { MUTATIONS } from "./anchors/rg-doors.anchors.mjs";

const ROOT = new URL("..", import.meta.url).pathname.replace(/^\/([A-Za-z]:)/, "$1");
const SUITE = "scripts/rg-doors.test.mts";
...
function runSuite() {
  ...
  const tally = /rg-doors: (\d+) passed, (\d+) failed/.exec(out);
...
console.log("\nred:rg-doors — E-240, E-238 and E-235. The mutations remove the FIX, not the defect.\n");
...
console.log(`\nred:rg-doors: ${caught}/${primaries.length} defects caught` +
