...
const RATCHET_ARBITRARY_SIZE = 934;   // −1, 2026-08-30, DG-A-06, same chip. ⚠️ A ratchet left ABOVE the number the tree already reaches is SLACK, not safety — it silently licenses one regression, and this one was caught by a reviewer re-deriving all three rather than by the suite, which passes either way.
...
const RATCHET_ARBITRARY_TRACKING = 271;   // −3, 2026-08-30, DG-A-06: three hand-typed `tracking-[0.08em]` chips went with the rails that adopted `FilterPill`.   // 2026-08-30, −39 more: DG-A-14's 39 prose fixes each dropped the eyebrow dressing, and the tracking token went with it — a row about the READING FLOOR paid a tracking dividend, the same way DG-P-03's skeleton adoption did. // was 313: ⭐ 2026-08-30, −289 IN ONE COMMIT and the largest fall this ratchet has ever taken: DG-A-11's tracking sweep put §T3's section eyebrow on `.eyebrow` (globals.css, beside `.amount`) instead of a hand-typed value at 289 call sites. ⛔ THAT IS THE ONLY WAY THIS RATCHET CAN REACH ZERO — CONVERGING nine tracking values onto one would have left the count untouched at 602 forever, because the defect it counts is a value written at a CALL SITE, not a value that disagrees with its neighbours. // 2026-08-29: -11 then -5, FieldLegend adopted where its recipe was hand-retyped BYTE FOR BYTE (DG-A-11). The last 5 were `<label>`s in the auth forms — `block … mb-1.5` around the component's exact class string — which the first pass missed because it looked for `<span>`s. Then -13 on 2026-08-30 (DG-P-03): the six loading skeletons that adopted `<PageHeader>` were each hand-typing its EYEBROW as well as its h1, so a row about headings paid a tracking dividend nobody predicted. ⚠️ It was found only because the handover's numbers were re-derived before being written down, which is the rule that keeps earning its keep.
