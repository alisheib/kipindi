...
const RATCHET_SUBFLOOR = 744; // -3, 2026-09-19 (C7 step 3's visual pass, the first renders ever taken of `report-pack-card.tsx`): C5-6 shipped a 10px centred SENTENCE under the pack's one control, and the two signature-slot lines — the moment an officer signed a Gaming Board filing, and the awaiting-signature state — were 10px and 11px under a 13px name. All three took `text-body-sm`, the platform's own shape for a second line under a name (`/admin/agents`, and the desk's roster, both of which chose it in writing BECAUSE it clears this floor). Lowered in the same commit as the run that measured it: 747 above a tree at 744 is slack, not safety. // -2, 2026-09-13 (KYC at withdrawal + the visual pass): the new admin reading sites (three justification counters, the refused report payout cell) were lifted to text-body-sm rather than written sub-floor, and the profile stat strip dropped two. // -N, 2026-09-08 (PLAYER QUERY stage 4): re-derived from the run and locked in rather than left as slack — this file's own rule is that a ratchet above the number the tree reaches SILENTLY LICENSES ONE REGRESSION. §3 fell because two cap notices written this stage were caught as NEW offenders while being written (a sentence is reading copy, so text-caption/label are below the floor and do not count as a fix) and moved to text-body-sm; the other three were already below their ratchet before this stage touched them.   // -1, 2026-08-31, DG-A-08: the proposals row cell stopped hand-rolling its three controls — `Reject` and `Delete` became the kit `<Button size="sm">` and `View chain` took `.row-link`, and the sub-floor `text-micro` went with them.   // -2, 2026-08-31, DG-S-02: `ui/tabs.tsx` hand-wrote its count pip THREE times, two of them at `text-[11px]` — under this floor and a fifth implementation of the pip the stage-9 consolidation existed to remove. All three are the kit `<CountBadge>` now, which caps at 99 and renders nothing at zero. ⚠️ AND THE FIRST ATTEMPT MOVED THIS NUMBER THE WRONG WAY, which is the finding worth keeping: putting the `segmented` variant's off-ladder `text-[12.5px]` onto `text-label` (12) added a NEW sub-floor site here (753 → 752, the two pips masking it), because 12 is UNDER §T4's 12.5px floor and a tab label is a §T3 CONTROL LABEL with no claim on the blessed sub-micro tier. The rung is `text-body-sm` (13) — which also puts all three variants on ONE size. Caught by re-deriving the ratchet rather than by reading the diff.   // -2, 2026-08-30, session 82: `install-invite`'s body copy (12px prose under this very floor, in the one dismissible dialog with no Escape path) and `report-pack-controls`' CopyHash, which took `.btn-sm`'s 13px when it adopted the kit. # −1 more, 2026-08-30, DG-A-06: `/admin/proposals`'s queue rail carried a `text-[11px]` chip — 22.5px, the smallest filter control in the console — and adopting the primitive's geometry retired it.   // ✅ 2026-08-30, −48 and DG-A-14 IS CLOSED: 39 of the 48 were FIXED (27 sentences lifted onto `text-body-sm`, 9 labels split from their hint, 3 more after a third reader overturned the fixer's refusal) and ⚠️ **9 came off by RECLASSIFICATION, not by moving a glyph** — adjudicated as genuine labels, and that half of the number is stated rather than folded in. // was 804: ⛔ 2026-08-30, +48: THE POPULATION GREW, NOT THE DEFECT — the third time this ratchet has had to be re-based upward for that reason, and the third time it was the BLESSING that was wrong rather than the product. §3 exempted "an UPPERCASE tracked microlabel", keyed on the DRESSING, so 48 sentences and labels-with-their-hint-welded-on were counted as labels while sitting 3-4px under §T4's reading floor. They are DG-A-14's population and they now count. // 2026-08-30: -5, the five section eyebrows that carried NO tracking at all and gained §T3's 0.14em with the rest (DG-A-11's tracking sweep). ⚠️ They fell out because a GLYPH moved — from `text-micro`'s own 0.4px to 1.4px — not because the blessing widened: `.eyebrow` was added to `isBlessedMicrolabel` in the same commit, and without it the sweep would have read as +291 on a ratchet that may only shrink. // 2026-08-29: -268 total, the ADMIN then PLAYER prose sweeps (DG-A-12); then -2 for `fee-simulator.tsx`'s two licence paragraphs, 10.5px prose lifted to `text-body-sm`. ⭐ THE FIRST TIME §3 MOVED IN THREE SWEEPS, and that is the point: §3 falls only when a GLYPH gets more legible, which is why it held flat at 763 through 376 renames that bought §4 alone.
...
const RATCHET_ARBITRARY_SIZE = 906; // -3, 2026-09-19 (C7 step 3's visual pass): the same three sites in `report-pack-card.tsx` — a sub-floor SENTENCE and a hand-typed SIZE are one call site counted by two rules, so fixing one lowers both. ⭐ This is what took §4 from 909-against-908 (a red inherited from `origin/main`, +1 NEW with no owner) to GREEN — the +1 was never found because nobody had rendered this card. // -6, 2026-09-13 (the visual pass: the new admin controls moved onto text-body-sm / text-micro / text-title-sm and the fixers converted hand-typed sizes they touched). // -1, 2026-09-09 (PLAYER QUERY stage 6 close): the /proposals summary sentence moved off text-[12px] onto text-body-sm, which is what took §3 back to its ratchet — a sub-floor SENTENCE and a hand-typed SIZE are the same call site counted by two rules, so fixing one lowers both. Lowered in the SAME commit, per this file own rule that a ratchet above the number the tree reaches is slack, not safety.   // -N, 2026-09-08 (PLAYER QUERY stage 4): re-derived from the run and locked in rather than left as slack — this file's own rule is that a ratchet above the number the tree reaches SILENTLY LICENSES ONE REGRESSION. §3 fell because two cap notices written this stage were caught as NEW offenders while being written (a sentence is reading copy, so text-caption/label are below the floor and do not count as a fix) and moved to text-body-sm; the other three were already below their ratchet before this stage touched them.   // -2, 2026-08-31, DG-A-12 (Ali sign-off): `ui/stat.tsx` named its two FREE rungs — `text-[14px]` -> `text-body` and `text-[18px]` -> `text-title-sm`, the same px, so no figure moved. The other five stay hand-typed on purpose: 15/17/24 are the --type-* ladder (T7) and 13.5/21 are off both, with 35 money call sites behind the 13.5 default.   // -4, 2026-08-31, DG-S-02: every hand-typed size in `ui/tabs.tsx` is gone — `text-[13px]` (line) and `text-[12.5px]` (segmented) onto `text-body-sm`, and the two `text-[11px]` count pips into the kit `<CountBadge>`. ⭐ The primitive that §K rule 7 (2026-08-31) sends every admin section rail to now spells no size by hand, which is the point of fixing it BEFORE an adopter: the two drifting variants had ZERO call sites, so the repaint cost nothing on the day and would have cost twelve consoles a week later. ⚠️ BUT THE RUNG SWAP IS NOT FREE ON THE ONE VARIANT THAT DOES SHIP, and an A/B bench in the real production fonts said so after the commit message had already claimed it was: `text-[13px]` sets a size and nothing else, while `text-body-sm` carries `letter-spacing: -0.05px` and `line-height: 18px` — so `/wallet`'s rail moved 92.44 → 92.03px on "Activity". A convergence (both shipped admin section rails already render `text-body-sm`), but a rendered one. ⛔ Moving a hand-typed size onto its ladder rung is never only a rename; check what else the rung carries.   // -10, 2026-08-30, session 82: DG-A-08's kit conversions delete a hand-typed size every time a hand-rolled control becomes a `<Button>`, and DG-P-03 moved six page h1s onto `text-title-lg`. # −1, 2026-08-30, DG-A-06, same chip. ⚠️ A ratchet left ABOVE the number the tree already reaches is SLACK, not safety — it silently licenses one regression, and this one was caught by a reviewer re-deriving all three rather than by the suite, which passes either way.
...
const RATCHET_INLINE_FONTSIZE = 32; // -N, 2026-09-08 (PLAYER QUERY stage 4): re-derived from the run and locked in rather than left as slack — this file's own rule is that a ratchet above the number the tree reaches SILENTLY LICENSES ONE REGRESSION. §3 fell because two cap notices written this stage were caught as NEW offenders while being written (a sentence is reading copy, so text-caption/label are below the floor and do not count as a fix) and moved to text-body-sm; the other three were already below their ratchet before this stage touched them.   // −2, 2026-09-03 (PV-13c): `admin/moderation`'s two comment chips stopped hand-typing `style={{ fontSize: 9.5, padding: "1px 7px" }}` around a `<Chip>` and took the kit's own `size="sm"` rung instead. ⭐ A row about finishing a COMPONENT migration paid a type-scale dividend, the same way DG-A-08's tap-floor row paid a tracking one — a call site that stops reaching around a primitive stops hand-typing its metrics too.
...
// ───────────────────────────────────────────────────────────────────────────────
{
  /* 🔴 STRIP CSS COMMENTS FIRST (found 2026-08-31, DG-A-12). This read RAW globals.css, so the
     prose at :4016 — "⛔ WHY NOT `--type-table: 12.5px`, which is the option the register
     offered Ali" — was parsed as a REAL rung, which is why §7 printed 13 `--type-*` vars when
     the stylesheet defines 12. Harmless for `table` (Tailwind has no such key, so it never
     reached the collision set) but the MECHANISM is unsafe in both directions: the Map is
     filled by iteration, so a comment merely MENTIONING an existing name later in the file
     silently OVERWRITES the real value — a discussion of a token could fail the drift check
     against correct code, or restate a frozen value and mask a genuine drift.
     ⛔ NOT `decomment()`: that is the JS/TS stripper and it also removes `//`, which in a
     stylesheet eats the rest of any line holding a `url(https://…)`. CSS has block comments
     only, so `decommentCss` is the CSS-aware sibling — ⭐ and it lives in `lib/decomment.mts`
     rather than here because writing the one-liner at this call site made THIS FILE the 56th
     private comment-stripper, and `test:decomment` §2.1 failed on it immediately. One home. */
  const css = decommentCss(readFileSync(join(SRC, "app", "globals.css"), "utf8"));
  const cfg = readFileSync(join(ROOT, "tailwind.config.ts"), "utf8");
