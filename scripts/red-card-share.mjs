/**
 * red:card-share — proves `test:card-share` catches the defects it names, each on its OWN
 ...
 *
 * ⛔ The anchors are DATA, in `scripts/anchors/card-share.anchors.mjs`, so `test:red-anchors`
 * can audit that every one still resolves exactly once WITHOUT running this file.
 *
 * Run: npm run red:card-share
 */
import { readFileSync, writeFileSync } from "node:fs";
import { execFileSync } from "node:child_process";
import { injectDefect } from "./red-anchor.mjs";
import { MUTATIONS } from "./anchors/card-share.anchors.mjs";
...
const GATE = "scripts/card-share.test.mts";
...
const base = runGate();
if (base.code !== 0) {
  console.error("REFUSING: test:card-share is already RED on the untouched tree.");
