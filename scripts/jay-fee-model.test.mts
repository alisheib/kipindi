/**
 * loser-share ("Jay") fee model — the golden suite.
 *
...
// Jay's rates: Platform 3% + Operator 10% = 13% of the losing pool.
const JAY: Partial<FeeRates> = { feeModel: "loser-share", platformFeeRate: 0.03, operatorFeeRate: 0.10 };
...
// ── 1 · The fee, both outcomes (Jay's sheet: 84,500 and 2,080) ───────────────
{
  const feeYes = poolFee(YES, NO, JAY, "YES"); // YES wins → NO (650,000) loses
  const feeNo = poolFee(YES, NO, JAY, "NO");   // NO wins  → YES (16,000) loses
...
// ── 2 · Per-player payouts (Jay's sheet, to the shilling) ────────────────────
{
  // YES side wins: Jimmy 1,000 / p2 5,000 / p3 10,000 (pool 16,000).
  const jimmy = settledPayoutFor({ yesPool: YES, noPool: NO, side: "YES", stake: 1_000 }, JAY);
  ok("2: Jimmy (YES 1,000) → 36,344", jimmy.payout === 36_344, `payout=${jimmy.payout}`);
  // NO side wins: Jay 100,000 / p2 250,000 / p3 300,000 (pool 650,000).
  const jayp = settledPayoutFor({ yesPool: YES, noPool: NO, side: "NO", stake: 100_000 }, JAY);
  ok("2: Jay (NO 100,000) → 102,142", jayp.payout === 102_142, `payout=${jayp.payout}`);
  // Every winner gets back at least their stake.
  ok("2: winner floor — Jimmy ≥ stake", jimmy.payout >= 1_000);
  ok("2: winner floor — Jay ≥ stake", jayp.payout >= 100_000);
}

// ── 3 · Conservation (no mint / no leak), both outcomes ──────────────────────
{
  // YES wins: winners are the YES side (stakes 1,000 / 5,000 / 10,000).
  const feeYes = poolFee(YES, NO, JAY, "YES");
  ...
  const feeNo = poolFee(YES, NO, JAY, "NO");
...
{
  const feeOne = poolFee(30_000, 0, JAY, "YES"); // nobody on NO → no losing pool
...
{
  const feeYes = poolFee(YES, NO, JAY, "YES"); // 84,500
...
{
  const proj = payoutFor({ yesPool: YES - 1_000, noPool: NO, side: "YES", stake: 1_000 }, JAY);
  const settled = settledPayoutFor({ yesPool: YES, noPool: NO, side: "YES", stake: 1_000 }, JAY);
...
{
  // Raise maxStake so Jay's example stakes (up to 650,000) are accepted — the
  // default cap is 100,000, which would otherwise reject the NO bet and make the
  // poll one-sided.
  await setGlobalConfig({ feeModel: "loser-share", platformFeeRate: 0.03, operatorFeeRate: 0.10, maxStake: 1_000_000 }, "jay-test");

  const m = await createMarket({
    titleEn: "Jay loser-share e2e", titleSw: "Soko la ada", category: "macro",
  ...
  await fundedUser("jay_y1");
  await fundedUser("jay_n1");
  await buyPosition("jay_y1", { marketId: m.id, side: "YES", stake: 16_000 });
  await buyPosition("jay_n1", { marketId: m.id, side: "NO", stake: 650_000 });

  const beforeY = await bal("jay_y1");
  ...
  const paidY = (await bal("jay_y1")) - beforeY;
  const settled = (await listPositionsForMarket(m.id)).find((p) => p.userId === "jay_y1");
...
console.log(`\njay-fee-model: ${pass} passed, ${fail} failed`);
