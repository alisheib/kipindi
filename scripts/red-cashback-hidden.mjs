/**
 * red:cashback-hidden — proves `test:cashback-hidden` catches the defects it names, each on its OWN
 ...
 *
 * ⭐ TWO OF THE SIX ARE NOT ABOUT FIRST/LAST AT ALL, and they are the ones worth reading.
 * `window-lists-everything` makes the pager reach TOO MUCH — every reachability assertion
 * then passes more easily while a 1,000-page list would render 1,000 buttons. And
 * `control-no-localising-sites` empties the population §3 sweeps, because a rule over zero
 * files passes in silence.
 *
 * ⭐ THE MATCH IS ON `FAIL <label>`, NOT ON A SECTION NUMBER, so a defect caught for the
 * wrong reason cannot print PASS.
 *
 * ⛔ The anchors are DATA, in `scripts/anchors/cashback-hidden.anchors.mjs`, so `test:red-anchors`
 * can audit that every one still resolves exactly once WITHOUT running this file.
 *
 * Run: npm run red:cashback-hidden
 */
import { readFileSync, writeFileSync } from "node:fs";
import { execFileSync } from "node:child_process";
import { injectDefect } from "./red-anchor.mjs";
import { MUTATIONS } from "./anchors/cashback-hidden.anchors.mjs";
...
const GATE = "scripts/cashback-hidden.test.mts";
...
const base = runGate();
if (base.code !== 0) {
  console.error("REFUSING: test:cashback-hidden is already RED on the untouched tree.");
