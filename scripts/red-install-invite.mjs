/**
 * `npm run red:install-invite` — DOES `test:install-invite` ACTUALLY CATCH ANYTHING?
 *
 *   node scripts/red-install-invite.mjs
 *
 * For each mutation declared in `scripts/anchors/install-invite.anchors.mjs`: inject the defect
...
import { MUTATIONS } from "./anchors/install-invite.anchors.mjs";

const ROOT = new URL("..", import.meta.url).pathname.replace(/^\/([A-Za-z]:)/, "$1");
const SUITE = "scripts/install-invite.test.mts";
...
function runSuite() {
  ...
  const tally = /install-invite: (\d+) passed, (\d+) failed/.exec(out);
...
console.log("\nred:install-invite — §3. Each mutation is a plausible SIMPLIFICATION, because that is how this feature will really break.\n");
...
console.log(`\nred:install-invite: ${caught}/${primaries.length} defects caught` +
