/**
 * red:pager-reach — proves `test:pager-reach` catches the defects it names, each on its OWN
 ...
 *
 * ⛔ The anchors are DATA, in `scripts/anchors/pager-reach.anchors.mjs`, so `test:red-anchors`
 * can audit that every one still resolves exactly once WITHOUT running this file.
 *
 * Run: npm run red:pager-reach
 */
import { readFileSync, writeFileSync } from "node:fs";
import { execFileSync } from "node:child_process";
import { injectDefect } from "./red-anchor.mjs";
import { MUTATIONS } from "./anchors/pager-reach.anchors.mjs";
...
const base = runGate();
if (base.code !== 0) {
  console.error("REFUSING: test:pager-reach is already RED on the untouched tree.");
