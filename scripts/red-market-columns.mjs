/**
 * red:market-columns — proves `test:market-columns` catches the defects it names, each on its OWN
 ...
 *
 * ⛔ The anchors are DATA, in `scripts/anchors/market-columns.anchors.mjs`, so `test:red-anchors`
 * can audit that every one still resolves exactly once WITHOUT running this file.
 *
 * Run: npm run red:market-columns
 */
import { readFileSync, writeFileSync } from "node:fs";
import { execFileSync } from "node:child_process";
import { injectDefect } from "./red-anchor.mjs";
import { MUTATIONS } from "./anchors/market-columns.anchors.mjs";
...
const GATE = "scripts/market-columns.test.mts";
...
const base = runGate();
if (base.code !== 0) {
  console.error("REFUSING: test:market-columns is already RED on the untouched tree.");
