...
export const NOT_EYEBROW = new Map([
  ...
  // 2026-09-13 · KYC at withdrawal: the queue index, the refused-funds report and the workstation header. Read, not guessed.
  ["app/admin/kyc/[id]/page.tsx :: <span className=\"font-mono text-micro uppercase tracking-[0.12em] text-text-subtle\"> ↵ #{queuePos + 1} of {pending.length} by submission · oldest {ageLabel(oldest)}","OTHER"],
  ["app/admin/kyc/[id]/page.tsx :: <Link href={\"/admin/kyc\" as Route} className=\"inline-flex items-center gap-1.5 h-[40px] px-3 rounded-md border border-border bg-bg-inset font-mono text-caption tracking-[","CONTROL_LABEL"],
  ["app/admin/kyc/page.tsx :: <div className=\"flex items-center gap-2 font-mono text-micro uppercase tracking-wider\"> ↵ <Link","CONTROL_LABEL"],
  ["app/admin/kyc/refused/page.tsx :: <td className=\"py-2 pr-3 font-mono text-micro uppercase tracking-wider\">{a.walletStatus ?? \"—\"}</td> ↵ <td className=\"py-2 pr-3 text-right font-mono tabular-nums text-tex","OTHER"],
  ...
  ["components/auth/auth-shell.tsx :: <div className=\"relative font-mono text-micro uppercase tracking-[0.16em] text-text-subtle\"> ↵ {t.auth.licensedByGbt} <a href={`tel:${HELPLINE_TEL()}`} className=\"whitesp","OTHER"],
  ["components/auth/auth-shell.tsx :: <p className=\"mt-6 text-center font-mono text-micro uppercase tracking-[0.16em] text-text-subtle lg:hidden\"> ↵ {t.auth.licensedByGbt} <a href={`tel:${HELPLINE_TEL()}`} cl","OTHER"],
