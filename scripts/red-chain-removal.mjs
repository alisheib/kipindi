/**
 * red:chain-removal — proves `test:chain-removal` catches the defects it names, each on its OWN
 ...
 *
 * ⛔ The anchors are DATA, in `scripts/anchors/chain-removal.anchors.mjs`, so `test:red-anchors`
 * can audit that every one still resolves exactly once WITHOUT running this file.
 *
 * Run: npm run red:chain-removal
 */
import { readFileSync, writeFileSync } from "node:fs";
import { execFileSync } from "node:child_process";
import { injectDefect } from "./red-anchor.mjs";
import { MUTATIONS } from "./anchors/chain-removal.anchors.mjs";
...
const GATE = "scripts/chain-removal.test.mts";
...
const base = runGate();
if (base.code !== 0) {
  console.error("REFUSING: test:chain-removal is already RED on the untouched tree.");
