 ...
 */
const CEILING = 473;   // -2, 2026-09-23: PRE-EXISTING RED, and not this session's work — the same shape as the 2026-09-05 HOUSE-3 entry below. The ratchet arrived at HEAD failing check 2 at 473 against 475: two usages had been removed by earlier work that did not lower the constant. Re-measured here rather than copied — 473 across 171 files — and confirmed to predate today: `git diff 7747dffa..HEAD -- src/` adds and removes ZERO inverted keys, so nothing in this session's window moved it. ⛔ Check 2 is an EQUALITY on purpose: a ceiling above the real count has stopped being a ratchet and silently re-admits two usages. // -1, 2026-09-13 (withdrawal review removed): the admin approvals page lost its Co-sign chip and its px-2.5. // -2, 2026-09-13 (the visual pass): p-3.5 → p-[14px] on the KYC upload card and space-y-2.5 → gap-[10px] on the register consents, both same-pixel. // -75, 2026-09-10: THE RATCHET WAS RED — 561 against a ceiling of 553, eight usages added by work that landed after 2026-09-05, and it had been holding `predeploy` red. ⭐ CLEARED BY MOVING 83 USAGES OFF THE STOCK SCALE WITH ZERO RENDERED CHANGE, which is possible for exactly FOUR of the eight inverted keys: the override is all px and the stock keys leak rem, and for these four the two scales land on the SAME pixel — proven from COMPILED CSS, not assumed: `16 → 4rem → 64px == 9 → 64px`, `20 → 5rem → 80px == 10`, `24 → 6rem → 96px == 11`, `32 → 8rem → 128px == 12`. The root really is 16px (globals.css sets no html font-size; production reports 16px), which is the premise that makes rem==px here. ⛔ `2.5` (10px), `3.5` (14px), `14` (56px) and `28` (112px) have NO override equivalent — moving those is a real ±2px rendered change and NONE were touched, which is why 478 is not lower. ⚠️ SCOPED TO `loading.tsx` SKELETONS, 31 files: placeholder boxes with no text to reflow, no interactive control to displace and nothing measuring them. **The same four keys appear 35 more times in files carrying real content and were deliberately LEFT** — this file's own note warns against a sweep that spreads a problem while tidying one. 🔴 **AND THE FIRST ATTEMPT AT THE FIX TRADED THIS FAILURE FOR ANOTHER ONE, WHICH IS THE PART WORTH READING.** Swapping `w-16 → w-9` satisfies THIS guard and violates `ui-consistency`'s `numeric-size-utility`, which condemns keys 7–12 on `h`/`w`/`min-*`/`max-*` precisely BECAUSE they read from the overridden scale — 30 new findings, in the same edit that made this one green. ⭐ **Both guards are right, so the remedy is PER-PREFIX**, and the other rule's own description names it (*"use an explicit literal (h-[32px])"*): width/height/size take `w-[64px]`, while padding·margin·gap·inset take the override key. 89 of the 90 sites here were width/height, so they are literals. ⭐ HOW THE "no rendered change" CLAIM WAS CHECKED rather than asserted: the whole diff was re-derived by applying the per-prefix map to every removed line and compared to the added line — all 88 changed lines differ ONLY by that swap, and the checker carries a non-empty control because its first version passed an EMPTY diff (0 >= 0). // -1, 2026-09-05, HOUSE-3: PRE-EXISTING RED, not this session's work. The ratchet arrived at HEAD failing check 2 at 553 against a ceiling of 554 — a parallel session removed an inverted key and did not lower the constant, and check 2 is an EQUALITY on purpose: a ceiling above the real count has stopped being a ratchet and silently re-admits one usage. Re-measured here, not copied: 553 across 185 files. // -7, 2026-09-03: `layout/needle-drawer.tsx` — the Needle's theme menu shipped ELEVEN inverted keys (3× `gap-2.5`, 3× `py-2.5`, `px-2.5`, 2× `pt-3.5`, `py-3.5`, `mt-3.5`), which is what took this ratchet 561 → 565 and held `test:all` red on `main`. All eleven moved onto the scale: 2.5 (Tailwind STOCK 10px) → 2 (this product's OVERRIDDEN 12px) and 3.5 (stock 14px) → 3 (override 16px). ⚠️ EVERY MOVE IS +2px AND IS A RENDERED CHANGE, said plainly rather than folded in — a drawer menu's gaps and paddings each grow 2px, verified at 320/390 not to overflow or clip. ⭐ AND NOTE WHICH DIRECTION THE DEFECT RAN: `gap-2.5` painted 10px where `gap-2` paints 12, so the file's author had written the LARGER-looking key for the SMALLER box, four times, without any way to notice. That inversion is derived from the two scales by §0, not typed in — which is why this guard could name it. ⛔ The file belongs to a parallel session; the change is 2px of air on the product's own scale, and the eleven sites are listed here so its owner can see exactly what moved.   // -1, 2026-08-31, DG-S-03: `/admin/roles`'s hand-rolled section rail declared its height as `py-2.5` — 10px on a scale where `py-2` is 12 — and it went with the rail when the page adopted the kit `<Tabs variant="line">`, whose 44px is a declared height rather than padding. ⭐ Worth reading twice, because it is the same defect in two guards at once: `tap-target` §3 reads a DECLARED height and is structurally blind to one made of padding, so `/admin/players/[id]`'s 52px `py-3` rail has never been visible to it either. A control that states its height as padding is invisible to the tap gate AND, at a `.5` step, counted by this one.   // -59, 2026-08-30, session 82 -- the largest fall this ratchet has taken, and it is a SIDE EFFECT: DG-A-08 replaced ~24 hand-rolled admin controls with kit `<Button>`s, and every one of them took its hand-typed `px-2.5 py-1.5` inverted-key padding with it. ⛔ The matcher was NOT touched this session (`git status` on this file: clean), which is the check that separates a cleanup from a collapse -- and §3 still sees 566 usages across 188 files. # 2026-08-30: -3 (DG-A-06). `px-2.5` left three hand-rolled selection capsules as they converted to FilterPill — CardSortControl's two byte-identical copies and /admin/proposals' queue rail. ⭐ The primitive's dense rank writes `px-2.5` too, so the win is real but SMALLER than the conversion looks: the inverted key moved from three call sites into ONE definition site, which is the shape this ratchet wants. Before that — 2026-08-29: -3, AdminKpi x2 + its skeleton left p-3.5 (14px) for p-2 (12px) — DG-A-16. Then -1 on 2026-08-30 (DG-P-03): the BackLink ghost's `w-16`. ⚠️ AND THIS GUARD EARNED ITS KEEP THE SAME DAY — adopting `PageHeader` in the deposit and withdraw skeletons meant COPYING that ghost into two more files, and this suite caught it at 631 before it shipped. A ratchet's job is not only to shrink a backlog; it is to stop a sweep from spreading one while it tidies something else.
