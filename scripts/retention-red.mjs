/**
 * RED PROOF for the retention pass (F-01 + F-09) — every assertion in `test:erasure` broken on purpose.
 *
 *   node scripts/retention-red.mjs        (npm run red:retention)
 *
 ...
 *
 * ⭐ A RETENTION PASS HAS THREE WAYS TO BE WRONG, not two, and the third is the new one.
 * It must DELETE what a published period covers (cases 1, 6) and reach NOTHING ELSE
 * (cases 3, 4, 9) — those two the suite has always held. The F-09 payload pass adds
 * **saying the wrong thing about what it did**: case 2 removes the tombstone so a pruned
 * payload is indistinguishable from one that never existed, case 5 stamps "pruned" over a
 * column that was always null, and case 7 names the audit field for a deletion that did not
 * happen. A pass that frees the right bytes and misreports them is still a pass that lies to
 * the next person reading the log.
 *
...
import { MUTATIONS } from "./anchors/retention.anchors.mjs";

const SUITE = "scripts/retention.test.mts";
...
for (const m of MUTATIONS) {
  ...
  const line = out.match(/^\s*DATA RETENTION: (\d+) passed, (\d+) failed$/m);
