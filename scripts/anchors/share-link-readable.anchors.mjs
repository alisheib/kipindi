...
export const MUTATIONS = [
  ...
  },
  {
    ...
    to: `      el.style.height = "auto";`,
    expect: "2: ⛔ …and it collapses BEFORE measuring, or the field can only ever grow",
  },
  {
    name: "height-ignores-the-borders",
    why: "⭐ THE 2px THE GUARD ACTUALLY CAUGHT, in my own fix. Tailwind's preflight sets `box-sizing: border-box`, so assigning `height` sets the BORDER box while `scrollHeight` measures the CONTENT box — leaving the field exactly `borderTop + borderBottom` short, with `overflow: hidden` shaving the bottom of the last line. Production reported 72 vs 70 at 393, 48 vs 46 at 768 and 44 vs 42 at 1440: the same 2px everywhere, which is what a constant border looks like",
    file: INVITE,
    suite: "share-link-readable",
    from: `el.style.height = \`\${el.scrollHeight + border}px\`;`,
    to: `el.style.height = \`\${el.scrollHeight}px\`;`,
    expect: "2: ⛔ …and it adds the borders back, because height sets the BORDER box",
  },
  {
