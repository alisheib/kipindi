...
/**
 * ⛔ AN `expect` NEVER PINS A COUNT THE RUN COMPUTES (found by the 2026-09-21 ops ← alerts merge).
 * Two declarations here quoted their assertion's label INCLUDING its runtime detail —
 * `7.retired · … (3 names swept over 1087 files)` and `7.nofold · … (5 foldings swept on 2 files)`.
 * The merge added two files under `src/`, the sweep printed **1089**, and `7.505`'s roll-call reported the
 * declaration STALE — while the assertion itself passed. ⛔ A declaration that goes stale because an unrelated
 * lane added a file is not measuring anything; it just fails on somebody else's commit and trains the next
 * session to wave the roll-call through.
 * ⭐ THIS IS NOT A WEAKENING. The roll-call matches by SUBSTRING and `7.retired` / `7.nofold` are unique, so the
 * shortened label identifies exactly the same assertion — the discarded half carried no discriminating power,
 * only a number that rots. Keep the stable prefix; never re-add the parenthesis.
 */
export const MUTATIONS = [
  ...
  if (reason.length < CONSOLE_REASON_MIN) return { ok: false, error: SWITCH_COPY.reasonShort };`,
    expect: "7.nofold · ⛔ neither ceremony case-folds or re-spaces what was typed",
    suite: "rules",
  },
  {
  ...
  },
  {
    ...
    expect: "7.retired · ⛔ no file under src/ names the retired normaliser",
    suite: "rules",
  },
];
