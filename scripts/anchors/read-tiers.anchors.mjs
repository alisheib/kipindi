...
export const MUTATIONS = [
  ...
  {
    name: "audit-files-a-payout-read-against-a-player",
    why: "the reveal's audit row goes back to the literal \"User\", so an msisdn read — whose subject is a TRANSACTION — is filed against a player id. The row still exists and still names the field, so nothing looks missing; it simply points an investigator at the wrong record, which is the one moment this trail is ever read",
    file: PLAYER_ACTIONS,
    suite: "read-tiers",
    from: `    targetType: "targetType" in spec ? spec.targetType : "User",`,
    to: `    targetType: "User",`,
    expect: "8.3 the reveal's audit row takes its targetType from the registry",
  },
  {
    name: "reveal-throws-instead-of-refusing",
    why: "the refusal goes back to a THROW. `sensitive-reveal.tsx` renders `r.error`, and a rejected transition never reaches that branch — so a refused officer sees NOTHING at all and reports the console as broken rather than as locked",
    file: PLAYER_ACTIONS,
    suite: "read-tiers",
    from: `  if (!gate.ok) return gate;`,
    to: `  if (!gate.ok) throw new Error(gate.error);`,
    expect: "8.5 …and it refuses SOFTLY",
  },
  {
    name: "mask-module-imports-the-store",
    why: "⛔ THE CLIENT/SERVER BOUNDARY, WHICH IS A RUNTIME CONTRACT `tsc` AND `next build` BOTH PASS. `phone-normalize.ts` is masked FROM CLIENT COMPONENTS (`app-shell.tsx`, `auth/otp`); one server import here drags the Prisma store into a browser chunk. This repo has already taken every page down once with exactly that shape",
    file: "src/lib/phone-normalize.ts",
    suite: "read-tiers",
    from: `export function maskPhone(`,
    to: `import { db } from "@/lib/server/store";\nexport function maskPhone(`,
    expect: "8.6 ⛔ the mask lives in a module with NO imports",
  },
  {
    name: "masked-csv-column-lies-about-itself",
    why: "the CSV header stops renaming itself when the column is masked, so a reconciler matching our file against Selcom's statement reads `+255••••01` as a corrupt number and opens an incident. A spreadsheet has no eye and no tooltip; the header is the only place the artefact can say which of the two it is",
    file: "src/app/api/admin/transactions/export/route.ts",
    suite: "read-tiers",
    from: `full ? "msisdn" : "msisdn_masked"`,
    to: `"msisdn"`,
    expect: "8.10 …and the column NAMES itself when masked",
  },
  {
    name: "dsar-bundle-starts-masking",
    why: "the DSAR export masks the phone. ⛔ That LOOKS like a privacy improvement and is a COMPLIANCE DEFECT: GDPR Art. 15 entitles the subject to their own data, and handing them dots is a failure to disclose. The one direction this axis must never subtract is the player's own record — READ-TIERS §6 says so and this is where it would bite",
    file: "src/lib/server/privacy.ts",
    suite: "read-tiers",
    from: `    phoneE164: user.phoneE164,`,
    to: `    phoneE164: maskPhone(user.phoneE164),`,
    expect: "8.18 ⛔ the DSAR bundle carries the FULL phone",
  },
  {
    name: "dsar-export-stops-being-audited",
    why: "a full-PII bundle is handed out and nothing records that it was. The file still downloads and the officer sees no difference — the only thing lost is the sentence a regulator asks for after an incident, which is the entire product of D4",
    file: "src/app/admin/players/[id]/actions.ts",
    suite: "read-tiers",
    from: `      action: "player.data_exported",`,
    to: `      action: "player.viewed",`,
    expect: "8.19 …and that export is AUDITED",
  },
  {
    name: "fiu-report-stops-masking-the-phone",
    why: "the statutory suspicious-activity report emits full phone numbers. Its own column header says `masked`, so the artefact would be self-contradicting — and it is the one file here that goes to an outside authority",
    file: "src/lib/server/reports/catalogue.ts",
    suite: "read-tiers",
    from: "`${u.phoneE164.slice(0, 4)}*****${u.phoneE164.slice(-2)}`",
    to: "u.phoneE164",
    expect: "8.20 ⛔ the FIU suspicious-activity report MASKS the phone",
  },
  {
    name: "self-exclusion-register-masks-instead-of-hashing",
    why: "the cross-operator register emits a masked display string where a salted digest belongs. ⛔ It reads as MORE private and destroys the artefact's purpose: other operators match on the digest to honour a self-exclusion, and `+255••••01` matches nothing — a self-excluded player stays blocked here and is admitted everywhere else",
    file: "src/lib/server/reports/catalogue.ts",
    suite: "read-tiers",
    from: `    const phoneHash = hashIdentifier(u.phoneE164);`,
    to: `    const phoneHash = maskPhone(u.phoneE164);`,
    expect: "8.21 ⛔ the cross-operator self-exclusion register HASHES it",
  },
  {
