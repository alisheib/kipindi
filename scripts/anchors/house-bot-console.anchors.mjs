...
export const MUTATIONS = [
  ...
  },
  {
    ...
    file: GATE,
    from: `      lossCell: dayBooks == null ? "—" : moneyUsage(book?.projectedLossTzs ?? 0, bot.capDailyLossTzs),`,
  ...
  },
  {
    ...
    file: GATE,
    from: `  return \`used \${formatTzs(Math.max(0, used))} of \${formatTzs(limit)}\`;`,
    to: `  return \`\${formatTzs(Math.max(0, used))} / \${formatTzs(limit)}\`;`,
  ...
  {
    name: "407-tabular · a money cell loses `tabular`, so its digits stop lining up column to column",
    file: PAGE,
    from: `                          <td className="p-3 tabular text-right"><span className="amount">{r.lossCell}</span></td>`,
    to: `                          <td className="p-3 text-right"><span className="amount">{r.lossCell}</span></td>`,
