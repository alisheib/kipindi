/**
 * red:msisdn-prefill — proves `test:msisdn-prefill` catches the defects it names, each on its OWN
 ...
 *
 * ⛔ The anchors are DATA, in `scripts/anchors/msisdn-prefill.anchors.mjs`, so `test:red-anchors`
 * can audit that every one still resolves exactly once WITHOUT running this file.
 *
 * Run: npm run red:msisdn-prefill
 */
import { readFileSync, writeFileSync } from "node:fs";
import { execFileSync } from "node:child_process";
import { injectDefect } from "./red-anchor.mjs";
import { MUTATIONS } from "./anchors/msisdn-prefill.anchors.mjs";
...
const GATE = "scripts/msisdn-prefill.test.mts";
...
const base = runGate();
if (base.code !== 0) {
  console.error("REFUSING: test:msisdn-prefill is already RED on the untouched tree.");
