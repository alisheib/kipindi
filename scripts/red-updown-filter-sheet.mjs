/**
 * red:updown-filter-sheet — proves `test:updown-filter-sheet` catches the defects it names, each
 * on its OWN assertion. Reintroduce the REAL defect, assert the gate goes red on THAT case's
 * assertion rather than on some incidental collapse, restore, and prove the gate is green again.
 *
 * ⭐ `trigger-says-filters` IS THE POSITIVE CONTROL AND IT IS WHY THIS SUITE EXISTS. It collapses
 * the filters PERFECTLY — one tidy button, 196px of chips gone, the board four rows higher — and
 * the trigger reads "Filters" instead of "Bitcoin · 3 min". Every space-saving claim about the
 * unit still holds, and the player has lost the answer to "what am I looking at?". If §2 ever
 * stops catching that, this guard is measuring furniture instead of meaning.
 *
 * ⭐ `split-at-lg` is the other one worth reading: widening the disclosure from `sm` to `lg` looks
 * like more of a good thing and removes working controls from tablets, where both rails measured
 * a single clean 44px row. The defect had a band; the fix has to have the same one.
 *
 * ⭐ THE MATCH IS ON `FAIL <label>`, NOT ON A SECTION NUMBER, so a defect caught for the wrong
 * reason cannot print PASS.
 *
 * ⛔ The anchors are DATA, in `scripts/anchors/updown-filter-sheet.anchors.mjs`, so
 * `test:red-anchors` can audit that every one still resolves exactly once WITHOUT running this
 * file.
 *
 * Run: npm run red:updown-filter-sheet
 */
import { readFileSync, writeFileSync } from "node:fs";
import { execFileSync } from "node:child_process";
import { injectDefect } from "./red-anchor.mjs";
import { MUTATIONS } from "./anchors/updown-filter-sheet.anchors.mjs";
...
const GATE = "scripts/updown-filter-sheet.test.mts";
...
const base = runGate();
if (base.code !== 0) {
  console.error("REFUSING: test:updown-filter-sheet is already RED on the untouched tree.");
