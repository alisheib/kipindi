/**
 * red:selcom-statement — proves `test:selcom-statement` catches the defects it names, each on
 ...
 *
 * ⭐ THE TWO WORTH READING ARE `ledger-labelled-rail` AND `collections-derived-from-ledger`,
 * because neither changes a single total. The first captions our own ledger figures as
 * something Selcom reported — every number on the page stays correct, and a regulator is
 * told the wrong institution vouched for them. The second invents the C2B balance Selcom
 * does not publish, out of our deposits, and the card then looks COMPLETE: two balances,
 * neither blank. That is the most tempting wrong fix on this whole unit, which is why A-5
 * exists and why the guard asserts the absence rather than the presence.
 *
 * ⭐ AND `control-internal-credits-zeroed` IS THE POSITIVE CONTROL — it zeroes the
 * conflation figure at source, so every *"BET_PAYOUT is not in the rail figure"* assertion
 * passes HARDER while the number the card exists to hold apart from the rail has silently
 * left the page. A suite that reported this clean would be scoring the absence of the
 * feature as the presence of the fix.
 *
 * ⚠️ THE RECONCILIATION IS NOT HERE, DELIBERATELY. Whether the page's figures match
 * production is settled by `npm run qa:selcom-statement`, which reads the rendered page and
 * an independent SQL census and compares them. This harness proves only that the offline
 * gate cannot be fooled. Neither is evidence for the other.
 *
 * ⛔ The anchors are DATA, in `scripts/anchors/selcom-statement.anchors.mjs`, so
 * `test:red-anchors` can audit that every one still resolves exactly once WITHOUT running
 * this file.
 *
 * Run: npm run red:selcom-statement
 */
import { readFileSync, writeFileSync } from "node:fs";
import { execFileSync } from "node:child_process";
import { injectDefect } from "./red-anchor.mjs";
import { MUTATIONS } from "./anchors/selcom-statement.anchors.mjs";
...
const GATE = "scripts/selcom-statement.test.mts";
...
const base = runGate();
if (base.code !== 0) {
  console.error("REFUSING: test:selcom-statement is already RED on the untouched tree.");
