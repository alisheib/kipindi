/**
 * `npm run red:bonus-relock` — DOES `test:bonus-relock` ACTUALLY CATCH ANYTHING?
 *
 *   node scripts/red-bonus-relock.mjs
 *
 * For each mutation declared in `scripts/anchors/bonus-relock.anchors.mjs`: inject the defect
...
import { MUTATIONS } from "./anchors/bonus-relock.anchors.mjs";

const ROOT = new URL("..", import.meta.url).pathname.replace(/^\/([A-Za-z]:)/, "$1");
const SUITE = "scripts/bonus-relock.test.mts";
...
function runSuite() {
  ...
  const tally = /bonus-relock: (\d+) passed, (\d+) failed/.exec(out);
...
console.log("\nred:bonus-relock — E-224, the bonus re-lock. The mutations remove the FIX, not the defect.\n");
...
console.log(`\nred:bonus-relock: ${caught}/${primaries.length} defects caught` +
