...
-- transaction that lock is HELD UNTIL COMMIT. Each table is locked ACCESS EXCLUSIVE ONCE, BEFORE any
-- other lock on it: a bet reads and then writes "Position" in one transaction, and a SHARE taken
-- first (a CREATE INDEX) deadlocks with it (40P01) when the ADD COLUMN then asks for more.
--   1 Position: the column, then the keyset index and its 3 marker indexes;
--   2 Transaction column → its index; 3 PredictionMarket; 4 User (sign-in writes) last.
-- Reads of a table wait while its small-table index builds run under that lock; tables above the
-- A23 thresholds are prepared by hand first (below), so their builds never run here.
-- lock_timeout makes every acquisition fail with 55P03 within 3 s rather than stall bets
-- (test:house-bot-migrations §b).
-- ⚠️ RESIDUAL, stated honestly: a transaction that already holds a lock on a table later in this
-- file (Transaction, PredictionMarket, User) and then needs an earlier one can still meet 40P01
-- while the file acquires its locks. The apply then rolls back whole, exactly as on 55P03, and the
-- R2 runbook retries.
--
...
-- ⛔ FIRST on this table: its ACCESS EXCLUSIVE must come before any index build's SHARE (header).
ALTER TABLE "Position" ADD COLUMN IF NOT EXISTS "houseBotId" TEXT;
-- The sweep keyset (PLAN §2). Built after the column, under the lock the ADD COLUMN already holds.
