/* 50pick.tz — Affiliate prototype harness + responsive shells.
   Player screens render in the kit Phone (mobile) or a responsive desktop
   web-app shell. Admin renders in a responsive operator shell. A left control
   rail toggles screen / viewport / theme / program-state / data-state. */
...
// Auto-fit a fixed-size stage child into the available area.
...
const statusFor = (screen, paused) => {
  if (screen === 'invite') return <StatusPill paused={paused} />;
  if (screen === 'profile') return <Chip variant="resolved">Gold tier</Chip>;
  if (screen === 'notif') return <Chip variant="pending">3 new</Chip>;
  return null;
};

const renderScreen = (screen, { populated, paused, bonusOn, wide }) => {
  if (screen === 'invite') return <InviteEarn populated={populated} paused={paused} wide={wide} />;
  if (screen === 'profile') return <ProfileEntry wide={wide} />;
  if (screen === 'register') return <Registration bonusOn={bonusOn} wide={wide} />;
  if (screen === 'notif') return <Notifications wide={wide} />;
  return null;
};

// ── Mobile player shell — the kit Phone ─────────────────────────────────
...
const DesktopPlayer = ({ screen, state }) => {
  const m = META[screen];
  // Registration is pre-auth → centered card, no app chrome
  if (m.preAuth) {
    return (
    ...
    <div style={{ display: 'flex', height: '100%', background: 'var(--bg)', color: 'var(--text)' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '28px' }}>
          <div style={{ maxWidth: 680, margin: '0 auto' }}>{renderScreen(screen, { ...state, wide: true })}</div>
        </div>
      </div>
    </div>
  );
...
const ADMIN_NAV = [
  ...
  ['Markets', [['predict', 'Markets', 'markets'], ['asterisk', 'AI candidates', 'ai']]],
  ['Compliance', [['check', 'Resolver queue', 'resolver'], ['clock', 'Audit log', 'audit']]],
];

const AdminWrap = ({ state, setPaused, wide }) => (
  <div style={{ display: 'flex', height: '100%', background: 'var(--bg-deep)', color: 'var(--text)' }}>
    {wide && (
      <aside style={{ width: 224, flexShrink: 0, borderRight: '1px solid var(--border)', background: 'var(--bg)', overflow: 'auto' }}>
        <div style={{ padding: '18px 18px 14px', borderBottom: '1px solid var(--border)' }}>
          <FiftyLockup size={19} color="var(--text)" />
          <Cap style={{ marginTop: 8 }}>Operator console</Cap>
        </div>
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
      <header style={{ height: 54, flexShrink: 0, borderBottom: '1px solid var(--border)', background: 'var(--bg)', display: 'flex', alignItems: 'center', gap: 14, padding: '0 22px' }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--text-subtle)' }}>Growth <span style={{ margin: '0 6px' }}>/</span> <span style={{ color: 'var(--text)', fontWeight: 600 }}>Affiliate</span></div>
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 10 }}>
          <span className="chip" style={{ background: 'color-mix(in oklab, var(--indigo-500) 16%, transparent)', color: 'var(--indigo-200)', borderColor: 'color-mix(in oklab, var(--indigo-500) 36%, transparent)' }}>ACTIVE</span>
          <Avatar initials="NK" size="sm" hue={268} />
        </div>
      </header>
...
const RailGroup = ({ label, children }) => (
  <div style={{ marginBottom: 16 }}><Cap style={{ marginBottom: 8 }}>{label}</Cap>{children}</div>
);
...
const App = () => {
  ...
  const state = { populated, paused, bonusOn };

  let stage;
  if (isAdmin) {
    stage = wide
      ? <FitStage w={1280} h={840}><Frame w={1280} h={840}><AdminWrap state={state} setPaused={setPaused} wide /></Frame></FitStage>
      : <FitStage w={393} h={820}><Frame w={393} h={820} radius={36}><AdminWrap state={state} setPaused={setPaused} wide={false} /></Frame></FitStage>;
  } else if (wide) {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', width: '100vw', background: 'oklch(12% 0.03 268)', overflow: 'hidden' }}>
      <div style={{ width: 280, flexShrink: 0, borderRight: '1px solid oklch(26% 0.04 268)', background: 'oklch(15% 0.035 268)', padding: 20, overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
        <div style={{ marginBottom: 6 }}><FiftyLockup size={22} color="oklch(97% 0.012 268)" /></div>
        <div style={{ fontSize: 12.5, color: 'oklch(68% 0.03 268)' }}>Affiliate / Referral</div>
        <div style={{ fontSize: 11, color: 'oklch(54% 0.03 268)', marginBottom: 20 }}>Feature 1 · player + admin · responsive</div>

        <RailGroup label="Player screens">
          <RailNav active={screen === 'invite'} icon="gift" label="Invite & Earn" sub="1.1 · the hero" onClick={() => setScreen('invite')} />
          <RailNav active={screen === 'profile'} icon="user" label="Profile entry" sub="1.2 · Account row" onClick={() => setScreen('profile')} />
          <RailNav active={screen === 'register'} icon="plus" label="Registration" sub="1.3 · referral ribbon" onClick={() => setScreen('register')} />
          <RailNav active={screen === 'notif'} icon="bell" label="Notifications" sub="1.4 · toasts & rows" onClick={() => setScreen('notif')} />
        </RailGroup>
        <RailGroup label="Admin">
          <RailNav active={isAdmin} icon="megaphone" label="Affiliate dashboard" sub="1.5 · /admin/affiliate" onClick={() => setScreen('admin')} />
        </RailGroup>
        ...
        {screen === 'invite' && <RailGroup label="Data state"><Seg2 value={populated ? 'p' : 'e'} onChange={v => setPopulated(v === 'p')} options={[{ v: 'e', l: 'Empty' }, { v: 'p', l: 'Populated' }]} /></RailGroup>}
        ...
        <div style={{ marginTop: 'auto', paddingTop: 16, fontSize: 10.5, color: 'oklch(56% 0.03 268)', lineHeight: 1.6 }}>
          ...
          Gold = every primary CTA + money received · Indigo = chrome · Claret = danger · YES-green / NO-red = betting only · mono TZS · EN + SW.
        </div>
      </div>
      {stage}
    </div>
