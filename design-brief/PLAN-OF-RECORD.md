...
| Command | What it proves |
|---|---|
| ~~`LOCALES=en,sw,zh npm run qa:landing-seam -- <dir> [url]`~~ | ⛔ **DELETED 2026-09-26 with its subject.** It probed the RG line ↔ footer seam; landing v3 removed that RG line (Ali, R4(5) in `docs/design-system/v4-2026-09-26-landing-ten/INHERIT-MANIFEST.md`) — the 18+ roundel and RG motto now sit in the hero trust lines and the footer, and the helpline is in both BY RULING (the placement map puts it on the first screen). A probe that counted the helpline twice as a regression would now fail the design it was meant to protect |
