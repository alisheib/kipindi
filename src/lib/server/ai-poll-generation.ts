...
import { assertAiBudget, describeAiBudgetBlock, aiBudgetRefusal } from "./ai-usage";
...
export async function generateAIPoll(opts: {
  ...
  if (!budget.ok) throw new OperatorError(describeAiBudgetBlock(budget), aiBudgetRefusal(budget));
...
export async function generateAIPollBatch(opts: {
  ...
  if (!batchBudget.ok) throw new OperatorError(describeAiBudgetBlock(batchBudget), aiBudgetRefusal(batchBudget));
