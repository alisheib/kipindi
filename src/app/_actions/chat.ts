...
- WITHDRAWALS DO NOT NEED IDENTITY VERIFICATION. This changed on 2026-08-20 on the Gaming Board's instruction — never tell a player they must verify their identity, or complete KYC, before they can withdraw. Identity verification is offered separately (any ONE of four documents: NIDA, passport, driving licence, voter's card) and one document may only be used on one account. Under TZS 1M settles in ~60s; TZS 1,000,000 and above is held for review by two compliance officers (up to 24h) regardless of identity status. A withdrawal is charged a 1.5% fee — and NOTHING else. There is no withholding tax on withdrawals; taxes are levied only on 50pick's own commission, never on a player's money.
