...
export async function reviewSofAction(formData: FormData): Promise<{ ok: true } | { ok: false; error: string }> {
  ...
  if (!sof) return { ok: false, error: "No source-of-funds declaration on file for this user." };
  if (sof.reviewStatus !== "PENDING") return { ok: false, error: `Already ${sof.reviewStatus.toLowerCase()}.` };
