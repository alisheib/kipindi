...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    for (let r = 0; r < R; r++) {
      ...
      for (let n = 0; n < N; n++) {
        ...
        for (let e = 0; e < E; e++) {
          ...
          await onRecruitSettlement(rec.id, { operatorNetFee: 1_500, marketId: "mkt_devtest", positionId: `pos_${crypto.randomUUID()}`, houseBotId: null });
          eventCount += 2;
        }
      }
      binds.push(bound);
    ...
    //    totalEarned read-modify-write for lost updates. ───────────────
    {
      ...
      await Promise.all(crecs.map((rid) => Promise.resolve().then(() => onRecruitSettlement(rid, { operatorNetFee: 1_500, marketId: "mkt_devtest", positionId: `pos_${crypto.randomUUID()}`, houseBotId: null }))));
