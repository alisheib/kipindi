...
export async function POST() {
  ...
  try {
    ...
    for (let i = 0; i < 8; i++) await onRecruitSettlement(rec.id, { operatorNetFee: 3_000, marketId: "mkt_devtest", positionId: `pos_${crypto.randomUUID()}`, houseBotId: null });
    ...
    await onRecruitSettlement(oldRec.id, { operatorNetFee: 3_000, marketId: "mkt_devtest", positionId: `pos_${crypto.randomUUID()}`, houseBotId: null });
    ...
    await onRecruitSettlement(pausedRec.id, { operatorNetFee: 3_000, marketId: "mkt_devtest", positionId: `pos_${crypto.randomUUID()}`, houseBotId: null });
    ...
    await onRecruitSettlement(zRec.id, { operatorNetFee: 3_000, marketId: "mkt_devtest", positionId: `pos_${crypto.randomUUID()}`, houseBotId: null });
    await onRecruitSettlement(zRec.id, { operatorNetFee: -3_000, marketId: "mkt_devtest", positionId: `pos_${crypto.randomUUID()}`, houseBotId: null }); // a negative fee must never credit
    ...
    await onRecruitSettlement(cRec.id, { operatorNetFee: 3_000, marketId: "mkt_devtest", positionId: `pos_${crypto.randomUUID()}`, houseBotId: null });
