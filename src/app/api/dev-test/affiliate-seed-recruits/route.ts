...
function mkUser(displayName: string): StoredUser {
...
export async function POST(req: NextRequest) {
  ...
  if (!db.affiliate.findByCode(code)) return NextResponse.json({ ok: false, error: "unknown code" }, { status: 404 });

  let bound = 0;
  for (let i = 0; i < Math.min(20, Math.max(1, n)); i++) {
    const rec = mkUser(NAMES[i % NAMES.length]);
    const r = bindRecruit({ recruitUserId: rec.id, code });
    if (r.bound) {
      bound++;
      if (activity && i % 2 === 0) onRecruitBet(rec.id, { stake: 50_000, operatorCommissionRate: 0.03 });
    }
  }
