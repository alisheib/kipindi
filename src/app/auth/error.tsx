...
export default function AuthError({
  ...
  const { t } = useT();
  useEffect(() => {
    console.warn("[50pick/auth] error boundary fired", { digest: error.digest });
  }, [error]);

  return (
    <div className="mx-auto flex min-h-[70svh] max-w-[440px] flex-col items-center justify-center px-5 py-12 text-center">
      ...
      <div className="mt-6 flex flex-col items-stretch gap-2 sm:flex-row sm:items-center">
        ...
        <Link href="/auth/login" className="btn btn-ghost btn-md inline-flex items-center justify-center gap-2">
          {t.auth.signInTitle}
          <I.arrowRight s={14} />
        </Link>
      </div>
