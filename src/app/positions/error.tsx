...
export default function PositionsError({
  ...
  const { t } = useT();
  useEffect(() => {
    console.warn("[50pick/positions] error boundary fired", { digest: error.digest });
  }, [error]);
  ...
  return (
    <div className="mx-auto flex min-h-[60svh] max-w-[560px] flex-col items-center justify-center px-5 py-12 text-center">
      ...
      <div className="mt-6 flex flex-col items-stretch gap-2 sm:flex-row sm:items-center">
        ...
        <Link href="/positions" className="btn btn-ghost btn-md inline-flex items-center justify-center gap-2">
          {t.error.backToPositions}
          <I.arrowRight s={14} />
        </Link>
      </div>
    </div>
