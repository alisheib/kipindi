...
> internal docs are unchanged (D19b).~~ ⛔ **Superseded by D20 (Ali, 2026-09-17):** the regulator's private paper was to be the Board draft alone (Commit 6) — ⛔ itself STRUCK 2026-09-20 by owner ruling D21, the Gaming Board needs nothing; no house report, CSV or memo is built, and Commit 7's console only controls the bots. Stakes 50pick places on a holder's account look exactly like the holder's own bets,
> and the holder receives no house-bot notice or email (D19c). The chatbot discloses nothing but never lies (D19d, Commit 6).
>
> **The holder-facing half of `C5-SPEC-EXTRACT.md` (deleted 2026-09-26 as spent; read it at `360935a3`) is struck** (item 12, §5 rows 13–17, §6 step 13, the `seed-house-stake`
> route, every player key). §1 below names every recorded text this file supersedes.
>
> **This file IS an authority for Commit 5**, the way `C4-SPEC.md` is for Commit 4. It superseded
> `C5-SPEC-EXTRACT.md` (deleted 2026-09-26). The plan's own order still applies above it (`04` > `02`/`03` > `PLAN` > `01`),
...
silently edited. `C5-SPEC-EXTRACT.md` and `C4-EMITTERS-EXTRACT.md`, which S2–S14, S30 and F6 cite (S2–S14 by line), were deleted 2026-09-26 as spent build extracts: read them with `git show 360935a3:plans/house-bots/<file>`.
...
| S2 | (deleted 2026-09-26) `C5-SPEC-EXTRACT.md:108` (officer door "same section, `schemaVersion: 2`"), `:107` (player door "add `houseLiquidity`; personal reads exclude house rows; add `houseAuditCount`"), `:159` (§3 item 5 R5 "Both doors, bundle `schemaVersion: 2`", personal reads with `excludeHouseBets`/`excludeHouse`, trigger players "both doors"), `:184` (§4 `test:dsar-secrets` "both doors contain … `houseLiquidity.designations[0].consent.verifiedAt`"), `:231` (§6 step 11), `:292` (§8 guard 15 "the house section must be added to both doors in the same shape") | Rulings 168–170 (both releasable doors house-free, one transaction projection, house-owned audit actions dropped), 236–239 (the internal record), 243 (`test:dsar-secrets` §6–§9 prove absence). Guard 15's premise is gone: no releasable door gains a section, so §5 stays green by construction. |
| S3 | (deleted 2026-09-26) `C5-SPEC-EXTRACT.md:36` W2 "a trigger player's export shows excluded days + countered count only" | PROGRESS W2 as re-opened by D19: no releasable export carries house wording, a trigger player's included; excluded days and the countered count live only in the internal record (rulings 236, 239). |
| S4 | (deleted 2026-09-26) `C5-SPEC-EXTRACT.md:41` §0 "Dependency on Commit 4: steps 8–11 are not built … cannot close visually until step 9's emitter renders" | Resolved: Commit 4 is closed; `notifyAdminsHouseBotAlert` exists (`notification-service.ts:2526`). What Commit 5 owes is the STAFF_EDGE copy row and its renders (rulings 220, 222). |
| S5 | (deleted 2026-09-26) `C5-SPEC-EXTRACT.md:79` §1 "(h) the A17 liquidity label on every holder outcome notice … `houseStake?: boolean` option" as "already landed" | Removed by C4 rulings 143–144. Commit 5 regressions assert the label's absence (ruling 250). |
| S6 | (deleted 2026-09-26) `C5-SPEC-EXTRACT.md:75-76` §1 (d)/(e) `HOUSE_POSITION` / `house_position_no_exit`, (f)/(n) `objHouseStakeOnly` | C4 rulings 146–147: `WINDOW_PASSED`, `exit_window_closed`, `objNotEligible`. |
| S7 | (deleted 2026-09-26) `C5-SPEC-EXTRACT.md:166` item 12, `:136-146` §2 holder-facing table, `:181` §4 "Props assertion for the holder chip surfaces (HB-ACC-14)", `:210-214` §5 rows 13–17, `:217` the seed route, `:233` §6 step 13, `:284` guard 7 (`test:feedback-law` on `sell-button.tsx`), `:286` guard 9 (player chip keys) | D19c. Absence renders and served-HTML assertions (rulings 247–248, 253). |
| S8 | (deleted 2026-09-26) `C5-SPEC-EXTRACT.md:245` P3 recommendation "`houseStakeByMarket` becomes the single producer and `oversight.ts` switches to it" | One requester RULE, not one reader: oversight keeps its own population and recomputes (rulings 178, 187). |
| S9 | (deleted 2026-09-26) `C5-SPEC-EXTRACT.md:248` P6 "the audit takes the zero shape" on a failed read | `houseStake: null`, never a zero shape (ruling 190). |
| S10 | (deleted 2026-09-26) `C5-SPEC-EXTRACT.md:254` P12 (a) "the two DSAR doors exclude" house rows; `:257` P15 two actor words | Rulings 168, 170 (no exclusion; house rows stay as own rows) and 238 (three actor words). |
| S11 | (deleted 2026-09-26) `C5-SPEC-EXTRACT.md:250` P8 "owner-only + `house_bot.exported` for commit 7's per-bot CSV button" | The per-bot CSV **route** ships in Commit 5 behind the amended gate (ruling 213); Commit 7 renders its buttons. |
| S12 | (deleted 2026-09-26) `C5-SPEC-EXTRACT.md:95` and `:162` "`getReportPack` → `getAuditByActionsDurable([...], {category:'ADMIN'})` filtered by `targetId`"; `:179` "the pack state survives 12,000 BET rows" | `getAuditForTargetsDurable` filtered by target AND actions in SQL (ruling 214); the ring-emptying proof (ruling 216). |
| S13 | (deleted 2026-09-26) `C5-SPEC-EXTRACT.md:164` "(pinned by C13's +60-day test)"; `:163` and `:122` "of which 50pick liquidity stakes" | The +60-day href test is Commit 7's (ruling 255, L38). KYC wording "of which house stakes" (ruling 197). |
| S14 | (deleted 2026-09-26) `C5-SPEC-EXTRACT.md:271` P29 "(a) stays in Commit 4 step 9" (the digest split) | Struck by D19c; only F6's email rule remains (ruling 235). |
...
| S30 | `C4-SPEC.md:21`, `:88`, `:203`, `:227`; `C4-EMITTERS-EXTRACT.md` (deleted 2026-09-26) §9 and E13 (the digest house split scheduled in Commit 4 step 9) | Struck by D19c; recorded as a Commit 4 residue closed by ruling 235 (L40). |
...
| F6 | **Recorded texts order a house section into the subject's file** (S1–S3, S15). Built literally they put designations, consent and events under "access: Granted (this document)". | PROGRESS.md:131; C5-SPEC-EXTRACT (deleted 2026-09-26); COMPLIANCE-DECISIONS.md:268 | 168, §1 |
...
| 14 | W20: the repository is public, so this spec is readable (BOOK) | ⛔ **CLOSED BY THE OWNER 2026-09-18 (ruling 469).** The repository stays PUBLIC by Ali's decision and the matter leaves the plan: not a blocker, not a report line, not a Phase 0 measurement, and **never repeated in a handover again**. D19 itself is unchanged. ~~**Accepted residual**: only Ali can change visibility; repeated in the handover.~~ *(Marked 2026-09-18 under ruling 516 (i): `PROGRESS.md`'s W20 row was closed and this twin still read as open, still ordering the handover repeat 469 deletes.)* |
...
227. **The GBT monthly pack's house memo, and W21.**
     ...
     - **W21** (§8): the memo reaches the Gaming Board on the first routine monthly filing after Ali switches ON (D1),
       possibly before his decision on the private Board draft (D19b; struck 2026-09-20 by D21). Default: build as ruled. Alternative: render only after
...
237. **The record's designations, positions and transactions.**
     ...
     - **Consent wording:** the internal record keeps R5's sealed value "password entered by 50pick" (P11 STANDS: it matches
       ...
       ⛔ **ANSWERED 2026-09-18 by replan ruling 503, not by Ali being asked again:** the built code addresses the OFFICER
       throughout, so `:158`/`:201` stand and the sibling that said the holder types it directly carries a dated correction
       (the line is now `:347`; the D19c restatement is `:255`). *(Marked 2026-09-18 under ruling 516 (i).)*
...
252. **Commit 5's closing gates.** Commit 5 closes only when all of these ran and were recorded:
     ...
     - **Source:** D19 draft 174 AMENDED, draft 176 AMENDED (operationalised here); D19 buildNotes (closing gates);
       PROGRESS L15/L18/L20; C5-SPEC-EXTRACT §6 step 15 (extract deleted 2026-09-26).
...
257. **Known gaps recorded, not repaired.**
     ...
     - **W20:** ⛔ **CLOSED BY THE OWNER 2026-09-18 (ruling 469) — the repository stays PUBLIC by Ali's decision, the matter
       leaves the plan, and no session raises, re-measures or repeats it in a handover again.** D19 is unchanged: house bots
       still reach no player and no holder. ~~the repository is public, so this file and every doc describing house bots are
       readable by anyone; only Ali can change it; repeated in the handover.~~ *(Marked 2026-09-18 under ruling 516 (i).)*
...
258. **The docs pass, in the same commit as the code.**
     ...
     - **`docs/COMPLIANCE-DECISIONS.md`:** `:268` (W2) rewritten to PROGRESS W2's re-opened default (no releasable export
       ...
       a lawyer). ~~`:215`'s paraphrase of D5 is FLAGGED to Ali through W24, not edited.~~ ⛔ **Overtaken 2026-09-18 by replan
       ruling 503:** W24 is closed on the measured code, and that paraphrase (now `:347`, with the D19c restatement at `:255`)
       carries a dated correction rather than a flag. *(Marked 2026-09-18 under ruling 516 (i) — this line still told a session
       the file must not be edited, after 503 had edited it.)*
     ...
     - **Marked superseded IN PLACE** (PROGRESS:38 says "mark each", and the five plan documents carry 0 D19 mentions today):
       ...
       `:1230-1236`, `:1329`, `:2102`, and R8's named reader for the pack; `C5-SPEC-EXTRACT.md` (the lines in §1 S2–S14; deleted 2026-09-26);
       `C4-SPEC.md:21`, `:88`, `:203`, `:227` and `C4-EMITTERS-EXTRACT.md` §9/E13 (the digest split; deleted 2026-09-26).
...
| 10 | `test:feedback-law` (`sell-button.tsx:117`), `test:chip-contract`, `test:i18n` | C5-SPEC-EXTRACT guards 7 and 9 (extract deleted 2026-09-26) | NOT touched: no player surface or key changes. A red here is a D19 defect, not a pin to move (253). |
...
| **W24** | **The D5 consent wording contradicts itself across the record of decisions.** `PLAN.md:24` and `COMPLIANCE-DECISIONS.md:158`/`:201` say "the owner types the account's password"; the D19 restatement (`PROGRESS.md:40`; `COMPLIANCE-DECISIONS.md:215`, `:291`) says "the holder types their own password". The system stores only `verifiedAt` and `verifiedById`. (Rulings 202, 237.) | ⛔ **Superseded in part by D20 (Ali, 2026-09-17):** both strings this default governs are struck (R1's consent line, 202; the internal record's sealed value, 237); the question itself (the record of decisions contradicts itself on D5) is not answered by D20. R1 prints only what the row stores: "account password verified in the 50pick console by <officer> at <time>". The internal record keeps R5's sealed "password entered by 50pick". ⛔ **W24 IS NOW CLOSED — by replan ruling 503 (2026-09-18), not by a further wait on Ali.** The built code is the measured fact and is unambiguous: `designation.ts:79` `empty: "Enter their password."`, `:102` "That isn't their current password.", `:109` "Check the holder's password for an owner" — the OFFICER is addressed throughout, and `C7-SPEC.md:1125` specifies an officer-operated field. So D5 is the OWNER typing the HOLDER's account password, verified like a sign-in and never creating a session; `COMPLIANCE-DECISIONS.md:158`/`:201` stand and the sibling that said the holder types it directly **has been corrected in place with a dated note** (now at `:255` and `:347`). The consent itself is unchanged: the holder agrees privately and gives the owner their password. ~~When Ali rules, both strings follow in one commit; `:215` records his ruling and is not silently edited.~~ *(Marked 2026-09-18 under ruling 516 (i): `PROGRESS.md`'s W24 row was closed by 503 and this twin still read as waiting on Ali.)* |
...
| **L29** | ✅ **RETIRED 2026-09-20 (C5-8 phase 1) — fixed on `main` by `893b199d` and present in this tree.** Re-measured, not quoted: `src/app/admin/reports/page.tsx:67` is `const REPORT_FORMATS = ["Excel", "PDF"] as const;`, guarded by `test:report-formats`. The four false labels remain only in the docstring that explains their removal. | — | See `PROGRESS.md` L29 for the measurement. |
...
- ~~W20: the repository is still public, so this spec and the docs describing house bots are readable by anyone.~~ ⛔ **CLOSED BY THE OWNER 2026-09-18 (ruling 469): the repository stays PUBLIC by his decision, the matter leaves the plan, and it is never raised, re-measured or handed over again.** *(Marked 2026-09-18 under ruling 516 (i) — it was still listed here as something waiting on Ali.)*
- W21–W24, the two W2 addenda, and X9–X14 wait on him, each with its default. ⛔ **Superseded in part by D20 (Ali, 2026-09-17):** W21, W22, both W2 addenda, X9, X10, X11 and X14 are moot (their surfaces are struck); W23 and X12 stand; X13 stands only as its row now says. ⛔ **And W24 is CLOSED (replan ruling 503, 2026-09-18) — it no longer waits on him**; its row above carries the measured reason and the dated correction it produced in `docs/COMPLIANCE-DECISIONS.md`. *(Marked 2026-09-18 under ruling 516 (i).)*
