...
132. **Two holder notices are missing and land with step 9's emitters:** `role_changed` (rows 12–13, "role changed" notice) and `erasure_request` (row 14, "stopped while we handle your request"). Each gets its `HouseBotOwnerNotice` kind, copy in three languages marked for native review, a cert-c1/c3 registry row and a rendered bell.
