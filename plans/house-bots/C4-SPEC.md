...
141. **One copy table for every alert code, and a row for a code nobody mapped.** `src/lib/house-bot/alert-copy.ts` holds every code the engine and the holder hook raise — derived from the call sites (`alertOnce`, `alerts.once`, `alerts.security`, and the hook's own claims), not from a document — in English, Swahili and Chinese, with the severity and the link. A code with no row still reads as a sentence and names itself, because the alternative is a bare machine token in a human's inbox, which is how UNMAPPED_REFUSAL once read. The module is pure: the house module law admits a small, deliberate set of value imports and the platform's money formatter is not one of them, so the caller injects it (`money`) and formatting shillings keeps one home.
142. **A caller's verb or sentence is never embedded in a translated body** (found by rendering the inbox, seventh session). The first render of `/notifications` in Swahili showed `Boti "Dar liquidity A": Player #A3F2K8 withdrew TZS 50,000` and a roster row whose Swahili body carried an English diff — because `notifyAdminsHouseBotMoneyEvent` took the verb as free text and `notifyAdminsHouseBotRoster` took the whole sentence. Both now take a CODE from a closed list plus structured parts, and each language's sentence is built in `alert-copy.ts` (`MONEY_EVENT_WORD`, `ROSTER_SENTENCE`). Where the detail IS an engine-composed English sentence — the staff-chosen side rule, which the activity feed owns — the English body keeps it and the other two carry the localised frame and point at the feed, rather than presenting English as a translation. ⚠️ This is what a render catches and a test does not: `test:cert-c3` §2 only asks that Swahili and Chinese are present and that no placeholder survives, and every one of those rows passed it.: W17 stays out of the money group; W4 is 7 years; W2 and W5-W16 stand as written; X7 is built (ruling 136) while X3 and X8 are not; L10 is fixed here (ruling 137). No decision in this file waits on Ali.
