...
| P12 | ~~Personal reads excluding house~~ ⛔ **SUPERSEDED by C5-SPEC §1 S10 (rulings 168, 170, owner ruling D19): neither releasable door excludes house rows — a holder's house stakes stay in both as their own rows, through one allowlist, with no house key, id or word. The text below is the record of what was proposed.** | (a) the two DSAR doors exclude, the officer player page reads `db.txn.findByUser` directly and stays byte-identical; (b) exclude everywhere `exportUserData` is called | **(a)** — R5 04:1226-1228 says the officer page output is unchanged; a byte-identical KPI assertion for a non-holder proves it |
...
| P15 | `events[].actor` ⛔ **SUPERSEDED by C5-SPEC §1 S10 (ruling 238: three actor words, in the owner-only internal record only).** | (a) `"50pick owner"` when `actorId != null`, `"system"` when null; (b) always "50pick owner" | **(a)** — R5's shape lists both; an engine auto-pause is not a person |
