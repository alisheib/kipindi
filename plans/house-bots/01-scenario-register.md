...
- **⚠️ 2026-09-26 (delegated, `docs/COMPLIANCE-DECISIONS.md` entry `2026-09-26 · D1 READ AS THE OWNER ROLE`):** D1 is enforced as the ADMIN role — the platform's owner tier — so a second ADMIN switching the desk ON is within D1, audited with its actor and alerted to every admin. ⛔ The alerts this scenario planned on designation, verify, Start, rules and limits saves are still NOT sent (only pause, remove and the switch); that build remains open.
