...
//         visualExtra?, carryMisses? }
// Committed from the tenth session's c5-step-fast (2026-09-17); the eleventh session made the scratchpad and the next
// ruling number arguments and added ruling 259's console-gate law to the standing rules.
const A = args
const SCR = A.scratch || 'C:/Users/asheib/AppData/Local/Temp/claude/C--Users-asheib/c9828811-b712-42cd-8f26-061b82a24511/scratchpad'
const NEXT = A.nextRuling || 260
const RULES = `
STANDING RULES (non-negotiable):
- Machine OMEGA-COMPILE01 (Windows 10, 32 GB, 12 CPUs). Repo worktree F:/kipindi-house-bots, branch house-bots (confirm with git branch --show-current before every commit). Push ONLY with: git push origin house-bots. NEVER push main, never put main in a refspec, never rebase, never force, never skip hooks. Never touch production (no railway, no production database, not even a read). Never turn the house-bot master switch on outside a scratch database. Never edit, stage, check out or build in F:/kipindi-main (another session's checkout). F:/kipindi-old-build is a clean origin/main worktree with its own node_modules: run a suite there only to compare a red, and leave it clean.
- You are the only writer in F:/kipindi-house-bots while you run (agents of one workflow run one at a time). Never run npm ci, npm install, npm rebuild or npx prisma generate in F:/kipindi-house-bots (if a step truly needs one, stop and report it in openIssues). Your scratch Postgres is 127.0.0.1:5433 (scripts/db-scratch.mts); never use ports 5434-5450.
...
- Pace (Ali, 2026-09-17: no unnecessary work): no mutation runs in this workflow (they are batched at the commit close); keep the review-fix loop tight; while iterating, run only the cases you changed through the memory child directly (npx tsx scripts/lib/<cases>.mts with the memory env the runner passes) or a section filter, and run each full two-store suite ONCE when the step's code is complete (and once more only for suites a later fix touched) — never a full two-store run after every small slice; do not run a test whose result you are already certain of (e.g. a suite no changed file can reach). Every re-run the spec names for the step, every case for new behaviour and every render is still required; when you skip a run as certain, say which and why in notMeasured.
...
For EACH carried MISSED mutation: strengthen the case in F:/kipindi-house-bots so the mutation would go red (see it red by applying the mutation in the main worktree, running the case, and restoring the original bytes with git diff --quiet on that file before anything else), or show precisely why the mutation is equivalent.
