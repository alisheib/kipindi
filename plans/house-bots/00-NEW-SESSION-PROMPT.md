...
| REL-5 | `dpl=` SHA = merge (`curl -s -D - -o /dev/null https://50pick.tz/ \| grep -io 'dpl=[0-9a-f]*' \| head -1 \| cut -d= -f2`); `/api/health` `houseBots.schemaReady=true` + `leadership.lifecycle.isMe=true`; after the 90 s boot grace, status OFF / 0 bots / 0 marked rows / engine enabled / fresh beats; PLAN §12 + §15 phase F; at +10 min 0 marked rows and 0 HOUSE_BOT notifications. |
