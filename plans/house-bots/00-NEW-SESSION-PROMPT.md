...
4. `01-scenario-register.md`: **281 scenarios** — CA 45 · CRA 36 · ENG 42 · FS 34 · HB-ACC 45 · HB-LC 39 · TGT 40 (re-derive with `npm run test:scenario-coverage`, never quote; the 241 this line used to carry is the non-`TGT` count and its old per-prefix breakdown summed to 298 and never described the file). By REL-0, each id must be covered by a test assertion (PROGRESS "Scenario coverage gate"), and **`npm run test:scenario-coverage` measures all 281 and prints the figure as a BRACKET** — 24/281 = 8.5% proven by name, 254/281 = 90.4% carried by an owning suite that exists and runs, 18 rows with no test at all, every one named; 0 unresolved. ⛔ **The low half is not the story:** the suites are indexed by amendment letter and ruling number, not by register id (188 of the 281 sit on a `Merged:` line in `04-amendments.md`), so grepping `scripts/` for an id searches the wrong index. ⛔ **And four rows read BACKWARDS against the shipped product** (`HB-LC-05`, `FS-31`, `HB-ACC-42`, `HB-ACC-07`) — reconcile before annotating, or a withdrawn requirement goes into a test name. ⛔ **Superseded in part by D20 (Ali, 2026-09-17):** an id D20 strikes counts only in the form its ⛔ D20 line in the register gives (`C5-D20-REPLAN.md` §2, ruling 250).
