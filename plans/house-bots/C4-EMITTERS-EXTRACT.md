...
| `ENGINE_ERRORS` | `outcomes.ts:310` → `engineSwitchOff` → `alerts.security` | — | `{ streak }` | A24 | `/admin/desk` |
...
| `LIMITS_FROM_FUTURE` | `planner.ts:380` | `ALERT_KEY.rulesFuture("global", v)` | `{ version }` | F4 | `/admin/desk?tab=limits` |
...
| `PRODUCT_DENIED` | `trigger.ts:330` | `ALERT_KEY.productDenied(value)` + EAT day | `{ productLine }` | ruling 90 | `/admin/desk?tab=limits` |
