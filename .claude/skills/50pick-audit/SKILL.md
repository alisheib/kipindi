...
4. ⛔ **PROVE THE BACKUP STILL WORKS ON THE NEW SCHEMA — `npm run verify:backup-schema`.** Boots a throwaway cluster, applies **every** migration, and runs the **real** `db:backup` against it. 🔴 **THIS STEP IS NOT OPTIONAL AND IT IS NOT COVERED BY ANY OTHER SUITE.** `db:backup` generates its schema DDL from the **LIVE DATABASE** (`prisma migrate diff --from-empty --to-url`), so a migration is precisely the kind of change that can break recovery — and nothing else in this repo ever runs the dump against a schema before it ships. On 2026-09-19 the house-bot migration did exactly that: the nightly failed **three consecutive nights** (runs 57–59), the platform went **80 hours with no verified backup**, and the only thing that said so was a 04:15 email. See `docs/BACKUP-RUNBOOK.md` → "THE THREE NIGHTS". ⚠️ If a parallel session holds port 5433, give yours its own: `KP_SCRATCH_PORT=5443 npm run verify:backup-schema`.
5. Commit. **Production gets it via the normal deploy** (`start` = `prisma migrate deploy && next start`, run by Railway on push) — not by you.
