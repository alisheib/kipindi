...
const Icon = {
  ...
  play: (p) => <Svg {...p}><circle cx="12" cy="12" r="9" /><path d="M10 8.5l5.5 3.5L10 15.5z" fill="currentColor" stroke="none" /></Svg>,
  ...
  /* ---- extended set — substitutes for the old kit's lucide icons (full app coverage) ---- */
  warning: (p) => <Svg {...p}><path d="M12 4.2 21 19.5H3z" /><path d="M12 10v4.5M12 17.4h.01" /></Svg>,
  alertCircle: (p) => <Svg {...p}><circle cx="12" cy="12" r="9" /><path d="M12 8v5M12 16h.01" /></Svg>,
  eye: (p) => <Svg {...p}><path d="M2.5 12S6 5.5 12 5.5 21.5 12 21.5 12 18 18.5 12 18.5 2.5 12 2.5 12z" /><circle cx="12" cy="12" r="3" /></Svg>,
  eyeOff: (p) => <Svg {...p}><path d="M3 3l18 18" /><path d="M10.6 5.2A10.6 10.6 0 0 1 12 5.1c6 0 9.5 6.9 9.5 6.9a17.3 17.3 0 0 1-3.1 3.9M6.5 6.6A17.2 17.2 0 0 0 2.5 12S6 18.9 12 18.9a10.5 10.5 0 0 0 4.2-.9" /><path d="M9.9 9.9a3 3 0 0 0 4.2 4.2" /></Svg>,
  edit: (p) => <Svg {...p}><path d="M4 20h4L18.5 9.5a2.12 2.12 0 0 0-3-3L5 17z" /><path d="M13.5 7.5l3 3" /></Svg>,
  camera: (p) => <Svg {...p}><path d="M4 8h3l1.5-2.2h7L17 8h3a1 1 0 0 1 1 1v9a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V9a1 1 0 0 1 1-1z" /><circle cx="12" cy="13" r="3.2" /></Svg>,
  trash: (p) => <Svg {...p}><path d="M4 7h16M9 7V5a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2M6.5 7l.9 13a1 1 0 0 0 1 .9h7.2a1 1 0 0 0 1-.9L18.5 7M10 11v6M14 11v6" /></Svg>,
  flag: (p) => <Svg {...p}><path d="M5.5 21V4M5.5 4.5h11l-2.2 3.2 2.2 3.2h-11" /></Svg>,
  ...
  checkCircle: (p) => <Svg {...p}><circle cx="12" cy="12" r="9" /><path d="M8.4 12.4l2.5 2.5 4.7-5.2" /></Svg>,
  bellRing: (p) => <Svg {...p}><path d="M7 10a5 5 0 0 1 10 0c0 4 1.5 5 2 6H5c.5-1 2-2 2-6z" /><path d="M10.5 20a1.7 1.7 0 0 0 3 0" /><path d="M3.6 6.4A6 6 0 0 1 6 3M20.4 6.4A6 6 0 0 0 18 3" /></Svg>,
  bellOff: (p) => <Svg {...p}><path d="M8.6 4.2A5 5 0 0 1 17 8c0 2.6.6 4 1.2 5M16.5 16.5H5c.5-1 2-2 2-6 0-.4 0-.7.05-1M10.5 20a1.7 1.7 0 0 0 3 0M3 3l18 18" /></Svg>,
  trendingUp: (p) => <Svg {...p}><path d="M3 17l6-6 4 4 8-8" /><path d="M21 7v5M21 7h-5" /></Svg>,
};
...
/* old-kit (lucide) name aliases → ensure developer references resolve */
Icon.alert = Icon.warning; Icon.alertTriangle = Icon.warning; Icon.xCircle = Icon.void; Icon.message = Icon.comment; Icon.pencil = Icon.edit; Icon.eyeOpen = Icon.eye; Icon.checkCircle2 = Icon.checkCircle; Icon.cloudSun = Icon.weather; Icon.bitcoin = Icon.crypto; Icon.music = Icon.entertainment; Icon.building = Icon.politics; Icon.camera2 = Icon.camera;
...
/* ---------- shared tier system (single source of truth — used by leaderboard + trade) ---------- */
const TIERC = { bronze: 'oklch(62% 0.10 60)', silver: 'oklch(78% 0.02 268)', gold: 'var(--gold-400)', diamond: 'var(--brand-400)' };
const TIER_GLYPH = { bronze: 'shield', silver: 'star', gold: 'crown', diamond: 'sparkle' };
...
function Chip({ tone = 'cat', icon, children }) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, height: status ? 23 : 21, padding: status ? '0 9px' : '0 8px', borderRadius: 'var(--r-pill)',
      ...
      background: bg, border: `1px solid ${bd}`, animation: tone === 'hot' ? 'hotPulse 1.8s ease-in-out infinite' : undefined }}>
      {tone === 'live' && <LiveDot />}{icon}{children}
    </span>
  );
}

/* ---------- ConvictionBar — THE single canonical YES/NO bar (alias: ProbabilityBar) ----------
   Split green-YES / red-NO fill with a gold boundary needle. One component, four variants:
     · split (default) — YES left, NO right, gold needle at the boundary
     · segmented       — two detached pills with a 4px gap
     · minimal         — single emerald fill, no needle (dense lists)
     · resolved        — split + gold shimmer sweep (settled markets)
   Height: pass `h` for an exact pixel height, or `size` micro(12)/large(24). `large` shows inline labels. */
function ConvictionBar({ yes = 50, h, size = 'micro', variant = 'split', reveal = true, animate, resolved = false, needle = true }) {
  const big = size === 'large';
  const H = h != null ? h : (big ? 24 : 12);
  const doAnim = animate != null ? animate : reveal;
  const [w, setW] = kS(doAnim ? 50 : yes);
  kE(() => { const t = setTimeout(() => setW(yes), 70); return () => clearTimeout(t); }, [yes]);
  const no = 100 - yes;
  const isResolved = resolved || variant === 'resolved';
  const yesGrad = 'linear-gradient(90deg, var(--yes-700), var(--yes-500))';
  const noGrad = 'linear-gradient(90deg, var(--no-500), var(--no-700))';
  const ease = 'width .5s var(--ease-stage), left .5s var(--ease-stage)';
  const aria = { role: 'progressbar', 'aria-valuenow': yes, 'aria-valuemin': 0, 'aria-valuemax': 100, 'aria-label': `YES ${yes}% · NO ${no}%` };
  const Shimmer = () => <span style={{ position: 'absolute', inset: 0, background: 'linear-gradient(110deg, transparent 35%, oklch(90% 0.1 84 / 0.5) 50%, transparent 65%)', backgroundSize: '200% 100%', animation: 'goldShimmer 1.6s linear infinite' }} />;

  if (variant === 'minimal') return (
    <div {...aria} style={{ height: H, borderRadius: 'var(--r-pill)', background: 'var(--bg-inset)', overflow: 'hidden', border: '1px solid var(--border)' }}>
      <div style={{ width: `${w}%`, height: '100%', background: yesGrad, transition: ease }} />
    </div>
  );

  if (variant === 'segmented') return (
    <div {...aria} style={{ display: 'flex', gap: 4, height: H }}>
      <div style={{ width: `${w}%`, background: yesGrad, borderRadius: 'var(--r-sm)', display: 'flex', alignItems: 'center', paddingLeft: 8, ...mono, fontSize: 10, color: '#fff', fontWeight: 700, transition: ease, boxShadow: 'inset 0 1px 0 oklch(95% 0.04 152 / 0.3)' }}>{big ? yes : ''}</div>
      <div style={{ flex: 1, background: noGrad, borderRadius: 'var(--r-sm)', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', paddingRight: 8, ...mono, fontSize: 10, color: '#fff', fontWeight: 700, boxShadow: 'inset 0 1px 0 oklch(92% 0.05 25 / 0.25)' }}>{big ? no : ''}</div>
    </div>
  );

  // split (default) + optional resolved shimmer + gold boundary needle
  return (
    <div {...aria} style={{ position: 'relative', height: H }}>
      <div style={{ display: 'flex', height: H, borderRadius: 'var(--r-pill)', overflow: 'hidden', background: 'var(--bg-inset)', border: '1px solid var(--border)', boxShadow: 'inset 0 1px 2px oklch(8% 0.05 264 / 0.5)' }}>
        {/* YES — emerald, fills from the left */}
        <div style={{ width: `${w}%`, height: '100%', background: yesGrad, transition: ease, position: 'relative', overflow: 'hidden', boxShadow: 'inset 0 1px 0 oklch(95% 0.04 152 / 0.32)', display: 'flex', alignItems: 'center', paddingLeft: big ? 10 : 0 }}>
          {big && <span style={{ ...mono, fontSize: 11, fontWeight: 700, color: '#fff' }}>YES {yes}</span>}
          {isResolved && <Shimmer />}
        </div>
        {/* NO — rose, fills the remainder on the right */}
        <div style={{ flex: 1, height: '100%', background: noGrad, position: 'relative', overflow: 'hidden', boxShadow: 'inset 0 1px 0 oklch(92% 0.05 25 / 0.25)', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', paddingRight: big ? 10 : 0 }}>
          {big && <span style={{ ...mono, fontSize: 11, fontWeight: 700, color: '#fff' }}>{no} NO</span>}
          {isResolved && <Shimmer />}
        </div>
      </div>
      {/* gold needle marks the YES/NO split */}
      {needle && <div style={{ position: 'absolute', left: `${w}%`, top: -3, bottom: -3, width: 3, transform: 'translateX(-50%)', background: 'linear-gradient(180deg, var(--gold-300), var(--gold-600))', borderRadius: 'var(--r-xs, 2px)', boxShadow: '0 0 0 1px var(--bg-elevated)', transition: ease }} />}
    </div>
  );
}
const ProbabilityBar = ConvictionBar; // single source of truth — both names resolve to the same component
  ...
  return (
    <button aria-label={`${yes ? 'YES' : 'NO'} at ${price}`}
      ...
      style={{ height: H, width: '100%', border: 'none', cursor: 'pointer',
        ...
        borderRadius: 'var(--r-sm)', color: '#fff', background: c.base,
...
function Btn({ variant = 'primary', size = 'md', children, leading, trailing, full, state, live, style }) {
  ...
  return (
    <button disabled={disabled || loading} onMouseEnter={() => setH(true)} onMouseLeave={() => { setH(false); setP(false); }} onMouseDown={() => setP(true)} onMouseUp={() => setP(false)}
      style={{ height: H, width: full ? '100%' : undefined, padding: `0 ${size === 'xl' ? 24 : 18}px`, borderRadius: 'var(--r-sm)', border, background: bg, color, filter,
...
/* ---------- Balance privacy — ONE global toggle governs every <Cash> across all boards ----------
   Tap any CashEye to mask/unmask all personal balances at once (banking-app pattern). State lives on
   window + a custom event so independent React roots on the canvas stay in sync. */
const CASH_EVT = 'cash-privacy';
if (window.__cashHidden === undefined) window.__cashHidden = false;
function setCashHidden(v) { window.__cashHidden = v; window.dispatchEvent(new CustomEvent(CASH_EVT, { detail: v })); }
function useCashHidden() {
  const [h, setH] = kS(window.__cashHidden);
  kE(() => { const fn = (e) => setH(e.detail); window.addEventListener(CASH_EVT, fn); return () => window.removeEventListener(CASH_EVT, fn); }, []);
  return h;
}
/* Masks the numeric part of an amount, preserving the sign + currency prefix so layout rhythm holds. */
function Cash({ children, dots = 5, style, className }) {
  const hidden = useCashHidden();
  const raw = String(children);
  let out = raw;
  if (hidden) { const m = raw.match(/^([+\-−]?\s*(?:TZS|USD|KES|\$)?\s*)/i); out = (m ? m[0] : '') + '•'.repeat(dots); }
  return <span className={className} style={style}>{out}</span>;
}
/* Small eye toggle to sit beside a balance — inherits the kit's inset-chip styling. */
function CashEye({ size = 15, bare = false, style }) {
  const hidden = useCashHidden();
  const [hv, setHv] = kS(false);
  const box = bare
    ? { width: 20, height: 20, border: 'none', background: 'none', borderRadius: 'var(--r-xs, 4px)' }
    : { width: 28, height: 28, borderRadius: 'var(--r-sm)', border: '1px solid', borderColor: hv ? 'var(--border-strong)' : 'var(--border)', background: 'var(--bg-inset)' };
  return <button onClick={() => setCashHidden(!window.__cashHidden)} onMouseEnter={() => setHv(true)} onMouseLeave={() => setHv(false)}
    aria-label={hidden ? 'Show balance' : 'Hide balance'} aria-pressed={hidden}
    style={{ display: 'inline-grid', placeItems: 'center', flexShrink: 0, color: hv ? 'var(--text)' : 'var(--text-muted)', cursor: 'pointer', transition: 'color .15s var(--ease-micro), border-color .15s', ...box, ...style }}>
    {(hidden ? Icon.eyeOff : Icon.eye)({ s: size })}
  </button>;
}

Object.assign(window, { Icon, Svg, mono, disp, LiveDot, Chip, ConvictionBar, ProbabilityBar, SideButton, SidePair, Btn, RollNum, MovePill, categoryGlyph, TIERC, TIER_GLYPH, Cash, CashEye, useCashHidden, setCashHidden });
