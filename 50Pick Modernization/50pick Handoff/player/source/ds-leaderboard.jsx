  ...
  return <div className="kit-screen body" style={{ width: '100%', minHeight: '100%', background: 'var(--bg)', color: 'var(--text)' }}>
    ...
    <div style={{ padding: '24px 32px 36px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 1.5fr', gap: 28, alignItems: 'start' }}>
        ...
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: '30px 40px 1fr 86px 60px 64px', gap: 10, padding: '0 14px 9px', ...mono, fontSize: 9.5, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-faint)' }}>
            <span>#</span><span>7d</span><span>Predictor</span><span>Trend</span><span style={{ textAlign: 'right' }}>Acc.</span><span style={{ textAlign: 'right' }}>Points</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 560, overflow: 'auto' }}>
            {LB.map((p) => <div key={p.handle} style={{ display: 'grid', gridTemplateColumns: '30px 40px 1fr 86px 60px 64px', gap: 10, alignItems: 'center', padding: '9px 14px', borderRadius: 'var(--r-md)',
