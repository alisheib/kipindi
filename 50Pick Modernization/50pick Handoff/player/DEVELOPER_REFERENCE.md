...
- `ConvictionDial` — 270° radial gauge; green arc + gold needle + mono value.
- `ConvictionBar` — read-only fill + thin gold needle (in cards).

### Probability bar — `ds-atoms2.jsx → ProbabilityBar`
4 variants: `split` (default, YES left/NO right, 2px seam) · `segmented` · `minimal` (dense lists) · `resolved` (gold shimmer). Sizes micro 12 / large 24. Include `role="progressbar"`, `aria-valuenow`, `aria-label`.
...
kit50.jsx          Icon set, Chip, ConvictionBar, SideButton/SidePair, Btn, RollNum, MovePill, LiveDot
...
ds-atoms2.jsx      loaders, Input/OtpBoxes, Avatar/TierBadge, ProgressBar/Stepped, Tooltip, ProbabilityBar variants, Skeleton
