  ...
  return <div className="kit-screen body" style={{ width: '100%', minHeight: '100%', background: 'var(--bg)', color: 'var(--text)', display: 'flex' }}>
    ...
    <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ height: 56, display: 'flex', alignItems: 'center', gap: 16, padding: '0 24px', borderBottom: '1px solid var(--border)', background: 'var(--panel)' }}>
        ...
        <span style={{ color: 'var(--text-muted)', position: 'relative' }}>{Icon.bell({ s: 19 })}<span style={{ position: 'absolute', top: -1, right: -1, width: 7, height: 7, borderRadius: 9, background: 'var(--no-400)' }} /></span>
      </div>
      <div style={{ padding: 24, overflow: 'auto' }}>
        ...
        <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 'var(--r-lg)', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '2.6fr 1fr 0.9fr 1fr 1fr 0.9fr 0.6fr', gap: 12, padding: '10px 18px', ...mono, fontSize: 9.5, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-faint)', borderBottom: '1px solid var(--border)' }}>
            <span>Question</span><span>Category</span><span>Status</span><span style={{ textAlign: 'right' }}>Pool</span><span style={{ textAlign: 'right' }}>Predictors</span><span style={{ textAlign: 'right' }}>Closes</span><span></span>
          </div>
          {ROWS.map((r, i) => { const [q, cat, st, pool, preds, closes] = r; return <div key={i} style={{ display: 'grid', gridTemplateColumns: '2.6fr 1fr 0.9fr 1fr 1fr 0.9fr 0.6fr', gap: 12, padding: '12px 18px', alignItems: 'center', borderTop: i ? '1px solid oklch(30% 0.08 268 / 0.5)' : 'none', fontSize: 13 }}>
            <span style={{ fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{q}</span>
            <span style={{ color: 'var(--text-muted)' }}>{cat}</span>
            <span>{st === 'live' ? <Chip tone="live">Live</Chip> : st === 'review' ? <Chip tone="resolved">Review</Chip> : <Chip tone="soon">Soon</Chip>}</span>
            <span className="mono" style={{ textAlign: 'right', color: 'var(--text-muted)' }}>TZS {pool}</span>
            <span className="mono" style={{ textAlign: 'right', color: 'var(--text-muted)' }}>{preds.toLocaleString()}</span>
            <span className="mono" style={{ textAlign: 'right', color: 'var(--text-subtle)' }}>{closes}</span>
            <span style={{ textAlign: 'right', color: 'var(--text-subtle)', cursor: 'pointer' }}>{Icon.ext({ s: 15 })}</span>
          </div>; })}
        </div>
      </div>
    </div>
