...
function Avatar({ initials = 'JK', tier, size = 40, seed }) {
  ...
  return <span style={{ position: 'relative', display: 'inline-flex', width: size, height: size, flexShrink: 0 }}>
    ...
    {tier && <span style={{ position: 'absolute', inset: 0, borderRadius: '50%', border: `1.5px solid ${ring}`, boxShadow: 'inset 0 0 0 1px oklch(10% 0.05 268 / 0.3)' }} />}
  </span>;
}
...
/* ============================================================
   PROBABILITY BAR — 4 variants (kit spec), no needle option
   ============================================================ */
function ProbabilityBar({ yes = 64, variant = 'split', size = 'micro', animate = true }) {
  const h = size === 'large' ? 24 : 12;
  const big = size === 'large';
  const [w, setW] = aS(animate ? 50 : yes);
  aE(() => { const t = setTimeout(() => setW(yes), 80); return () => clearTimeout(t); }, [yes]);
  const yesGrad = 'linear-gradient(90deg, var(--yes-700,oklch(48% 0.14 152)), var(--yes-500))';
  const noGrad = 'linear-gradient(90deg, var(--no-500), var(--no-700,oklch(46% 0.17 25)))';
  const ease = 'width .7s var(--ease-stage), left .7s var(--ease-stage)';
  if (variant === 'minimal') return <div style={{ height: h, borderRadius: 999, background: 'var(--bg-inset)', overflow: 'hidden', border: '1px solid var(--border)' }}><div style={{ width: `${w}%`, height: '100%', background: yesGrad, boxShadow: '0 0 8px var(--yes-500)', transition: ease }} /></div>;
  if (variant === 'segmented') return <div style={{ display: 'flex', gap: 4, height: h }}>
    <div style={{ width: `${w}%`, background: yesGrad, borderRadius: 6, display: 'flex', alignItems: 'center', paddingLeft: 8, ...mono, fontSize: 10, color: '#fff', fontWeight: 700, transition: ease, boxShadow: 'inset 0 1px 0 oklch(95% 0.04 152 / 0.3)' }}>{big ? yes : ''}</div>
    <div style={{ flex: 1, background: noGrad, borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', paddingRight: 8, ...mono, fontSize: 10, color: '#fff', fontWeight: 700, boxShadow: 'inset 0 1px 0 oklch(92% 0.05 25 / 0.25)' }}>{big ? 100 - yes : ''}</div>
  </div>;
  // split (default) + resolved shimmer + glowing gold boundary needle
  return <div style={{ position: 'relative', display: 'flex', height: h, borderRadius: 999, overflow: 'visible', background: 'var(--bg-inset)', border: '1px solid var(--border)', boxShadow: 'inset 0 1px 2px oklch(8% 0.05 264 / 0.5)' }}>
    <div style={{ position: 'absolute', inset: 0, borderRadius: 999, overflow: 'hidden', display: 'flex' }}>
      <div style={{ width: `${w}%`, background: yesGrad, position: 'relative', overflow: 'hidden', boxShadow: 'inset 0 1px 0 oklch(95% 0.04 152 / 0.32)', transition: ease, display: 'flex', alignItems: 'center', paddingLeft: big ? 10 : 0 }}>
        {big && <span style={{ ...mono, fontSize: 11, fontWeight: 700, color: '#fff' }}>YES {yes}</span>}
        {variant === 'resolved' && <span style={{ position: 'absolute', inset: 0, background: 'linear-gradient(110deg, transparent 35%, oklch(90% 0.1 84 / 0.5) 50%, transparent 65%)', backgroundSize: '200% 100%', animation: 'goldShimmer 1.6s linear infinite' }} />}
      </div>
      <div style={{ flex: 1, background: noGrad, boxShadow: 'inset 0 1px 0 oklch(92% 0.05 25 / 0.25)', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', paddingRight: big ? 10 : 0 }}>
        {big && <span style={{ ...mono, fontSize: 11, fontWeight: 700, color: '#fff' }}>{100 - yes} NO</span>}
      </div>
    </div>
    {/* glowing gold boundary needle — same vocabulary as the conviction dial */}
    <span style={{ position: 'absolute', top: -2, bottom: -2, left: `${w}%`, width: 2, transform: 'translateX(-50%)', borderRadius: 2, background: 'var(--gold-300)', transition: ease }} />
  </div>;
}
