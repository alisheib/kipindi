...
function PositionCard({ p }) {
  ...
  return <div style={{ background: 'var(--bg-elevated)', border: `1px solid ${won ? 'oklch(78% 0.13 80 / 0.4)' : 'var(--border)'}`, borderRadius: 'var(--r-lg)', padding: 16,
    ...
    <div style={{ marginBottom: 12 }}><ConvictionBar yes={p.yes} h={8} resolved={won || lost} /></div>
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 12 }}>
      {[['Stake', `TZS ${p.stake.toLocaleString()}`, 'var(--text)'], [lost ? 'Final' : 'Value', lost ? 'TZS 0' : `TZS ${p.value.toLocaleString()}`, 'var(--text)'], ['P&L', `${p.pnl >= 0 ? '+' : '−'}TZS ${Math.abs(p.pnl).toLocaleString()}`, pnlC]].map(([k, v, c]) => <div key={k}><div style={{ ...mono, fontSize: 9, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-faint)' }}>{k}</div><Cash style={{ ...mono, fontSize: 13.5, fontWeight: 700, color: c, display: 'block' }}>{v}</Cash></div>)}
    </div>
