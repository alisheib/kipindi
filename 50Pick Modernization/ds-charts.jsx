...
function ConvictionDial({ value = 64, size = 132 }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      <text x={cx} y={cy - 4} textAnchor="middle" style={{ ...mono, fontSize: 26, fontWeight: 700, fill: 'var(--text)' }}>{value}<tspan style={{ fontSize: 14, fill: 'var(--text-subtle)' }}>%</tspan></text>
      <text x={cx} y={cy + 15} textAnchor="middle" style={{ ...mono, fontSize: 11, fontWeight: 600, letterSpacing: '0.04em', fill: 'var(--yes-400)' }}>YES · {(value / 100).toFixed(2)}</text>
    </svg>
  );
}
