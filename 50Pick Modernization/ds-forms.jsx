...
/* ---- controls ---- */
function Switch({ on: o = true, label = 'toggle' }) { const [on, setOn] = frS(o); return <button role="switch" aria-checked={on} aria-label={label} onClick={() => setOn(!on)} style={{ width: 44, height: 26, borderRadius: 999, border: 'none', cursor: 'pointer', background: on ? 'var(--accent-500)' : 'var(--bg-inset)', position: 'relative', transition: 'background .18s var(--ease-micro)' }}><span style={{ position: 'absolute', top: 3, left: on ? 21 : 3, width: 20, height: 20, borderRadius: 999, background: '#fff', transition: 'left .18s var(--ease-micro)', boxShadow: '0 1px 3px oklch(10% 0.05 264 / 0.5)' }} /></button>; }
function Checkbox({ on: o = true, label }) { const [on, setOn] = frS(o); return <label role="checkbox" aria-checked={on} aria-label={label} tabIndex={0} onKeyDown={(e) => { if (e.key === ' ' || e.key === 'Enter') { e.preventDefault(); setOn(!on); } }} style={{ display: 'inline-flex', alignItems: 'center', gap: 9, cursor: 'pointer', fontSize: 13.5, color: 'var(--text)' }} onClick={() => setOn(!on)}><span style={{ width: 19, height: 19, borderRadius: 5, border: `1px solid ${on ? 'var(--accent-500)' : 'var(--border-strong)'}`, background: on ? 'var(--accent-500)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all .12s' }}>{on && <Icon.check s={13} sw={3} style={{ color: '#06130d' }} />}</span>{label}</label>; }
function Radio({ checked, label }) { return <label role="radio" aria-checked={!!checked} aria-label={label} tabIndex={0} style={{ display: 'inline-flex', alignItems: 'center', gap: 9, cursor: 'pointer', fontSize: 13.5, color: 'var(--text)' }}><span style={{ width: 19, height: 19, borderRadius: 999, border: `1px solid ${checked ? 'var(--accent-500)' : 'var(--border-strong)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{checked && <span style={{ width: 9, height: 9, borderRadius: 999, background: 'var(--accent-500)' }} />}</span>{label}</label>; }
...
function FormsBoard() {
  return <ScreenF>
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}><Stepper />{['1k', '5k', '10k', '25k', '50k'].map((c, i) => <span key={c} role="button" tabIndex={0} aria-pressed={i === 3} aria-label={`Stake ${c}`} style={{ ...mono, fontSize: 13, fontWeight: 600, minHeight: 40, display: 'inline-flex', alignItems: 'center', padding: '0 16px', borderRadius: 'var(--r-pill)', cursor: 'pointer', color: i === 3 ? '#fff' : 'var(--text-muted)', background: i === 3 ? 'var(--brand-500)' : 'var(--bg-elevated2)', border: `1px solid ${i === 3 ? 'var(--brand-500)' : 'var(--border)'}` }}>{c}</span>)}</div>
